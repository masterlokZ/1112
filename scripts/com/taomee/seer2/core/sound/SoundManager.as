package com.taomee.seer2.core.sound
{
   ...
   public class SoundManager
   {
      ...
      private static var _hjPlayer:SceneSoundPlayer;
      ...
      private static function initialize() : void
      {
         ...
         _hjPlayer = new SceneSoundPlayer();
      ...
      private static function get bgPlayer() : SceneSoundPlayer
      {
         return _bgPlayer;
      }
      ...
      private static function stopAll() : void
      {
         ...
         var _loc2_:Array = _soundPlayerMap.getKeys();
      ...
      private static function playBackSound() : void
      {
         ...
         {
            QueueLoader.load(URLUtil.getMapSoundSetting(),"text",onSettingDataLoaded);
         }
         else
         {
      ...
      private static function getCurrentMapHJSoundSetting() : String
      {
         ...
         var _loc2_:uint = uint(SceneManager.active.mapResourceID);
         var _loc1_:XMLList = _soundSettingData.elements("map");
         for each(_loc3_ in _loc1_)
         {
            if(_loc3_.attribute("id").toString() == _loc2_.toString())
            {
      ...
      private static function parseHJSound(param1:String) : void
      {
         ...
         if(param1 != "")
         {
            ...
            {
               if(!_hjPlayer.hasURL(_loc3_.url))
               {
                  _loc4_ = true;
                  break;
               }
            }
            updateHJPlayer(_loc4_,_loc2_);
         }
         else
         {
            _hjPlayer.stop();
      ...
      private static function parseBackSound(param1:String) : void
      {
         ...
         if(param1 != "")
         {
            ...
            for each(_loc3_ in _loc2_)
            {
               if(!_bgPlayer.hasURL(_loc3_.url))
               {
                  _loc4_ = true;
                  break;
               }
            }
            updateSoundPlayer(_loc4_,_loc2_);
         }
         else
         {
            _isBgPlaying = false;
      ...
      private static function updatePlayerList(param1:Boolean, param2:Array, param3:SceneSoundPlayer) : void
      {
      ...
      private static function parseSoundSetting(param1:String) : Array
      {
         ...
         while(_loc3_ < _loc7_)
         {
            _loc2_ = String(_loc8_[_loc3_]);
            _loc5_ = _loc2_.split(":");
            _loc4_ = String(_loc5_[0]);
            if(_loc4_ != "" && _loc4_ != null)
            {
               _loc6_.push({
                  "url":URLUtil.getMapSoundUrl(_loc4_),
                  "rep":Number(_loc5_[1])
               });
            }
            _loc3_++;
