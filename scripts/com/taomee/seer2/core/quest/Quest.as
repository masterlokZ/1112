package com.taomee.seer2.core.quest
{
   ...
   public class Quest
   {
      ...
      {
         var _loc6_:QuestClientBuffer = new QuestClientBuffer(this._clientBuffer.pack());
         if(param2)
         {
            _loc6_.setStepStatus(param1,49);
         }
         else
         {
            _loc6_.setStepStatus(param1,48);
         }
         var _loc5_:LittleEndianByteArray = new LittleEndianByteArray();
         var _loc4_:* = this.getStep(param1).outcome != null;
         var _loc3_:int = param2 && _loc4_ ? 1 : 0;
         _loc5_.writeByte(_loc3_);
         _loc5_.writeBytes(_loc6_.pack(),0);
         return _loc5_;
      }
      ...
      public function updateStepStatus(param1:int, param2:Boolean) : void
      {
         if(param2)
         {
            this._clientBuffer.setStepStatus(param1,49);
         }
         else
         {
            this._clientBuffer.setStepStatus(param1,48);
         }
      }
      ...
      public function isStepCompete(param1:int) : Boolean
      {
         ...
         if(this._status == 1)
         {
            _loc2_ = this._clientBuffer.getStepStatus(param1) == 49;
      ...
      public function getCurrentNpcIdVec() : Vector.<int>
      {
         ...
         if(this.status == 0)
         {
            _loc2_.push(this.acceptNpcId);
         }
         else if(this.status == 1)
         {
            for each(_loc1_ in this.getCurrentStepVec())
            {
               _loc2_.push(_loc1_.relatedNpcId);
            }
         }
         return _loc2_;
      }
      ...
      {
         var _loc1_:QuestStep = null;
         var _loc2_:Vector.<QuestStep> = new Vector.<QuestStep>();
         for each(_loc1_ in this._stepVec)
         {
            if(this.isStepOpen(_loc1_.id))
            {
               _loc2_.push(_loc1_);
            }
         }
      ...
      public function get isNewOnline() : Boolean
      {
         ...
         return (new Date().getTime() - this._onlineTime.getTime()) / (1000 * 86400) < 7;
      }
