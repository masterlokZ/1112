package com.taomee.seer2.core.scene
{
   ...
   public class LayerManager
   {
      ...
      public static function setup(param1:Sprite) : void
      {
         ...
         _stage.addEventListener("enterFrame",enterFrameHandler);
      ...
      public static function sceneMove() : void
      {
         ...
         if(_mapModel)
         {
            ...
            _loc7_ = -1;
            if(_mapModel.width <= _loc5_)
            {
               _loc6_ = 0;
               _loc1_ = _mapModel.width;
            }
            else
            {
               if(_actor)
               {
                  _loc6_ = clamp(_actor.x - _loc5_ / 2,0,_mapModel.width - _loc5_);
                  if(_mapModel.far.width > 0)
                  {
                     _loc7_ = _loc6_ * (_mapModel.width - _mapModel.far.width) / (_mapModel.width - _loc5_);
                  }
               }
               else
               {
                  _loc6_ = (_mapModel.width - _loc5_) / 2;
               }
               _loc1_ = _loc5_;
            }
            if(_mapModel.height <= _loc4_)
            {
               _loc2_ = 0;
               _loc3_ = _mapModel.height;
            }
            else
            {
               if(_actor)
               {
                  _loc2_ = clamp(_actor.y - _loc4_ / 2,0,_mapModel.height - _loc4_);
               }
               else
               {
                  _loc2_ = (_mapModel.height - _loc4_) / 2;
               }
               _loc3_ = _loc4_;
            }
            if(_mapModel.x != -_loc6_ || _loc6_ == 0)
            {
               _mapLayer.x = -_loc6_;
            }
            if(_mapModel.y != -_loc2_ || _loc2_ == 0)
            {
               _mapLayer.y = -_loc2_;
            }
            if(_loc7_ >= 0)
            {
               if(_mapModel.far.x != _loc7_)
               {
                  _mapModel.far.x = _loc7_;
               }
            }
            if(_movieContainer)
            {
               if(_movieContainer.x != -_loc6_)
               {
                  _movieContainer.x = -_loc6_;
               }
               if(_movieContainer.y != -_loc2_)
               {
                  _movieContainer.y = -_loc2_;
               }
            }
         }
      }
