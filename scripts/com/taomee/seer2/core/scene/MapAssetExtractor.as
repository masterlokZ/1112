package com.taomee.seer2.core.scene
{
   ...
   public class MapAssetExtractor
   {
      ...
      public function extractPath() : void
      {
         ...
         this._path.scaleX = this._path.scaleY = _loc2_;
         ...
         var _loc1_:BitmapData = new BitmapData(this._path.width,this._path.height,true,16777215);
         _loc1_.draw(this._path,new Matrix(_loc2_,0,0,_loc2_));
         this._pathData = new Array(this._horizontalGridNum);
         var _loc4_:int = 0;
         while(_loc4_ < this._horizontalGridNum)
         {
            this._pathData[_loc4_] = new Array(this._verticalGridNum);
            _loc3_ = 0;
            while(_loc3_ < this._verticalGridNum)
            {
               if(_loc1_.getPixel32(_loc4_,_loc3_) == 0)
               {
                  this._pathData[_loc4_][_loc3_] = true;
               }
               else
               {
                  this._pathData[_loc4_][_loc3_] = false;
               }
               _loc3_++;
            }
            _loc4_++;
      ...
      private function disableNonameChildren(param1:DisplayObjectContainer) : void
      {
         ...
         while(_loc3_ < _loc2_)
         {
            _loc4_ = param1.getChildAt(_loc3_) as InteractiveObject;
            if(_loc4_ != null)
            {
               if(this.hasGivenName(_loc4_) == false)
               {
      ...
      private function hasGivenName(param1:InteractiveObject) : Boolean
      {
         ...
         if(_loc3_ == null || _loc3_ == "" || StringUtil.beginsWith(_loc3_,_loc2_))
         {
            return false;
         }
         return true;
