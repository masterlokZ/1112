package com.taomee.seer2.core.ui.toolTip.tipSkins
{
   ...
   public class IconTipSkin extends BaseTipSkin
   {
      ...
      override public function show(param1:String) : void
      {
         ...
         var _loc3_:* = getDefinitionByName("com.taomee.seer2.app.component.IconDisplayer");
         var _loc5_:* = getDefinitionByName("com.taomee.seer2.app.config.ItemConfig");
         var _loc4_:* = getDefinitionByName("com.taomee.seer2.app.config.PetConfig");
         this._icon = new _loc3_();
         if(this._isPet)
         {
         ...
         {
            if(_loc5_.getSuitDefinition(this._id))
            {
               this._nameTxt.text = _loc5_.getSuitDefinition(this._id).name;
            }
            this._icon.setIconUrl(_loc5_.getMeeItemIconUrl(this._id));
            this._icon.x = 3;
            this._icon.y = 3;
         }
         else
         {
            this._nameTxt.text = _loc5_.getItemName(this._id);
            this._icon.setIconUrl(_loc5_.getItemIconUrl(this._id));
            this._icon.x = 3;
            this._icon.y = 3;
         }
         if(this._nameTxt.x + this._nameTxt.textWidth > _tipTxt.x + _tipTxt.textWidth)
         {
            _loc2_ = this._nameTxt.x + this._nameTxt.textWidth;
         }
         else
         {
            _loc2_ = _tipTxt.x + _tipTxt.textWidth;
         }
         if(_back)
         {
            _back.width = _loc2_ + 22;
