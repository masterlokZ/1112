   ...
   public class MysteryYiTeNoNoManager
   {
      ...
      private static function onComplete(param1:SceneEvent) : void
      {
         ...
         if(SceneManager.active)
         {
            ...
            _loc4_ = int(mapIDlist.indexOf(_loc2_));
            if(_loc4_ != -1)
            {
               ...
               if(_loc3_ < 0.5)
               {
                  StatisticsManager.sendNovice("0x1003374D");
                  hasNoNoAppearTime += 1;
                  if(hasNoNoAppearTime == 20)
                  {
                     SceneManager.removeEventListener("switchComplete",onComplete);
                  }
                  QueueLoader.load(URLUtil.getActivityAnimation(resPath[_loc4_]),"domain",onLoadComplete);
               }
            }
         }
      }
      ...
      private static function addInteractiveObj() : void
      {
         if(!hitNoNoLibs)
         {
            QueueLoader.load(URLUtil.getActivityAnimation("nonoZaChangZi/dianjinono"),"domain",onLoadNoNoComplete);
         }
         else
         {
            createNono();
      ...
      private static function createNono() : void
      {
         ...
         nono.addEventListener("clicked_ten_times",removeNoNo);
      }
      ...
      private static function removeNoNo(param1:Event) : void
      {
         if(SceneManager.active)
         {
            nono.removeEventListener("clicked_ten_times",removeNoNo);
            DisplayObjectUtil.removeFromParent(nono);
            nono.dispose();
            nono = null;
            QueueLoader.load(URLUtil.getActivityAnimation("nonoZaChangZi/nonodisappear"),"domain",onLoadDisappear);
         }
      }
