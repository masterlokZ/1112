package com.taomee.seer2.app.yilingshouevolution
{
   ...
   public class YiLingShouEvolutionManager
   {
      ...
      {
         QueueLoader.load(URLUtil.getActivityAnimation("yilingshouAct/yilingshouAct"),"swf",function(param1:ContentInfo):void
         {
      ...
      private static function initMc() : void
      {
         ...
         while(i < 6)
         {
            ...
            if(i < 3)
            {
               ...
               swapBtnList[i].addEventListener("click",onSwap);
      ...
      private static function onPing(param1:MouseEvent) : void
      {
         var e:MouseEvent = param1;
         pingBtn.visible = false;
         ServerBufferManager.getServerBuffer(354,function(param1:ServerBuffer):void
         {
            var server:ServerBuffer = param1;
            var _isPlay:Boolean = Boolean(server.readDataAtPostion(1));
            if(!_isPlay)
            {
               ServerBufferManager.updateServerBuffer(354,1,1);
      ...
      private static function firstPlayFullScreen(param1:Function = null) : void
      {
         ...
         ServerBufferManager.getServerBuffer(355,function(param1:ServerBuffer):void
         {
            var _loc2_:Boolean = Boolean(param1.readDataAtPostion(1));
            if(!_loc2_)
            {
               ServerBufferManager.updateServerBuffer(355,1,1);
      ...
      private static function creatNPC815() : void
      {
         ...
         _npc815.labelPosition = 1;
         _npc815.label = "坠天";
         _npc815.buttonMode = true;
         MobileManager.addMobile(_npc815,"npc");
         _npc815.addEventListener("click",onNpcClick);
         _mouseObj = new MouseClickHintSprite();
         _mouseObj.x = 0;
         _mouseObj.y = -170;
      ...
      public static function dispose() : void
      {
         if(_npc815)
         {
            _npc815.removeEventListener("click",onNpcClick);
            MobileManager.removeMobile(_npc815,"npc");
            _npc815 = null;
         }
         if(_npc307)
         {
