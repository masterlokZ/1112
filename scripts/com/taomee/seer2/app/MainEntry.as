package com.taomee.seer2.app
{
   ...
   public class MainEntry
   {
      ...
      public function initialize1(param1:Sprite, param2:Object) : void
      {
         ...
         TaomeeManager.stage.addEventListener("resize",this.onResize);
         this.onResize(null);
         try
         {
      ...
      private function onSwitchComplete(param1:SceneEvent) : void
      {
         ...
         var t:Object;
         var dynConfigItem:ContextMenuItem;
         var evt:SceneEvent = param1;
         TaomeeManager.stage.removeEventListener("resize",this.onResize);
         ...
         t = this._root.contextMenu;
         t.customItems.push(imageLevelItem);
         imageLevelItem.addEventListener("menuItemSelect",function(param1:ContextMenuEvent):void
         {
            getImageModuleShow();
         });
         dynConfigItem = new ContextMenuItem("重载配置");
         t.customItems.push(dynConfigItem);
         dynConfigItem.addEventListener("menuItemSelect",function(param1:ContextMenuEvent):void
         {
            DynConfig.loadConfigCallback(function():void
            {
      ...
      private function loginOnline() : void
      {
         ...
         Connection.addEventListener("connect",this.onSocketConnect);
         Connection.addEventListener("ioError",this.onSocketError);
         Connection.addEventListener("securityError",this.onSocketSecurityError);
         try
         {
      ...
      private function onSocketConnect(param1:Event) : void
      {
         ...
         Connection.removeEventListener("connect",this.onSocketConnect);
         Connection.removeEventListener("securityError",this.onSocketSecurityError);
      ...
      private function getTopLeftTmcid() : LittleEndianByteArray
      {
         ...
         {
            _loc2_.writeUTFBytes(GlobalsManager.tab);
         }
         else
         {
            _loc2_.writeUTFBytes("0");
         }
         return _loc2_;
      }
      ...
      private function onSocketError(param1:IOErrorEvent) : void
      {
         ...
         StatisticsManager.sendError("online_conn_failed");
      }
      ...
      private function onSocketSecurityError(param1:SecurityErrorEvent) : void
      {
         ...
         StatisticsManager.sendError("online_safe_sandbox");
         if(this._isDeputize)
         {
            return;
         }
      ...
      private function onLoginOnline(param1:MessageEvent) : void
      {
         ...
         {
            UILoader.load(URLUtil.getUISwf("UI"),"domain",onLoadUI);
         });
      }
      ...
      {
         BeanManager.addEventListener("complete",this.onBeanComplete);
         BeanManager.start("1");
      }
      
      private function onBeanComplete(param1:BeanEvent) : void
      {
         BeanManager.removeEventListener("complete",this.onBeanComplete);
         ...
         if(param1.id == "1")
         {
            ...
            {
               QuestManager.addEventListener("init",this.onQuestInit);
            }
         }
         else if(param1.id == "2")
         {
      ...
      {
         var _loc2_:uint = 0;
         QuestManager.removeEventListener("init",this.onQuestInit);
         ...
         if(QuestManager.isFreshQuestComplete())
         {
            ...
            if(SceneType.getTypeFromMapID(LoginInfo.mapId) == 5)
            {
      ...
      {
         var _loc1_:int = SceneType.getFromServerType(LoginInfo.sceneId);
         if(_loc1_ == 4 && TeamManager.teamId == 0)
         {
