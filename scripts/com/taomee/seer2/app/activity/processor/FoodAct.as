package com.taomee.seer2.app.activity.processor
{
   ...
   public class FoodAct
   {
      ...
      public function FoodAct(param1:MapModel)
      {
         ...
         this.getURL();
         StatisticsManager.sendNovice("0x10033813");
      }
      
      private function getURL() : void
      {
         QueueLoader.load(URLUtil.getActivityAnimation("foodAct"),"swf",function(param1:ContentInfo):void
         {
      ...
      {
         SceneManager.addEventListener("switchComplete",this.onSwitch);
         ...
         this._guo.addEventListener("click",this.askGoGuo);
         this._set.addEventListener("click",this.onSet);
         this._vip.addEventListener("click",this.onVip);
      ...
      {
         if(SceneManager.prevSceneType == 2 && FightManager.currentFightRecord.initData.positionIndex == 418)
         {
            this.removePet();
            this._petNum = 0;
         }
         if(SceneManager.prevMapID == 1421 && SceneManager.active.type != 2)
         {
            SceneManager.removeEventListener("switchComplete",this.onSwitch);
      ...
      {
         if(DateUtil.isInHourScope(13,15,0,0) || DateUtil.isInHourScope(18,20,0,0))
         {
            return true;
         }
         return false;
      ...
      private function addPet() : void
      {
         ...
         this._petNpc.addEventListener("click",this.onPetClick);
      }
      ...
      private function removePet() : void
      {
         if(this._petNpc)
         {
            this._petNpc.removeEventListener("click",this.onPetClick);
      ...
      {
         FightManager.startFightWithWild(418);
      }
      ...
      private function otherChange(param1:MessageEvent) : void
      {
         ...
         _loc2_ = ActorManager.getRemoteActor(_loc5_);
         if(_loc2_)
         {
            MorphManager.startRemoteMorph(_loc4_,_loc2_);
         }
      }
      ...
      private function updateGuo(param1:MessageEvent) : void
      {
         ...
         if(!this._idList)
         {
            ...
            while(i < 3)
            {
               if(this._idList[i] != 0)
               {
                  ...
                  if(tempActor)
                  {
                     ...
                     {
                        index3 = int(this.PET_LIST.indexOf(tempActor.getInfo().morphInfo.morphId));
                        if(index3 != -1)
                        {
            ...
            ActiveCountManager.requestActiveCount(202234,function(param1:uint, param2:uint):void
            {
               _petNum = param2;
               if(_petNum > 0)
               {
      ...
      {
         var _loc2_:int = int(param1.indexOf(ActorManager.actorInfo.id));
         if(_loc2_ != -1)
         {
      ...
      private function onDressUp(param1:MessageEvent) : void
      {
         ...
         var _loc6_:int = int(_loc5_.readUnsignedByte());
         var _loc3_:int = int(_loc5_.readUnsignedByte());
         if(_loc6_ == 0)
         {
            if(ActorManager.actorInfo.id == _loc7_)
            {
               ...
               _loc2_ = int(MorphSwitch.morphSwitch(_loc3_));
               FoodAct.dressCacheId = _loc2_;
               ActorManager.actorInfo.morphInfo.morphId = MorphSwitch.morphSwitch(_loc3_);
            ...
            else
            {
               ...
               if(_loc4_)
               {
                  _loc4_.getInfo().morphInfo.morphId = MorphSwitch.morphSwitch(_loc3_);
                  MorphManager.startRemoteMorph(MorphSwitch.morphSwitch(_loc3_),_loc4_);
               }
            }
         }
         else if(ActorManager.actorInfo.id == _loc7_)
         {
      ...
      private function getAddGuoUid(param1:Vector.<int>, param2:Vector.<int>) : int
      {
         ...
         {
            if(param1[_loc3_] != 0 && param2.indexOf(param1[_loc3_]) == -1)
            {
      ...
      private function getRemoveGuoUid(param1:Vector.<int>, param2:Vector.<int>) : int
      {
         ...
         {
            if(param2[_loc3_] != 0 && param1.indexOf(param2[_loc3_]) == -1)
            {
      ...
      private function foodGoGuoEffect(param1:int) : void
      {
         ...
         index = int(this.PET_LIST.indexOf(petId));
         if(index != -1)
         {
            res = this.getMovie("GoGuo" + (index + 1));
            this._mapModel.content.addChild(res);
            index1 = int(this._idList.indexOf(uid));
      ...
      public function dispose() : void
      {
         this._idList = null;
         this.removeCommand();
         this._guo.removeEventListener("click",this.askGoGuo);
         this._set.removeEventListener("click",this.onSet);
         this.removeChangeOkTip();
         this.removeCookingTip();
         this.removeSelfTip();
         this.removePet();
