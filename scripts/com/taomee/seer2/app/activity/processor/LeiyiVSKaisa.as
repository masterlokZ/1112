package com.taomee.seer2.app.activity.processor
{
   ...
   public class LeiyiVSKaisa
   {
      ...
      {
         var _loc1_:String = URLUtil.getActivityAnimation("FitKaiyi");
         QueueLoader.load(_loc1_,"swf",this.onLoadActiveResource);
      }
      ...
      private function onTimeLoad(param1:int) : void
      {
         ...
         else if(_loc4_ != 2)
         {
            if(_loc4_ == 3)
            {
               if(this._flag2 < _loc3_ && this._ispaly == false)
               {
      ...
      private function initMC() : void
      {
         ...
         Tick.instance.addRender(this.onTimeLoad,30000);
         var _loc1_:int = 0;
         var _loc3_:int = 0;
         if(DateUtil.inInDateScope(_loc2_.day,13,1,13,24))
         {
            _loc1_ = 1;
            _loc3_ = 1;
         }
         else if(DateUtil.inInDateScope(_loc2_.day,19,1,19,24))
         {
            _loc1_ = 1;
            _loc3_ = 2;
         }
         else if(DateUtil.inInDateScope(_loc2_.day,13,25,13,30))
         {
            _loc1_ = 2;
            _loc3_ = 1;
         }
         else if(DateUtil.inInDateScope(_loc2_.day,19,25,19,30))
         {
            _loc1_ = 2;
            _loc3_ = 2;
         }
         ...
         if(_loc1_ == 1)
         {
            if(SceneManager.prevSceneType == 2 && FightManager.currentFightRecord.initData.positionIndex == 40)
            {
         ...
         {
            if(SceneManager.prevSceneType == 2 && FightManager.currentFightRecord.initData.positionIndex == 155)
            {
               this.initFightNpc();
               if(FightManager.fightWinnerSide != 1)
               {
            ...
            else if(SceneManager.prevSceneType == 2 && FightManager.currentFightRecord.initData.positionIndex == 156)
            {
               this.initFightNpc();
               if(FightManager.fightWinnerSide != 1)
               {
                     ...
                     MovieClipUtil.playMc(_mc_1,1,_mc_1.totalFrames,function():void
                     {
                        NpcDialog.show(NpcUtil.getSeerNpcId(),"我",[[0,"凯萨……！凯萨！！振作起来啊！！不要向黑暗妥协！！我们相信你！！"]],["犯错并不可怕，可怕的是一直错下去！"],[function():void
                        {
                           ...
                           MovieClipUtil.playMc(_mc_2,1,_mc_2.totalFrames,function():void
                           {
                              ...
                              _loc1_.addEventListener("click",onClickButtonHandler);
                           },true);
                        }]);
                     },true);
                  }]);
      ...
      private function addNpc() : void
      {
         ...
         this._split.addEventListener("click",this.onFightKaiyiHandler);
      }
         ...
         NpcDialog.show(511,"影灵兽",[[0,"很好……很好……你们的体力已经完全透支，不久以后，我们会在死亡的黑暗深渊重聚……！"]],["不会的！他们不会倒下！！"],[function():void
         {
            NpcDialog.show(512,"凯伊",[[0,"我……已经……"]],["雷伊……不，凯萨……你们怎么了！！"],[function():void
            {
               ...
               MovieClipUtil.playMc(_mc_4,1,_mc_4.totalFrames,function():void
               {
                  ...
                  _sickLeiyi.addEventListener("click",onLeiyiSayHandler);
                  _sickKaisa.addEventListener("click",onKaisaSayHandler);
               },true);
            }]);
         }]);
      }
      ...
      private function initFightNpc() : void
      {
         ...
         this._leiyi.addEventListener("click",this.onFightLeiyiHandler);
         this._kaisa.addEventListener("click",this.onFightKaisaHandler);
      }
