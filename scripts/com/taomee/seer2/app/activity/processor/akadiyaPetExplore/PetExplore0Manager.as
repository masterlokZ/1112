package com.taomee.seer2.app.activity.processor.akadiyaPetExplore
{
   ...
   public class PetExplore0Manager
   {
      ...
      {
         _fightIndex = 0;
         SceneManager.changeScene(9,80558);
      ...
      private static function initNpc() : void
      {
         ...
         resUrl = URLUtil.getRes("activity/animation/akadiya/nilu.swf");
         ...
         if(SceneManager.prevSceneType == 2)
         {
            if(_fightId == FightManager.currentFightRecord.initData.positionIndex)
            {
               if(FightManager.currentFightRecord.isWinWar())
               {
                  ...
                  if(_fightIndex == 2)
                  {
                     SceneManager.changeScene(9,80559);
                  }
                  initNpc();
               }
               else
      ...
      {
         var _loc2_:Array = [400239,400240];
         var _loc1_:int = 0;
         while(_loc1_ < _loc2_.length)
         {
            if(ItemManager.getItemQuantityByReferenceId(_loc2_[_loc1_]) < 10)
            {
               return false;
            }
            _loc1_++;
