package com.taomee.seer2.app.actor.equip
{
   ...
   public class ActorEquipHelper
   {
      ...
      private var _equipPlayer:MultipleEquipPlayer;
      ...
      public function ActorEquipHelper(param1:Actor)
      {
         super();
         this._actor = param1;
         this._equipPlayer = new MultipleEquipPlayer(this._actor);
      ...
      public function initEquip() : void
      {
         ...
         var _loc3_:int = 0;
         var _loc2_:Vector.<EquipItem> = this._actor.getInfo().equipVec.concat();
         for each(_loc1_ in _loc2_)
         {
            this.addEquip(_loc1_.referenceId);
         }
         _loc4_ = this._actor.getInfo().ridingPetInfo;
         if(_loc4_)
         {
            if(_loc4_.resourceId != 0)
            {
               _loc3_ = PetRideShopConfig.getEquipIdByPetId(_loc4_.resourceId);
               if(PetRideShopConfig.getEquipIdByPetId(_loc3_))
               {
                  this.addEquip(_loc3_);
               }
            }
            else if(_loc4_.petRideChipId != 0)
            {
               this.addEquip(PetRideShopConfig.getEquipIdByChipBackId(_loc4_.petRideChipId));
            }
         }
         this.checkAddDogz();
      }
      ...
      public function updateEquipVec(param1:Vector.<EquipItem>) : void
      {
         ...
         var _loc2_:EquipItem = null;
         var _loc3_:Vector.<EquipItem> = this.getDifferenceVec(this._equipItemVec,param1);
         var _loc5_:Vector.<EquipItem> = this.getDifferenceVec(param1,this._equipItemVec);
         for each(_loc4_ in _loc3_)
         {
            this.removeEquip(_loc4_.referenceId);
            if(_loc4_.slotIndex == 3)
            {
         ...
         for each(_loc2_ in _loc5_)
         {
            ...
            if(_loc2_.slotIndex == 3)
            {
               this.updateDogzState(true,_loc2_.referenceId);
            }
         }
      }
      ...
      private function getDifferenceVec(param1:Vector.<EquipItem>, param2:Vector.<EquipItem>) : Vector.<EquipItem>
      {
         ...
         {
            if(this.isEquipExistInEquipVec(_loc3_,param2) == false)
            {
               _loc4_.push(_loc3_);
            }
         }
      ...
      {
         var _loc2_:int = this._equipItemVec.length - 1;
         while(_loc2_ >= 0)
         {
