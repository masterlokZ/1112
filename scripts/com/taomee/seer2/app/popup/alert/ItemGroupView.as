package com.taomee.seer2.app.popup.alert
{
   ...
   public class ItemGroupView extends Sprite implements IAlert
   {
      ...
      public function ItemGroupView()
      {
         ...
         while(_loc2_ < 3)
         {
            _loc1_ = this._mainUI["mc_" + _loc2_];
            _loc1_.buttonMode = true;
            _loc1_.addEventListener("click",this.onMcClick);
            this._mcVec.push(_loc1_);
            _loc3_ = new ItemIcon(_loc1_.width,_loc1_.height);
            _loc3_.x = _loc1_.x;
            _loc3_.y = _loc1_.y;
         ...
         this._chooseBtn.addEventListener("click",this.onChooseBtnClick);
      ...
      private function getGroupVec() : Vector.<ItemGroup>
      {
         ...
         var _loc2_:Vector.<ItemGroup> = new Vector.<ItemGroup>();
         var _loc1_:Vector.<ItemGroup> = ItemGroupConfig.getItemGroupVec(this._category);
         var _loc4_:int = 0;
         while(_loc4_ < 3)
         {
            if(_loc4_ != this._selectedIndex)
            {
               do
               {
                  _loc3_ = _loc1_[Math.floor(Math.random() * _loc1_.length)];
               }
               while(_loc1_.indexOf(_loc3_) == -1 && this._group != _loc3_);
               
               _loc2_.push(_loc3_);
            }
            else
            {
               _loc2_.push(this._group);
            }
            _loc4_++;
         }
         return _loc2_;
