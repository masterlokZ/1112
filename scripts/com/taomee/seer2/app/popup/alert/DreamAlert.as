package com.taomee.seer2.app.popup.alert
{
   ...
   public class DreamAlert extends Sprite implements IAlert
   {
      ...
      private function initEvent() : void
      {
         ...
         this._fightCountBtn.addEventListener("click",this.onFightCount);
         this._autoFightBtn.addEventListener("click",this.onAuto);
         this._entryMapBtn.addEventListener("click",this.onEntryMap);
      }
      ...
      private function onEntryMap1557(param1:MessageEvent) : void
      {
         ...
         {
            this._dreamOneMC2.visible = false;
            ServerBufferManager.updateServerBuffer(350,3,1);
         }
         SceneManager.changeScene(1,this._dreamInfo.mapId);
      }
      ...
      public function show(param1:AlertInfo) : void
      {
         ...
         ItemManager.addEventListener1("requestSpecialItemSuccess",this.onSuccess);
      ...
      private function onDreamFamilyInfoOne(param1:MessageEvent) : void
      {
         ...
         this._starMC.gotoAndStop(_loc6_ + 1);
         var _loc3_:int = 0;
         while(_loc3_ < _loc6_)
         {
            ColorFilter.setGrayscale(this._pointTxtList[_loc3_]);
            _loc3_++;
         }
         var _loc2_:int = int(ItemManager.getSpecialItem(605616) ? int(ItemManager.getSpecialItem(605616).quantity) : 0);
         this._fightCountTxt.text = _loc5_ >= 999 ? "不限制次数" : "本关次数上限:" + (String(_loc5_ + _loc2_));
         _loc5_ += _loc2_;
         if(_loc5_ >= 999)
         {
      ...
      public function dispose() : void
      {
         ...
         {
            this._fightCountBtn.removeEventListener("click",this.onFightCount);
         }
         if(this._autoFightBtn)
         {
            this._autoFightBtn.removeEventListener("click",this.onAuto);
         }
         this._closeBtn = null;
