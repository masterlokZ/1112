package com.taomee.seer2.app.popup.alert
{
   ...
   public class GetPetInBagAlert extends Sprite implements IAlert
   {
      ...
      public function GetPetInBagAlert()
      {
         super();
         this.initialize();
      }
      ...
      private function initialize() : void
      {
         ...
         this._confirmBtn.addEventListener("click",this.onConfirmBtnClick);
         ...
         while(_loc1_ < 5)
         {
            ...
            this._btnList[_loc1_].addEventListener("click",this.onBtn);
            _loc1_++;
         }
         addChild(this._ui);
         this.funcBtnFilter();
      ...
      {
         var _loc7_:BuyPropInfo = null;
         var _loc6_:Object = null;
         var _loc3_:uint = 0;
         var _loc2_:uint = 0;
         var _loc4_:String = null;
         var _loc5_:int = int(this._btnList.indexOf(param1.currentTarget as SimpleButton));
         if(_loc5_ < 2)
         {
            _loc7_ = new BuyPropInfo();
            _loc7_.itemId = this.moduleList[_loc5_];
            ShopManager.buyBagItem(_loc7_);
         }
         else if(_loc5_ < 3)
         {
            _loc6_ = {};
            _loc6_.type = 0;
            _loc6_.subType = 2;
            ModuleManager.showModule(URLUtil.getAppModule(this.moduleList[_loc5_]),"",_loc6_);
         }
         else if(_loc5_ < 4)
         {
            _loc3_ = PetConfig.getPetDefinition(this._info.referenceId).emblemId;
            if(_loc3_ == 0)
            {
               return;
            }
            _loc2_ = ItemConfig.getEmblemDefinition(_loc3_).miBuyID;
            if(_loc2_ == 0)
            {
               return;
            }
         ...
         {
            _loc4_ = PetConfig.getPetDefinitionInfo(this._info.referenceId).superModule;
            ModuleManager.showModule(URLUtil.getAppModule(_loc4_),"");
         }
      }
      ...
      private function onConfirmBtnClick(param1:MouseEvent) : void
      {
         ...
         this._confirmBtn.removeEventListener("click",this.onConfirmBtnClick);
      ...
      {
         var _loc2_:String = URLUtil.getPetIcon(this._info.referenceId);
         var _loc1_:String = PetConfig.getPetDefinition(this._info.referenceId).name;
         this._iconDisplay.setIconUrl(_loc2_);
         this._contentTxt.htmlText = "精灵:<font color=\'#ff0000\'>" + _loc1_ + "</font>已经放入精灵背包";
         this._nameTxt.text = _loc1_;
      }
