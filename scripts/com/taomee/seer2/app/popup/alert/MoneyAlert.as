package com.taomee.seer2.app.popup.alert
{
   ...
   public class MoneyAlert extends Sprite implements IAlert
   {
      ...
      private function setup() : void
      {
         ...
         this._okBtn.addEventListener("click",this.onOk);
         this._chpherCancelBtn.addEventListener("click",this.onCancelBtn);
         this._currContentTxt.addEventListener("change",this.onChangeTxt);
         ...
         this._cancelBtn.addEventListener("click",this.onCancelBtnClick);
         this._ui["goBtn"].addEventListener("click",function(param1:MouseEvent):void
         {
            dispose();
      ...
      private function onOk(param1:MouseEvent) : void
      {
         ...
         var _loc4_:int = 0;
         var _loc5_:LittleEndianByteArray = new LittleEndianByteArray();
         var _loc7_:String = MD5.hash(this._entryTxt.text);
         var _loc6_:int = _loc7_.length;
         var _loc3_:int = 0;
         while(_loc3_ < _loc6_)
         {
            _loc2_ = _loc7_.substr(_loc3_,2);
            _loc4_ = parseInt(_loc2_,16);
            _loc5_.writeByte(_loc4_);
            _loc3_ += 2;
         }
         this.sendMoney(_loc5_);
      }
      ...
      private function onConfirmBtnClick(param1:MouseEvent) : void
      {
         ...
         PayManager.checkSetopChpher(function(param1:Boolean):void
         {
            ...
            if(b)
            {
               ...
               {
                  var _loc8_:LittleEndianByteArray = null;
                  ...
                  {
                     _loc8_ = new LittleEndianByteArray();
                     _loc7_ = MD5.hash("0");
                     _loc4_ = _loc7_.length;
                     _loc3_ = 0;
                     while(_loc3_ < _loc4_)
                     {
                        _loc6_ = _loc7_.substr(_loc3_,2);
                        _loc5_ = parseInt(_loc6_,16);
                        _loc8_.writeByte(_loc5_);
                        _loc3_ += 2;
                     }
                     sendMoney(_loc8_);
                  }
      ...
      {
         Connection.removeCommandListener(CommandSet.CLI_BUY_MONEY_COUNT_1254,this.onCommand);
         this._currContentTxt.removeEventListener("change",this.onChangeTxt);
         this._confirmBtn.removeEventListener("click",this.onConfirmBtnClick);
         this._cancelBtn.removeEventListener("click",this.onCancelBtnClick);
         this._confirmHandler = null;
         this._cancelHandler = null;
         this._ui = null;
         this._leftBtn = null;
