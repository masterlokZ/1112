package com.taomee.seer2.app.popup.alert
{
   ...
   public class DoorResultAlert extends Sprite implements IAlert
   {
      ...
      public function dispose() : void
      {
         ...
         this._confirmBtn.removeEventListener("click",this.onConfirmHandler);
      ...
      private function onGetLib(param1:ResourceLibrary) : void
      {
         ...
         if(this._result)
         {
            if(this._doorType == 0)
            {
               if(this._doorRule == 1 || this._doorRule == 5)
               {
                  _loc2_ = param1.getMovieClip("DoorWinResultUI");
                  this._petPanel = _loc2_["petPanel"];
                  this.updatePetInfo();
               }
               else if(this._doorRule == 2 || this._doorRule == 3 || this._doorRule == 4)
               {
                  ...
                  else if(this._doorRule == 3)
                  {
                     this._support = AtleticoDoorSupport.getInstance();
                  }
                  else if(this._doorRule == 4)
                  {
                  ...
                  this._doorResultSwitchPetPanel.addEventListener("loadCompleteShowPanel",this.onLoadCompleteShowPanel);
                  this._doorResultSwitchPetPanel.addEventListener("selectSwitchComplete",this.onSwitchComplete);
                  addChild(this._doorResultSwitchPetPanel);
                  return;
               }
            }
            else if(this._doorType == 1)
            {
               if(this._equipId == 1)
               {
         ...
         this._confirmBtn.addEventListener("click",this.onConfirmHandler);
      }
      
      private function onLoadCompleteShowPanel(param1:Event) : void
      {
         this._doorResultSwitchPetPanel.removeEventListener("loadCompleteShowPanel",this.onLoadCompleteShowPanel);
         AlertManager.addPopUp(this._info,this);
      }
      ...
      {
         this.dispose();
         if(this._result == false || this._doorRule != 5)
         {
            Connection.send(CommandSet.DOOR_OUT_1049);
            SceneManager.changeScene(1,DOOR_TARGETMAPID[this._doorRule]);
         }
      }
      
      private function onSwitchComplete(param1:Event) : void
      {
         this._doorResultSwitchPetPanel.removeEventListener("selectSwitchComplete",this.onSwitchComplete);
      ...
      private function showGetPetPanel() : void
      {
         ...
         var _loc2_:MovieClip = this._lib.getMovieClip("DoorWinResultUI");
         _loc2_.y = 60;
         this._petPanel = _loc2_["petPanel"];
         this.updatePetInfo();
         addChild(_loc2_);
         LayerManager.focusOnTopLayer();
         var _loc1_:uint = uint((this._support as IDoorServerPetable).petId);
         var _loc3_:uint = uint((this._support as IDoorServerPetable).resourceId);
         this._support.rewardIDList = null;
         PetInfoManager.requestAddToBagFromStorage(_loc1_,_loc3_);
         this._confirmBtn = _loc2_["confirmBtn"];
         this._confirmBtn.addEventListener("click",this.onConfirmHandler);
      }
      ...
      private function updatePetInfo() : void
      {
         ...
         if(this._petInfo != null)
         {
            ...
            url = URLUtil.getPetIcon(this._petInfo.resourceId);
