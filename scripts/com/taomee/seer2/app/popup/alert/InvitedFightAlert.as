package com.taomee.seer2.app.popup.alert
{
   ...
   public class InvitedFightAlert extends Sprite implements IAlert
   {
      ...
      private function onGetLib(param1:ResourceLibrary) : void
      {
         ...
         this._cacelButton.addEventListener("click",this.cancelHandler);
      ...
      private function showPanel() : void
      {
         ...
         confirmHandler = function(param1:Vector.<uint>):void
         {
            ...
            _loc4_.writeUnsignedInt(_userId);
            _loc4_.writeUnsignedInt(1);
            _loc4_.writeUnsignedInt(param1.length);
            var _loc6_:uint = 0;
            while(_loc6_ < param1.length)
            {
               _loc4_.writeUnsignedInt(param1[_loc6_]);
               _loc6_++;
            }
            Connection.send(CommandSet.FIGHT_ACCEPT_PVP_1513,_loc4_);
            var _loc5_:Array = [];
            for each(_loc3_ in param1)
            {
               _loc5_.push(_loc3_);
            }
            _loc2_ = SOMultiBehavior.getKey(_fightMode);
            SOMultiBehavior.writeDefaultPets(_loc2_,_loc5_);
            stopInterval();
            dispose();
         };
      ...
      private function intervalHandler() : void
      {
         ...
         ++this._intervalIndex;
         this._timeTxt.text = String(15 - this._intervalIndex);
         if(this._intervalIndex >= 15)
         {
            this.stopInterval();
            this._timerIndex = 0;
