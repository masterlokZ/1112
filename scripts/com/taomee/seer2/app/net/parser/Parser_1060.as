package com.taomee.seer2.app.net.parser
{
   ...
   public class Parser_1060
   {
      ...
      public function addItem(param1:Boolean = true, param2:Function = null, param3:Boolean = true) : void
      {
         ...
         var _loc6_:int = 0;
         var _loc8_:uint = this.addItemVec.length;
         var _loc5_:Vector.<ItemDescription> = new Vector.<ItemDescription>();
         var _loc4_:uint = 0;
         while(_loc4_ < _loc8_)
         {
            if(this.addItemVec[_loc4_].id == 1)
            {
               ActorManager.actorInfo.coins += this.addItemVec[_loc4_].count;
               if(param1)
               {
                  _loc5_.push(new ItemDescription(this.addItemVec[_loc4_].id,this.addItemVec[_loc4_].count,this.addItemVec[_loc4_].expireTime));
               }
            }
            else if(19 != this.addItemVec[_loc4_].id)
            {
               _loc7_ = false;
               _loc6_ = 0;
               while(_loc6_ < _loc5_.length)
               {
                  if(_loc5_[_loc6_].referenceId == this.addItemVec[_loc4_].id)
                  {
                     _loc5_[_loc6_].quantity += this.addItemVec[_loc4_].count;
                     _loc7_ = true;
                     break;
                  }
                  _loc6_++;
      ...
      {
         var _loc2_:uint = this.removeItemVec.length;
         var _loc1_:uint = 0;
         while(_loc1_ < _loc2_)
         {
            if(this.removeItemVec[_loc1_].id == 1)
            {
               ActorManager.actorInfo.coins -= this.removeItemVec[_loc1_].count;
            }
            else
            {
               ItemManager.reduceItemQuantity(this.removeItemVec[_loc1_].id,this.removeItemVec[_loc1_].count);
            }
            _loc1_++;
         }
      }
