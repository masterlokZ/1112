package com.taomee.seer2.app.net
{
   ...
   public class Connection
   {
      ...
      private static var _endian:String = "littleEndian";
      ...
      private static function initialize() : void
      {
         ...
         _socket.addEventListener("data",onSocketData);
      }
      ...
      {
         var _loc7_:BlockedCommandStructure = null;
         var _loc6_:Vector.<Array> = null;
         var _loc3_:Array = null;
         var _loc2_:Vector.<Message> = null;
         var _loc4_:Message = null;
         var _loc5_:uint = param1.id;
         if(_blockedMap.containsKey(_loc5_) == true)
         {
            _loc7_ = _blockedMap.remove(_loc5_);
            _loc6_ = _loc7_.requestArgs;
            for each(_loc3_ in _loc6_)
            {
               send(param1,_loc3_);
            }
            _loc2_ = _loc7_.messageVec;
      ...
      {
         var _loc5_:BlockedCommandStructure = null;
         var _loc4_:Vector.<Array> = null;
         var _loc3_:ByteArray = null;
         if(_blockedMap.containsKey(param1.id))
         {
            _loc5_ = _blockedMap.getValue(param1.id);
            _loc4_ = _loc5_.requestArgs;
            _loc4_.push(rest);
         }
         else
         {
            _loc3_ = RequestPacker.pack(_userId,param1.id,rest,_endian);
            _socket.send(_loc3_);
            _logger.info("C2S: " + Command.findDescriptionById(param1.id) + " dataLength: " + (_loc3_.length - 18));
         }
      }
