package com.taomee.seer2.app.robot
{
   ...
   public class RobotActor extends Actor
   {
      ...
      override public function updateNick(param1:String) : void
      {
         ...
         {
            _nickImage = LabelRenderer.getLabelImage(param1,16776960,3342336);
         }
         else
         {
      ...
      {
         this.getActorAttach("chat").excute(param1);
      }
      
      override public function addShootEffect(param1:ShootHitEffect) : void
      {
         this.getActorAttach("shoot").excute({
            "effect":param1,
            "isAdd":true
         });
      }
      ...
      private function initActorAttach() : void
      {
         ...
         var _loc2_:ChatActorAttach = new ChatActorAttach();
         _loc2_.attach(this);
         _acttachVec.push(_loc2_);
         var _loc1_:ShootActorAttach = new ShootActorAttach();
         _loc1_.attach(this);
      ...
      private function initEventListener() : void
      {
         ...
         this.addEventListener("addedToStage",this.onAddToStage);
         this.addEventListener("removedFromStage",this.onRemoveFromStage);
         _hotObject.addEventListener("click",this.onMouseClick);
         _userInfo.addEventListener("followingPetUpdate",this.onFollowingPetUpdate);
         _userInfo.addEventListener("equipUpdate",this.onEquipUpdate);
         _userInfo.addEventListener("trainerScoreUpdate",this.onTrainerScoreUpdate);
         _userInfo.vipInfo.addEventListener("vipUpdate",this.onVipUpdate);
         _userInfo.addEventListener("NONOINFO_UPDATE",this.updateNonoInfo);
         NonoUtil.addEventListener("NONO_FLAG_CHANGE",this.updateNonoInfo);
         PetInfoManager.addEventListener("petPropertiesChange",this.onPetPropertiesChange);
         Tick.instance.addRender(this.checkRange,1000);
         addEventListener("finished",this.onActorMoveFinished);
         addEventListener("start",this.onActorMoveStart);
      }
      
      private function disposeEventListener() : void
      {
         this.removeEventListener("addedToStage",this.onAddToStage);
         this.removeEventListener("removedFromStage",this.onRemoveFromStage);
         _hotObject.removeEventListener("click",this.onMouseClick);
         _userInfo.removeEventListener("followingPetUpdate",this.onFollowingPetUpdate);
         _userInfo.removeEventListener("equipUpdate",this.onEquipUpdate);
         _userInfo.removeEventListener("trainerScoreUpdate",this.onTrainerScoreUpdate);
         _userInfo.vipInfo.removeEventListener("vipUpdate",this.onVipUpdate);
         _userInfo.removeEventListener("NONOINFO_UPDATE",this.updateNonoInfo);
         NonoUtil.removeEventListener("NONO_FLAG_CHANGE",this.updateNonoInfo);
         ...
         removeEventListener("finished",this.onActorMoveFinished);
         removeEventListener("start",this.onActorMoveStart);
      }
