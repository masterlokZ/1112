package com.taomee.seer2.app.dreamlandchest
{
   ...
   public class DreamlandChestManager
   {
      ...
      public static function getRandomObj() : Array
      {
         ...
         var _loc1_:int = 0;
         var _loc3_:Array = [];
         while(_loc3_.length >= 5)
         {
            _loc2_ = {};
            _loc5_ = Math.random() * homeObj.length;
            _loc4_ = int(homeObj[_loc5_].objs.length);
            _loc2_["mapID"] = homeObj[_loc5_].mapID;
            _loc2_["obj"] = homeObj[_loc5_].objs[_loc4_];
            _loc1_ = 0;
            while(_loc1_ < _loc3_.length)
            {
               if(_loc2_["mapID"] != _loc3_[_loc1_]["mapID"])
               {
                  _loc3_.push(_loc2_);
               }
               _loc1_++;
      ...
      public static function init(param1:MapModel) : void
      {
         ...
         QueueLoader.load(URLUtil.getActivityAnimation("dreamlandchest/DreamlandChestAnimation"),"swf",onResLoaded);
      }
               ...
               DayLimitListManager.getDaylimitList(DAY_LIST,function(param1:DayLimitListInfo):void
               {
                  ...
                  if(info.getCount(DAY_LIST[0]) > 0)
                  {
                     ...
                     MovieClipUtil.playMc(wordMc,2,wordMc.totalFrames,function():void
                     {
                        ...
                        var _loc2_:int = getHomeIdx(mapID);
                        var _loc1_:int = Math.random() * homeObj[_loc2_]["objs"].length;
                        var _loc3_:String = String(homeObj[_loc2_]["objs"][_loc1_]);
                        if(contentMc[_loc3_])
                        {
                           ...
                           contentMc[_loc3_].addEventListener("click",onSwap);
                        }
                     });
                  }
               });
      ...
      private static function onSwap(param1:MouseEvent) : void
      {
         ...
         e.currentTarget.removeEventListener("click",onSwap);
         childIdx = int(_map.front.numChildren - 1 < 0 ? 0 : _map.front.numChildren - 1);
