package com.taomee.seer2.app.yiteSuper
{
   ...
   public class AnyiteSuperManager
   {
      ...
      public static function startGame(param1:int) : void
      {
         ...
         _isStart = true;
         SceneManager.changeScene(9,_loc2_);
      ...
      public static function checkEnterLimit() : Boolean
      {
         ...
         var _loc4_:Array = [10,11,12,608];
         var _loc3_:int = int(ActorManager.actorInfo.followingPetInfo == null ? 0 : int(ActorManager.actorInfo.followingPetInfo.resourceId));
         if(ActorManager.actorInfo.followingPetInfo == null || _loc4_.indexOf(_loc3_) == -1)
         {
         ...
         while(_loc2_ < _loc6_.length)
         {
            ...
            if(_loc1_.getPetDefinition().starLevel > 4)
            {
               _loc5_ = false;
               break;
            }
         ...
         if(_loc5_ == false || _loc6_.length >= 6)
         {
      ...
      public static function initMap(param1:MapModel = null) : void
      {
         ...
         {
            SceneManager.active.mapModel.front["exitBtn"].addEventListener("click",exitMap);
         }
         if(!_isStart)
         {
         ...
         if(_petIcon != null)
         {
            ...
            _petIcon.removeEventListener("click",onPetClick);
         ...
         _petIcon.addEventListener("click",onPetClick);
      ...
      public static function leaveMap() : void
      {
         ...
         if(SceneManager.active.mapModel.front["exitBtn"])
         {
            SceneManager.active.mapModel.front["exitBtn"].removeEventListener("click",exitMap);
         }
      }
      
      public static function onPetClick(param1:* = null) : void
      {
         var _loc3_:Array = getNpcDialogs(_currentLevel,_currentBossIndex);
         var _loc5_:int = getNpcId(_currentLevel,_currentBossIndex);
         var _loc4_:int = getPetId(_currentLevel,_currentBossIndex);
         var _loc2_:String = PetConfig.getPetDefinition(_loc4_).name;
         NpcDialog.show(_loc5_,_loc2_,[[0,_loc3_[0]]],[_loc3_[1],"→进入挑战"],[cancelFight,fight]);
      }
