package com.taomee.seer2.app.debugTools
{
   ...
   public class SimulateSendingProtocolPanel extends Sprite
   {
      ...
      private function initset() : void
      {
         ...
         this._msgContentTxt.text = DynSwitch.simulateSendingProtocolHint || "协议号:1055 兑换 1224 购买 1500 对战\n兑换类参数:\n2413 约瑟 4658 养成 4659 星屑 3915 神迹米咔\n对战类参数:\n1095 龙魄 1432 阿兹尔 1505 无限D药 1655 罗澜 1897 雷克萨斯";
      ...
      {
         this._sendBtn.addEventListener("click",this.onSendBtn);
         this._clearBtn.addEventListener("click",this.onClearBtn);
         this._scrollBar.addEventListener("move",this.onScrollerMove);
      ...
      private function cloneParametersVector(param1:Vector.<ParameterInfo>) : Vector.<ParameterInfo>
      {
         ...
         {
            _loc3_ = new ParameterInfo(_loc4_.pid,_loc4_.name,_loc4_.type,_loc4_.isArray,_loc4_.length,_loc4_.fixedLength,_loc4_.description);
            if(_loc4_.type == "struct")
            {
               _loc3_.structList = this.cloneParametersVector(_loc4_.structList);
            }
            _loc2_.push(_loc3_);
         }
         return _loc2_;
      ...
      {
         var _loc7_:ParameterInfo = null;
         var _loc10_:Array = null;
         var _loc5_:LittleEndianByteArray = null;
         var _loc12_:String = "";
         ...
         while(param2.length > 0 && param3.length > 0)
         {
            ...
            if(_loc9_ != -1)
            {
               _loc12_ = _loc12_.slice(0,_loc9_);
            }
            if(_loc7_.type == "struct")
            {
               _loc11_ = false;
               ...
               {
                  _loc10_ = [];
                  param2.unshift(_loc12_);
                  this.parserParamArray(_loc10_,param2,_loc7_.structList);
                  param1.push(_loc10_);
               }
            }
            else
            {
               _loc5_ = new LittleEndianByteArray();
               this.writeTempData(_loc5_,_loc7_.type,_loc12_,_loc7_.fixedLength);
               param1.push(_loc5_);
               if(_loc8_)
               {
                  param3.unshift(_loc7_);
               }
               if(_loc8_ == false && _loc4_ > 0)
               {
                  param1.splice(param1.length - _loc4_,0,_loc4_);
                  _loc4_ = 0;
               }
            }
         }
      ...
      private function writeTempData(param1:LittleEndianByteArray, param2:String, param3:String, param4:int = 0) : void
      {
         ...
         else if(param2 == "char")
         {
            ...
            {
               param1.writeBytes(_loc6_,0,param4);
               if(_loc5_ < param4)
               {
                  _loc7_ = 0;
                  _loc7_ = 0;
                  while(_loc7_ < param4 - _loc5_)
                  {
                     param1.writeByte(0);
                     _loc7_++;
                  }
            ...
            readData = function():void
            {
               ...
               else
               {
                  ...
                  else if(tempParam.type == "char")
                  {
                     ...
                     {
                        _loc2_ = int(data.readUnsignedInt());
                        _loc1_ = 0;
                        _loc1_ = 0;
                        while(_loc1_ < _loc2_)
                        {
      ...
      private function buyItemHandler(param1:String) : void
      {
         ...
         var _loc2_:uint = 0;
         var _loc4_:BuyPropInfo = new BuyPropInfo();
         _loc3_ = param1.split(",");
         if(_loc3_.length <= 0)
         {
         ...
         _loc4_.itemId = _loc2_;
         if(isNaN(Number(_loc3_[1])) == false)
         {
            _loc4_.buyNum = uint(_loc3_[1]);
         }
         if(_loc2_ <= 500500 || _loc2_ > 800000)
         {
            ShopManager.buyBagItem(_loc4_);
         }
         else
         {
            ShopManager.buyVirtualItem(_loc4_);
         }
      }
