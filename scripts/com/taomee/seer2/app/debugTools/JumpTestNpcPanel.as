package com.taomee.seer2.app.debugTools
{
   ...
   public class JumpTestNpcPanel extends Sprite
   {
      ...
      {
         this._panelJumpBtn.addEventListener("click",this.onPanelJumpBtn);
         ...
         this._mapJumpBtn.addEventListener("click",this.onMapJumpBtn);
         this._npcDragSwitchBtn.addEventListener("click",this.onNpcSwitchBtn);
         this._confirmBtn.addEventListener("click",this.onConfirmBtn);
         this._mainUI["testDialogBtn"].addEventListener("click",this.onTestDialogBtnClick);
         var _loc1_:int = 0;
         _loc1_ = 0;
         while(_loc1_ < this._tabBtnList.length)
         {
            this._tabBtnList[_loc1_].addEventListener("click",this.onTabBtn);
            _loc1_++;
         }
         ...
         {
            this._panelItemMcList[_loc1_].addEventListener("click",this.onPanelItemMc);
            _loc1_++;
         }
      }
      ...
      {
         var _loc2_:URLRequest = new URLRequest("http://10.1.1.159/cgi-bin/HelloWorld.exe");
      ...
      private function onUrlLoaderComplete(param1:Event) : void
      {
         ...
         URLLoader(param1.currentTarget).removeEventListener("complete",this.onUrlLoaderComplete);
      ...
      private function onPanelDataComplete(param1:Event) : void
      {
         ...
         URLLoader(param1.currentTarget).removeEventListener("complete",this.onPanelDataComplete);
         var _loc2_:Object = URLLoader(param1.currentTarget).data;
         if(_loc2_ == null)
         {
            return;
         ...
         while(_loc3_ <= 20)
         {
            if(_loc2_.hasOwnProperty("p" + _loc3_))
            {
               this._panelItemArr.push(_loc2_["p" + _loc3_]);
            }
            _loc3_++;
         }
      ...
      {
         URLLoader(param1.currentTarget).removeEventListener("complete",this.onUrlLoaderComplete);
         URLLoader(param1.currentTarget).removeEventListener("ioError",this.onIoError);
      }
      
      private function onHttpStatus(param1:HTTPStatusEvent) : void
      {
         URLLoader(param1.currentTarget).removeEventListener("httpStatus",this.onHttpStatus);
         if(param1.status == 404)
         {
            URLLoader(param1.currentTarget).removeEventListener("complete",this.onUrlLoaderComplete);
            URLLoader(param1.currentTarget).removeEventListener("ioError",this.onIoError);
            throw new Error("CGI应用程序 http://10.1.1.159/cgi-bin/HelloWorld.exeJumpTestNpcPanel.getConfigData()");
         }
      }
      
      public function getPanelData(param1:uint) : void
      {
         var _loc2_:URLRequest = new URLRequest("http://10.1.1.159/cgi-bin/SavePanelAndMap/DebugToolSavePanelAndMap.exe");
         var _loc3_:URLVariables = new URLVariables();
         this._userId = param1.toString();
         _loc3_.p0 = 2;
         _loc3_.p1 = this._userId;
         ...
         this._panelDataLoader.dataFormat = "variables";
         this._panelDataLoader.addEventListener("complete",this.onPanelDataComplete);
         this._panelDataLoader.addEventListener("ioError",this.onIoError);
         this._panelDataLoader.addEventListener("httpStatus",this.onHttpStatus);
      ...
      private function onPanelJumpBtn(param1:MouseEvent) : void
      {
         ...
         if(this._panelCombobox.editorTxt == "DEBUG_OFF")
         {
      ...
      {
         var _loc2_:int = int(this._tabBtnList.indexOf(param1.currentTarget as SimpleButton));
      ...
      {
         var _loc2_:int = int(this._panelItemMcList.indexOf(param1.currentTarget as MovieClip));
      ...
      private function addPanelItem(param1:String) : void
      {
         var _loc2_:String = "";
         var _loc3_:int = 0;
         _loc3_ = int(this._panelItemArr.indexOf(param1));
         if(_loc3_ == -1)
         {
