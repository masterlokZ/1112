package com.taomee.seer2.app.actives.fireClaw
{
   ...
   public class FireClawPayMode
   {
      ...
      private const DATA_LIST:Array = [203072,203073,203075,203076,203074,203077,203078];
      ...
      public function dispose() : void
      {
         ...
         while(_loc1_ < 8)
         {
            this.fireList[_loc1_].removeEventListener("click",this.toFightTrue);
            if(_loc1_ < 2)
            {
               this.cloneFireList[_loc1_].removeEventListener("click",this.toFightClone);
            }
            _loc1_++;
         }
      }
      ...
      public function setup() : void
      {
         ...
         var _loc1_:Boolean = SceneManager.prevSceneType == 2 && !FightManager.isWinWar() && FightManager.getPositionIndex() == 563;
         if(_loc1_)
         {
            this.exitMap();
            return;
      ...
      private function getData(param1:Parser_1142) : void
      {
         ...
         var _loc3_:uint = VipManager.vipInfo.isVip() ? 5 : 3;
         var _loc5_:uint = param1.infoVec[0];
         _loc5_ = _loc5_ >= _loc3_ ? _loc3_ : _loc5_;
         this.leftCount = _loc3_ - _loc5_ + param1.infoVec[1];
         ...
         _loc2_ = this.cloneNum == 0 && SceneManager.prevSceneType == 2 && (FightManager.getPositionIndex() == 564 || FightManager.getPositionIndex() == 564);
         if(_loc2_)
         {
            this.exitMap();
            return;
         }
         this.fightSeat = param1.infoVec[3] >= 1 ? param1.infoVec[3] - 1 : 0;
         this.needClone = Boolean(param1.infoVec[4]);
         if(SceneManager.prevSceneType == 2 && FightManager.isWinWar() && FightManager.getPositionIndex() == 563 && !this.needClone)
         {
            this.exitMap();
            return;
         }
      ...
      private function initEvent() : void
      {
         ...
         while(_loc1_ < 8)
         {
            this.fireList[_loc1_].addEventListener("click",this.toFightTrue);
            if(_loc1_ < 2)
            {
               this.cloneFireList[_loc1_].addEventListener("click",this.toFightClone);
            }
            _loc1_++;
         }
         this.waterGunBtn.addEventListener("click",this.toBuy);
         this.actItemBtn.addEventListener("click",this.toBuy);
         this.fightItemBtn.addEventListener("click",this.toBuy);
      }
      ...
      private function fightPet(param1:Parser_1224 = null) : void
      {
         ...
         {
            FightManager.startFightWithBoss(564);
         }
         else if(this.clickFire == this.cloneFireList[1])
         {
            FightManager.startFightWithBoss(565);
         }
         else
         {
            _loc2_ = uint(this.fireList.indexOf(this.clickFire));
      ...
      private function toFightTrue(param1:MouseEvent) : void
      {
         ...
         var _loc2_:uint = uint(this.fireList.indexOf(this.clickFire));
         ModuleManager.toggleModule(URLUtil.getAppModule("FireClawRulePanel"),"",_loc2_);
      }
      ...
      {
         var _loc2_:Boolean = SceneManager.prevSceneType == 2 && FightManager.getPositionIndex() == 563;
         this.playNum = 0;
         DisplayObjectUtil.removeFromParent(this.fireList[this.fightSeat]);
         var _loc1_:int = 0;
         while(_loc1_ < this.cloneNum)
         {
            if(BitUtil.getBit(this.cloneValue,_loc1_))
            {
               this.cloneFireList[_loc1_].x = this.CLONEFIRE_SEAT[this.fightSeat][_loc1_ * 2];
               this.cloneFireList[_loc1_].y = this.CLONEFIRE_SEAT[this.fightSeat][_loc1_ * 2 + 1];
               if(_loc2_)
               {
                  this.eftList[_loc1_].x = this.cloneFireList[_loc1_].x;
                  this.eftList[_loc1_].y = this.cloneFireList[_loc1_].y;
                  this.content.addChild(this.eftList[_loc1_]);
                  MovieClipUtil.playMc(this.eftList[_loc1_],1,this.eftList[_loc1_].totalFrames,this.clearEft);
               }
               else
               {
                  this.cloneFireList[_loc1_].play();
                  this.content.addChild(this.cloneFireList[_loc1_]);
               }
            }
            _loc1_++;
         }
      ...
      private function reqClone() : void
      {
         ...
         Connection.send(CommandSet.RANDOM_EVENT_1140,55,0);
      }
      ...
      private function onRandomStatus(param1:MessageEvent) : void
      {
         ...
         ActiveCountManager.requestActiveCount(203075,this.getState);
      }
