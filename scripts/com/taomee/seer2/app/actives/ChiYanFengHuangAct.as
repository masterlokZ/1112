package com.taomee.seer2.app.actives
{
   ...
   public class ChiYanFengHuangAct
   {
      ...
      {
         ActiveCountManager.requestActiveCountList([204000,204001,204003],this.getForlimitData);
      }
      ...
      private function calcuHowManyGet() : void
      {
         var i:int = 0;
         var canGet:uint = 0;
         if(SceneManager.prevSceneType == 2 && FightManager.currentFightRecord.initData.positionIndex == 907)
         {
            if(FightManager.fightWinnerSide == 1)
            {
               ...
               {
                  SwapManager.swapItem(2678,1,function(param1:IDataInput):void
                  {
                     new SwapInfo(param1);
      ...
      private function onCLick(param1:MouseEvent) : void
      {
         ...
         if(this.canFight > 0)
         {
            FightManager.startFightWithBoss(907);
         }
         else
         {
            _loc2_ = new BuyPropInfo();
            _loc2_.itemId = 603711;
      ...
      {
         this.wuzixiangBtn.addEventListener("click",this.onwuzixiangBtn);
         this.zhufuBtn.addEventListener("click",this.onzhufuBtn);
         this.beVip.addEventListener("click",this.onbeVip);
         this.exitbtn.addEventListener("click",this.onexitbtn);
         this.petBagBtn.addEventListener("click",this.onpetBagBtn);
      }
      ...
      {
         ItemManager.addEventListener1("requestSpecialItemSuccess",this.onSucc);
      ...
      private function onSucc(param1:ItemEvent) : void
      {
         ...
         ItemManager.removeEventListener1("requestSpecialItemSuccess",this.onSucc);
         item = ItemManager.getSpecialItem(500604);
         count = int(item != null ? int(item.quantity) : 0);
         if(count > 0)
         {
      ...
      {
         this.wuzixiangBtn.removeEventListener("click",this.onwuzixiangBtn);
         this.zhufuBtn.removeEventListener("click",this.onzhufuBtn);
         this.beVip.removeEventListener("click",this.onbeVip);
         this.exitbtn.removeEventListener("click",this.onexitbtn);
         this.petBagBtn.removeEventListener("click",this.onpetBagBtn);
      }
