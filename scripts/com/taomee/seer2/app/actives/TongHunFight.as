   ...
   public class TongHunFight
   {
      ...
      public function setup() : void
      {
         ...
         while(_loc1_ < 4)
         {
            ...
            this.monsterList[_loc1_].addEventListener("click",this.toFight);
            _loc1_++;
         }
         this.mapContent["hunMc"].stop();
         this.mapContent["hunMc"].buttonMode = true;
         this.mapContent["hunMc"].addEventListener("click",this.toFightHun);
         ActiveCountManager.requestActiveCountList([204316,204315,204314],this.getState);
      }
      ...
      public function dispose() : void
      {
         ...
         while(_loc1_ < 4)
         {
            this.monsterList[_loc1_].removeEventListener("click",this.toFight);
            SeatTipsManager.removeSeat(this.SEAT_TIPS[_loc1_],951);
            _loc1_++;
         }
         this.mapContent["hunMc"].removeEventListener("click",this.toFightHun);
      }
      ...
      protected function toFightHun(param1:MouseEvent) : void
      {
         ...
         if(this.leftCount != 0)
         {
            ...
            {
               FightManager.startFightWithBoss(1033);
            }]);
         }
         else
         {
            ...
            {
               ShopManager.buyItemForId(603942,buComplete);
            }]);
         }
      }
      ...
      {
         var _loc2_:int = int(this.monsterList.indexOf(param1.target as MovieClip));
      ...
      private function getState(param1:Parser_1142) : void
      {
         ...
         DayLimitManager.getDoCount(1190,this.getFightCount);
         this.figheBoss = BitUtil.getBit(param1.infoVec[2],1);
      }
      
      private function getFightCount(param1:uint) : void
      {
         this.leftCount = this.getCanNum(param1,this.buyCount,3);
         this.updateDis();
      }
