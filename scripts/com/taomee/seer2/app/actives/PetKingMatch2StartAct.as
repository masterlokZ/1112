package com.taomee.seer2.app.actives
{
   ...
   public class PetKingMatch2StartAct
   {
      ...
      private var stoneSeat:Array = [new Point(440,280),new Point(500,300),new Point(460,415),new Point(500,330),new Point(300,400),new Point(210,370),new Point(675,340),new Point(510,430)];
      ...
      public function setup() : void
      {
         ...
         if(this.time.fullYear == 2013 && this.time.month == 6 && this.time.date == 27)
         {
            ...
            SceneManager.addEventListener("switchStart",this.switchStart);
         }
      }
      ...
      private function switchStart(param1:SceneEvent) : void
      {
         ...
         var _loc2_:int = int(this.MAP_LIST.indexOf(this.mapid));
         if(_loc2_ == -1)
         {
            return;
         }
      ...
      private function checkIndex(param1:Array) : void
      {
         ...
         var _loc4_:String = this.time.hours.toString();
         var _loc6_:String = this.time.minutes < 10 ? "0" + this.time.minutes.toString() : this.time.minutes.toString();
         var _loc5_:uint = uint(_loc4_ + _loc6_);
         this.mapIndex = -1;
         var _loc3_:int = 0;
         while(_loc3_ < 8)
         {
         ...
         if(this.mapid == this.MAP_LIST[this.mapIndex])
         {
            ...
            _loc2_.writeByte(1);
            Connection.send(CommandSet.CLI_GET_WORLD_BOSS_BLOOD_DIMUSEN_1538,_loc2_);
         }
      }
      ...
      private function onShootEnd(param1:ShootEvent) : void
      {
         ...
         if(info.userID == ActorManager.actorInfo.id && this.stone_mc.hitTestPoint(info.endPos.x,info.endPos.y))
         {
            ...
            if(this.isTime1 || this.isTime2)
            {
               ...
               SwapManager.swapItem(1688,1,null,null,spe);
               --this.currentScore;
      ...
      {
         var _loc2_:int = 0;
         var _loc1_:int = 2000 - this.currentScore;
         if(_loc1_ <= 500)
         {
            _loc2_ = 1;
         }
         if(_loc1_ > 500 && this.currentScore <= 1000)
         {
            _loc2_ = 2;
         }
         if(_loc1_ > 1000 && this.currentScore <= 1500)
         {
            _loc2_ = 3;
         }
         if(_loc1_ > 1500 && this.currentScore < 2000)
         {
            _loc2_ = 4;
         }
         if(_loc1_ == 2000)
         ...
         this.stone_mc.gotoAndStop(_loc2_);
      }
      ...
      {
         QueueLoader.load(URLUtil.getActivityAnimation("StoneCup"),"swf",this.loadComplete);
      }
      ...
      private function loadComplete(param1:ContentInfo) : void
      {
         ...
         this.stone_mc.addEventListener("click",this.openPanel);
