package com.taomee.seer2.app.actives
{
   ...
   public class QingFengBaiDiAct
   {
      
      private static var _instance:QingFengBaiDiAct;
      ...
      private const FIGHT_BOSS_ID:int = 1759;
      
      private const damage_get_count:Array = [10,5,3,2,1];
      
      private const damage_get_value:Array = [30000,15000,10000,5000,2000];
      
      private const FOR_LIST:Array = [206280];
      
      private const DAY_LIST:Array = [1879];
      
      public function QingFengBaiDiAct()
      {
         super();
      }
      
      public static function getInstance() : QingFengBaiDiAct
      {
         if(_instance == null)
         {
            _instance = new QingFengBaiDiAct();
         }
         return _instance;
      }
         ...
         ActiveCountManager.requestActiveCountList(this.FOR_LIST,function(param1:Parser_1142):void
         {
            ...
            DayLimitListManager.getDaylimitList(DAY_LIST,(function():*
            {
               ...
               return callBack = function(param1:DayLimitListInfo):void
               {
                  ...
                  var canGet:uint = 0;
                  if(SceneManager.prevSceneType == 2 && FightManager.currentFightRecord.initData.positionIndex == 1759)
                  {
                     if(FightManager.fightWinnerSide == 1)
                     {
                        ...
                        AlertManager.showAlert("由于你赢得了胜利,你将获得【清风之羽】" + canGet + "个",function():void
                        {
                           ServerBufferManager.updateServerBuffer(449,1,0);
                           SceneManager.changeScene(1,70);
                           TweenNano.delayedCall(1,function():void
                           {
                              ModuleManager.showAppModule("QingFengBaiDiModeOnePanel");
                           });
                        });
                     }
                     else
      ...
      private function onCLick(param1:MouseEvent) : void
      {
         ...
         if(this.canFight > 0)
         {
            ServerBufferManager.getServerBuffer(449,function(param1:ServerBuffer):void
            {
               var _loc2_:Boolean = Boolean(param1.readDataAtPostion(1));
               if(!_loc2_)
               {
                  ServerBufferManager.updateServerBuffer(449,1,1);
                  FightManager.startFightWithBoss(1759);
               }
               else
               {
