package com.taomee.seer2.app.actives
{
   ...
   public class ChallengePhenixAct
   {
      ...
      private var limitIds:Array = [550,551,552,553,554,535];
      
      private var callFight:Array = [[216,217,218,219,230],[220,221,222,223,231],[224,225,226,227,232]];
      
      private var resArr:Array = [[529,524],[530,525],[521,526],["PhinexHalo",527]];
      ...
      private function getBuffer(param1:ServerBuffer) : void
      {
         ...
         var isPlay:int = buff.readDataAtPostion(4);
         if(isPlay == 0 && QuestManager.isComplete(35))
         {
            MovieClipUtil.playFullScreen(URLUtil.getActivityFullScreen("ChallengePhenixAct"),function():void
            {
               ...
               isPlay = 1;
               ServerBufferManager.updateServerBuffer(57,4,1);
               requestLimit();
            });
         }
         else
      ...
      private function initMobile() : void
      {
         ...
         if(this.fightPhinexNum >= this.maxNum && SceneManager.prevSceneType == 2)
         {
         ...
         if(SceneManager.prevSceneType == 2 && FightManager.currentFightRecord.initData.positionIndex == this.fightPhinexId && !FightManager.isWinWar())
         {
      ...
      private function checkPhinex() : void
      {
         ...
         if(this.datas[this.limitIds[0]] > 1 && this.datas[this.limitIds[1]] > 1 && this.datas[this.limitIds[2]] > 1)
         {
            ...
            _loc1_.addEventListener("click",this.fightPhinex);
         }
      }
      ...
      private function checkCall(param1:int) : void
      {
         ...
         {
            _loc2_ = int(this.mobileVec.indexOf(this.currentTarget));
            if(_loc2_ == param1 && SceneManager.prevSceneType == 2 && FightManager.currentFightRecord.initData.positionIndex == this.callFightId)
            {
               this.showMobile();
            }
            else
            {
               ...
               MobileManager.removeMobile(this.mobileVec[param1],"npc");
               this.mobileVec[param1].resourceUrl = URLUtil.getNpcSwf(this.resArr[param1][1]);
               MobileManager.addMobile(this.mobileVec[param1],"npc");
            }
            this.mobileVec[param1].removeEventListener("click",this.toCall);
         }
      }
      ...
      private function setDefalutRes(param1:int, param2:String, param3:String) : void
      {
         ...
         else
         {
            ...
            this.mobileVec[param1].addEventListener("click",this.toCall);
         }
      ...
      public function dispose() : void
      {
         ...
         while(_loc1_ < 4)
         {
            ...
            {
               this.mobileVec[_loc1_].removeEventListener("click",this.toCall);
         ...
         if(this.mobileVec[3])
         {
            this.mobileVec[3].removeEventListener("click",this.fightPhinex);
         }
         Connection.removeCommandListener(CommandSet.GET_RANGE_DAY_LIMIT_1209,this.onGetRange);
      }
      ...
      private function toCall(param1:MouseEvent) : void
      {
         ...
         var _loc2_:int = int(this.mobileVec.indexOf(this.currentTarget));
         if(_loc2_ > 0)
         {
      ...
      {
         var _loc1_:int = int(this.mobileVec.indexOf(this.currentTarget));
         TooltipManager.remove(this.mobileVec[_loc1_]);
         MobileManager.removeMobile(this.mobileVec[_loc1_],"npc");
         ...
         {
            QueueLoader.load(URLUtil.getActivityAnimation("CallAnimation"),"swf",this.onLoadComplete);
         }
         else
         {
            this.toPlay();
      ...
      private function toPlay() : void
      {
         ...
         MovieClipUtil.playMc(this.callAnimation,1,this.callAnimation.totalFrames,function():void
         {
            ...
            MobileManager.addMobile(currentTarget,"npc");
         });
      }
   }
}
