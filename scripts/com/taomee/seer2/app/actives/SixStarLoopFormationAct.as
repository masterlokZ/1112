package com.taomee.seer2.app.actives
{
   ...
   public class SixStarLoopFormationAct
   {
      ...
      private static var _instance:SixStarLoopFormationAct;
      ...
      public static function getInstance() : SixStarLoopFormationAct
      {
         if(!_instance)
         {
            _instance = new SixStarLoopFormationAct();
         }
         return _instance;
      }
      ...
      public function dispose() : void
      {
         ...
         if(this.centerNpc)
         {
            this.centerNpc.removeEventListener("click",this.toFight);
            this.centerNpc = null;
         }
         if(this.loopNpc)
         {
            ...
            while(_loc1_ < 6)
            {
               this.loopNpc[_loc1_].removeEventListener("click",this.toLoopFight);
               _loc1_++;
            }
         }
         this.resetBtn.removeEventListener("click",this.btnClickHandler);
         this.ruleBtn.removeEventListener("click",this.btnClickHandler);
         this.buyPassBtn.removeEventListener("click",this.btnClickHandler);
         this.buyCountBtn.removeEventListener("click",this.btnClickHandler);
         this.buyItemBtn.removeEventListener("click",this.btnClickHandler);
         this.vipBtn.removeEventListener("click",this.btnClickHandler);
      ...
      public function setup() : void
      {
         ...
         this.initLine();
         this.getData();
         ServerBufferManager.getServerBuffer(57,this.getBuffer);
      }
      ...
      private function getBuffer(param1:ServerBuffer) : void
      {
         ...
         if(!_loc2_)
         {
            ...
            ServerBufferManager.updateServerBuffer(57,24,1);
         }
      }
      ...
      {
         this.resetBtn.addEventListener("click",this.btnClickHandler);
         this.ruleBtn.addEventListener("click",this.btnClickHandler);
         this.buyPassBtn.addEventListener("click",this.btnClickHandler);
         this.buyCountBtn.addEventListener("click",this.btnClickHandler);
         this.buyItemBtn.addEventListener("click",this.btnClickHandler);
         this.vipBtn.addEventListener("click",this.btnClickHandler);
      ...
      private function btnClickHandler(param1:MouseEvent) : void
      {
         ...
         switch(param1.target)
         {
            ...
            case this.buyPassBtn:
               ...
               _loc2_.itemId = 603194;
               _loc2_.buyComplete = this.buyComplete;
               ShopManager.buyVirtualItem(_loc2_);
               _loc4_ = "0x1003400d";
               break;
            case this.buyCountBtn:
               _loc3_ = VipManager.vipInfo.isVip() ? 2 : 1;
               if(this.fightboss_state < _loc3_)
               {
                  AlertManager.showAlert("你还有" + (_loc3_ - this.fightboss_state) + "次挑战机会，不用购买挑战次数！");
                  return;
               }
               _loc2_ = new BuyPropInfo();
               _loc2_.itemId = 603193;
               _loc2_.buyComplete = this.buyComplete;
               ShopManager.buyVirtualItem(_loc2_);
               _loc4_ = "0x1003400e";
               break;
         ...
         StatisticsManager.sendNovice(_loc4_);
      }
      
      private function buyComplete(param1:Parser_1224) : void
      {
         this.hideLine();
         if(param1.itemId == MatchingConfig.getNewId(603194))
         {
            this.isBuyPass = true;
         }
         this.getData();
      ...
      private function getData() : void
      {
         ...
         _loc1_.writeUnsignedInt(771);
         _loc1_.writeUnsignedInt(772);
         _loc1_.writeUnsignedInt(767);
         _loc1_.writeUnsignedInt(768);
      ...
      private function getDayLimitList(param1:DayLimitListInfo) : void
      {
         ...
         this.fightloop_state = param1.getCount(771).toString().split("");
         this.fightloop_state.reverse();
         if(this.fightloop_state.length >= 7 && param1.getCount(772) != 0)
         {
            _loc2_ = param1.getCount(772).toString().split("");
            _loc2_.reverse();
            this.fightloop_state = this.fightloop_state.concat(_loc2_);
         }
         this.fight6v6_state = param1.getCount(767);
         this.fightboss_state = param1.getCount(768);
      ...
      private function checkLineComplete() : void
      {
         var _loc4_:String = null;
         var _loc1_:int = 0;
         var _loc3_:uint = this.fightloop_state.length;
         var _loc2_:Boolean = false;
         var _loc5_:uint = 0;
         while(_loc5_ < _loc3_ - 1)
         {
            _loc4_ = String(this.fightloop_state[_loc5_].toString() + this.fightloop_state[_loc5_ + 1].toString());
            _loc1_ = int(this.LINE_STATE1.indexOf(_loc4_));
            if(_loc1_ == -1)
            {
               _loc1_ = int(this.LINE_STATE2.indexOf(_loc4_));
            }
            if(_loc1_ > 11 || _loc1_ == -1)
            {
               _loc2_ = true;
            }
            if(_loc1_ != -1)
            {
               if(!this.line_list[_loc1_].visible)
               {
                  this.line_list[_loc1_].visible = true;
               }
               else
               {
                  _loc2_ = true;
         ...
         if(!_loc2_ && _loc3_ == 13)
         {
            ...
            {
               this.centerNpcId = 615;
            }
            else
            {
               this.centerNpcId = 616;
            }
      ...
      private function createLoopNpc() : void
      {
         ...
         while(_loc2_ < 6)
         {
            ...
            _loc1_.resourceUrl = URLUtil.getNpcSwf(this.LOOPNPC_ID[_loc2_]);
            _loc1_.addEventListener("click",this.toLoopFight);
            MobileManager.addMobile(_loc1_,"npc");
            this.loopNpc.push(_loc1_);
            this.loopNpc[_loc2_].x = this.LOOPNPC_SEAT[_loc2_].x;
            this.loopNpc[_loc2_].y = this.LOOPNPC_SEAT[_loc2_].y;
            _loc2_++;
         }
      }
      ...
      private function toLoopFight(param1:MouseEvent) : void
      {
         ...
         var _loc2_:int = int(this.loopNpc.indexOf(param1.target as Mobile));
         FightManager.startFightWithWild(this.FIGHTLOOP_ID[_loc2_]);
      }
      
      private function toReset() : void
      {
         SwapManager.swapItem(1710,1,this.swapComplete);
      }
      ...
      private function createCenterNpc() : void
      {
         ...
         MobileManager.addMobile(this.centerNpc,"npc");
         this.centerNpc.addEventListener("click",this.toFight);
         ...
         if(FightManager.getPositionIndex() == 549 && FightManager.isJustWinFight())
         {
            this.centerNpc.visible = false;
      ...
      private function checkFightTarget() : void
      {
         ...
         {
            FightManager.startFightWithBoss(550);
         }
      }
      ...
      private function toFight(param1:MouseEvent) : void
      {
         ...
         {
            FightManager.startFightWithBoss(549);
         }
         else
         {
