package com.taomee.seer2.app.actives
{
   ...
   public class HunYuanHeiDiAct
   {
      
      private static var _instance:HunYuanHeiDiAct;
      ...
      private const FIGHT_BOSS_ID:int = 1763;
      
      private const damage_get_count:Array = [10,5,3,2,1];
      
      private const damage_get_value:Array = [30000,15000,10000,5000,2000];
      
      private const FOR_LIST:Array = [206288];
      
      private const DAY_LIST:Array = [1880];
      
      public function HunYuanHeiDiAct()
      {
         super();
      }
      
      public static function getInstance() : HunYuanHeiDiAct
      {
         if(_instance == null)
         {
            _instance = new HunYuanHeiDiAct();
         }
         return _instance;
      }
         ...
         ActiveCountManager.requestActiveCountList(this.FOR_LIST,function(param1:Parser_1142):void
         {
            ...
            DayLimitListManager.getDaylimitList(DAY_LIST,(function():*
            {
               ...
               return callBack = function(param1:DayLimitListInfo):void
               {
                  ...
                  var canGet:uint = 0;
                  if(SceneManager.prevSceneType == 2 && FightManager.currentFightRecord.initData.positionIndex == 1763)
                  {
                     if(FightManager.fightWinnerSide == 1)
                     {
                        ...
                        AlertManager.showAlert("由于你赢得了胜利,你将获得【混元结晶】" + canGet + "个",function():void
                        {
                           ServerBufferManager.updateServerBuffer(451,1,0);
                           SceneManager.changeScene(1,70);
                           TweenNano.delayedCall(1,function():void
                           {
                              ModuleManager.showAppModule("HunYuanHeiDiModeOnePanel");
                           });
                        });
                     }
                     else
      ...
      private function onCLick(param1:MouseEvent) : void
      {
         ...
         if(this.canFight > 0)
         {
            ServerBufferManager.getServerBuffer(451,function(param1:ServerBuffer):void
            {
               var _loc2_:Boolean = Boolean(param1.readDataAtPostion(1));
               if(!_loc2_)
               {
                  ServerBufferManager.updateServerBuffer(451,1,1);
                  FightManager.startFightWithBoss(1763);
               }
               else
               {
