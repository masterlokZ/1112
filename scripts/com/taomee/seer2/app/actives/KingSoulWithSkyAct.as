package com.taomee.seer2.app.actives
{
   ...
   public class KingSoulWithSkyAct implements IActInstance
   {
      ...
      private var timeDate:Date = new Date();
      ...
      {
         SceneManager.addEventListener("switchComplete",this.onSwithComplete);
         SceneManager.addEventListener("switchStart",this.onStart);
      }
      ...
      private function onSwithComplete(param1:SceneEvent) : void
      {
         ...
         this.mapIndex = this.isWeekEnd ? this.WEEKEND_MAPS.indexOf(_loc2_) : int(this.GENERAL_MAPS.indexOf(_loc2_));
         if(this.mapIndex < 0)
         {
            return;
         }
         ...
         if(!this.isWeekEnd)
         {
            if(this.timeDate.hours >= 9 && this.timeDate.hours < 23)
            {
               ...
               Connection.send(CommandSet.RANDOM_EVENT_1140,60,0);
            }
         }
      ...
      private function createYueSe() : void
      {
         ...
         if(!this.yueSeMobile.hasEventListener("click"))
         {
            this.yueSeMobile.addEventListener("click",this.showDia);
         }
         this.yueSeMobile.resourceUrl = URLUtil.getNpcSwf(624);
      ...
      private function showDia(param1:MouseEvent) : void
      {
         ...
         var npcNode:XML = <npc id="591" resId="591" name="约瑟" dir="0" width="25" height="90" pos="841,288"
         ...
         else if(this.isWeekEnd)
         {
            ...
            {
               MobileManager.removeMobile(yueSeMobile,"npc");
            }]);
         }
         else
         {
      ...
      private function toDia(param1:DialogPanelEvent) : void
      {
         ...
         if(params == "fight_yuese")
         {
            ...
            {
               FightManager.startFightWithBoss(586);
            }]);
         }
         else
         {
            ModuleManager.toggleModule(URLUtil.getAppModule("KingSoulWithSkyRulePanel"));
         }
      }
