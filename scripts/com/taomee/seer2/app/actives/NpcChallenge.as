package com.taomee.seer2.app.actives
{
   ...
   public class NpcChallenge
   {
      ...
      public function init(param1:int, param2:Array) : void
      {
         ...
         this._npcid = npcId;
         DialogPanel.addEventListener("customUnitClick",this.showNpcChallengePanel);
         if(SceneManager.prevSceneType == 2)
         {
            index = int(positionIndex.indexOf(FightManager.currentFightRecord.initData["positionIndex"]));
            if(index != -1 && !FightManager.isJustWinFight())
            {
               NpcDialog.show(npcId,MobileManager.getMobile(npcId,"npc").label,[[0,"我可不是一般的精灵，要战胜我可不是这么简单的事哦。"]],["还是放弃吧"," 再来一次！ "],[null,function():void
               {
      ...
      {
         DialogPanel.removeEventListener("customReplyClick",this.showNpcChallengePanel);
      }
   }
}
