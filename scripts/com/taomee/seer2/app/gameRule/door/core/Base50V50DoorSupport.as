package com.taomee.seer2.app.gameRule.door.core
{
   ...
   public class Base50V50DoorSupport extends Base21LevelDoorSupport implements IDoorServerEquipable
   {
      ...
      protected var _rewardIDList:Vector.<ServerReward>;
      ...
      {
         _guard_mc.removeEventListener("escape_end",this.onUpdateNextTrailHandler);
         canLevelNow = true;
      ...
      private function getLastEquipId() : int
      {
         var _loc2_:int = -1;
         var _loc1_:int = 0;
         while(this.hasEquipId(this._ITEMS[_loc1_]))
         {
            _loc2_ = int(this._ITEMS[_loc1_]);
            if(++_loc1_ > 4)
            {
               break;
            }
      ...
      {
         var _loc3_:Vector.<EquipItem> = ItemManager.getEquipVec();
         var _loc5_:uint = _loc3_.length;
         var _loc4_:Boolean = false;
         var _loc2_:uint = 0;
         while(_loc2_ < _loc5_)
         {
            if(_loc3_[_loc2_].referenceId == param1)
            {
               _loc4_ = true;
               break;
            }
