package com.taomee.seer2.app.gameRule.nono.time
{
   ...
   public class NonoButlerController
   {
      ...
      private var _activityCount:int = this._activitySpeed;
      ...
      {
         var _loc4_:NonoTimeInfo = null;
         var _loc2_:uint = uint(param1.readUnsignedInt());
         this.createNonoTimeInfos();
         var _loc3_:uint = 0;
         while(_loc3_ < _loc2_)
         {
            _loc4_ = new NonoTimeInfo();
            _loc4_.type = param1.readUnsignedByte();
            _loc4_.catch_time = param1.readUnsignedInt();
            _loc4_.end_time = param1.readUnsignedInt();
            this.add(_loc4_);
            _loc3_++;
         }
         this.run();
      ...
      {
         Tick.instance.addRender(this.onTimer,30 * 1000);
      }
      ...
      private function checkTimeVec() : void
      {
         ...
         while(_loc1_ < this._timeVec.length)
         {
            ...
            {
               this._timeVec[_loc1_].end_time -= 30;
            }
            _loc1_++;
         }
      }
