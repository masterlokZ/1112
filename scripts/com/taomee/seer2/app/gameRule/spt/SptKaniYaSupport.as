package com.taomee.seer2.app.gameRule.spt
{
   ...
   public class SptKaniYaSupport
   {
      ...
      {
         this._map = null;
         ShootController.removeEventListener("playEnd",this.onShoot);
      ...
      private function initUI() : void
      {
         ...
         this._pillarInfos.push(new PillarInfo(0,_loc1_["pillar_mc1"],_loc1_["pillar_eff_mc1"]));
         this._pillarInfos.push(new PillarInfo(1,_loc1_["pillar_mc2"],_loc1_["pillar_eff_mc2"]));
         this._pillarInfos.push(new PillarInfo(2,_loc1_["pillar_mc3"],_loc1_["pillar_eff_mc3"]));
         this._pillarInfos.push(new PillarInfo(3,_loc1_["pillar_mc4"],_loc1_["pillar_eff_mc4"]));
         this._pillarInfos.push(new PillarInfo(4,_loc1_["pillar_mc5"],_loc1_["pillar_eff_mc5"]));
         ShootController.addEventListener("playEnd",this.onShoot);
      ...
      private function playFullSceneAnimiation() : void
      {
         ...
         this._animationPlayer.addEventListener("end",this.onAnimationEnd);
      ...
      private function onAnimationEnd(param1:Event) : void
      {
         ...
         this._animationPlayer.removeEventListener("end",this.onAnimationEnd);
      ...
      {
         this._kaniya_MC.addEventListener("click",this.onSPTClick);
      }
      
      private function removeSptClickListener() : void
      {
         if(Boolean(this._kaniya_MC) && this._kaniya_MC.hasEventListener("click"))
         {
            this._kaniya_MC.removeEventListener("click",this.onSPTClick);
         }
      }
      ...
      private function onServerBufferDataHandler(param1:ServerBuffer) : void
      {
         ...
         this._winHistory = param1.readDataAtPostion(this._sptBufferPosition) == 1;
      ...
      private function fightFailure(param1:LittleEndianByteArray) : void
      {
         ...
         if(_loc2_ < 2)
         {
            _loc4_ = new XML(SptDialogConfig.getSptDialog(this._sptBossId,4,[SptDialogConfig.getTalkContent(this._sptBossId)]));
      ...
      private function onSPTClick(param1:MouseEvent) : void
      {
         ...
         {
            var _loc2_:* = param1.readDataAtPostion(_sptBufferPosition - 1) == 1;
            if(_loc2_)
            {
               if(_winHistory)
               {
         ...
         var winNeverTalk:Function = function():void
         {
            ...
            var _loc1_:DialogDefinition = new DialogDefinition(_loc2_);
            DialogPanel.showForCommon(_loc1_,onClose1);
         };
         ...
         {
            var _loc2_:XML = new XML(SptDialogConfig.getSptDialog(_sptBossId,10));
            var _loc1_:DialogDefinition = new DialogDefinition(_loc2_);
            DialogPanel.showForCommon(_loc1_,onClose5);
         };
         ...
         {
            ServerBufferManager.getServerBuffer(1,checkServerBufferHandler);
         }
         else
         {
            unConditionTalk();
      ...
      private function startFight() : void
      {
         ...
         while(_loc1_ < 5)
         {
            if(this._properties.indexOf(_loc1_) == -1)
            {
               _loc2_.push(_loc1_);
            }
            _loc1_++;
         }
         _fighterID = FightManager.startFightWithSPTBoss(this._sptBossId,_loc2_);
      }
   }
}
