package com.taomee.seer2.app.gameRule.ring
{
   ...
   public class RingSupport implements ISupportable
   {
      ...
      public function init(param1:MapModel) : void
      {
         ...
         this._ringTable.addEventListener("click",this.onClickRingTable);
         TooltipManager.addCommonTip(this._ringTable,"点击进入无禽战");
         this._map.ground.addEventListener("click",this.onGroundClick);
         ...
         EventManager.addEventListener("message",this.onMessage);
      }
      ...
      {
         EventManager.removeEventListener("message",this.onMessage);
         clearTimeout(this._talkTimeOutID);
         this.countdownOver();
         this._map.ground.removeEventListener("click",this.onGroundClick);
         this._ringTable.removeEventListener("click",this.onClickRingTable);
      ...
      private function requestRinger(param1:uint, param2:Boolean = false) : void
      {
         ...
         if(this.checkTime() && this.checkBattery())
         {
            ...
            _loc3_.position = 0;
            Connection.send(CommandSet.FIGHE_PLAYER_INVITE_1503,_loc3_);
         }
      }
      ...
      private function onGetUserSimpleInfo(param1:MessageEvent) : void
      {
         ...
         if(_loc4_ == this._ringerId)
         {
            Connection.removeCommandListener(CommandSet.USER_GET_SIMPLE_INFO_1028,this.onGetUserSimpleInfo);
            _loc3_ = new UserInfo();
            _loc3_.id = _loc4_;
            UserInfoParseHelper.readSimpleInfo(_loc3_,_loc2_);
            this.removeOldRingerActor();
            this._ringActor = ActorManager.addOtherActor(_loc3_);
            this._ringActor.x = RING_LOCATION.x;
            this._ringActor.y = RING_LOCATION.y;
            TooltipManager.addCommonTip(this._ringActor,"擂主:" + _loc3_.nick);
      ...
      private function onUpdateRingInfo(param1:MessageEvent) : void
      {
         ...
         if(_loc5_ != this._ringerId)
         {
            this._oldRingerId = this._ringerId;
            this._ringerId = _loc5_;
            if(this._ringerId == 0)
            {
               this._ringerReadyFight = true;
            }
         }
         this._ringAtkId = _loc3_.readUnsignedInt();
         this._win_counter = _loc3_.readUnsignedInt();
         var _loc4_:uint = uint(_loc3_.readUnsignedInt());
         if(this._ringerId != 0 && this._ringAtkId != 0)
         {
            this._ringerReadyFight = false;
         }
         _loc2_ = this._win_counter;
         _loc2_ = uint(_loc2_ < 0 ? 0 : _loc2_);
         this.ringerAutoTalk(_loc2_);
         if(this._ringerId == 0)
         {
         }
         this.updateRingStatus();
      ...
      private function checkTime() : Boolean
      {
         ...
         var _loc1_:Boolean = DateUtil.isInHourScope(6,23,0,59);
         if(!_loc1_)
         {
