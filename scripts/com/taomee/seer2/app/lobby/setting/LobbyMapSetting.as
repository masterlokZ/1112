package com.taomee.seer2.app.lobby.setting
{
   ...
   public class LobbyMapSetting
   {
      ...
      public static function parseSetting(param1:XML) : void
      {
         ...
         var _loc2_:* = false;
         _settingXML = param1;
         var _loc4_:XMLList = _settingXML.elements("map");
         for each(_loc6_ in _loc4_)
         {
            _loc5_ = uint(_loc6_.attribute("dayId").toString());
            _loc3_ = 0;
            if(_loc6_.hasOwnProperty("nightId") != null)
            {
               _loc3_ = uint(_loc6_.attribute("nightId").toString());
            }
            _loc2_ = false;
            if(_loc6_.hasOwnProperty("change") != null)
            {
               _loc2_ = uint(_loc6_.attribute("change").toString()) == 1;
            }
            _mapSettingVector.push(new MapSetting(_loc5_,_loc3_,_loc2_));
         }
      }
      ...
      private static function getMapSetting(param1:uint) : MapSetting
      {
         ...
         var _loc4_:uint = _mapSettingVector.length;
         var _loc3_:uint = 0;
         while(_loc3_ < _loc4_)
         {
            if(_mapSettingVector[_loc3_].dayId == param1 || _mapSettingVector[_loc3_].nightId == param1)
            {
               _loc2_ = _mapSettingVector[_loc3_];
               break;
            }
            _loc3_++;
         }
