package com.taomee.seer2.app.arena.data
{
   ...
   public class FighterTeam
   {
      ...
      public function FighterTeam(param1:TeamInfo)
      {
         ...
         {
            for each(_loc4_ in _loc2_.fighterInfoVec)
            {
               this._fighterInfoVec.push(_loc4_);
            }
            for each(_loc3_ in _loc2_.changeFighterInfoVec)
            {
               this._changeFighterInfoVec.push(_loc3_);
            }
         }
      }
      ...
      public function changeMainSubPosition(param1:Function) : void
      {
         ...
         this.replaceFighterPosition(1,oldSubFighter);
         this.replaceFighterPosition(2,oldMainFighter);
         counter = 0;
         if(oldSubFighter)
         {
      ...
      public function createFighter(param1:ResourceQueue, param2:ArenaDataInfo) : void
      {
         ...
         {
            _loc5_ = new Fighter(_loc4_,this._teamInfo.clientSide,param2,this.isFitInfo(_loc4_));
            param1.addCache("pet",_loc5_.resourceUrl,_loc5_.onResoureceLoaded);
            this._fighterVec.push(_loc5_);
            this._initFighterVec.push(_loc5_);
         }
         for each(_loc3_ in this._changeFighterInfoVec)
         {
            _loc5_ = new Fighter(_loc3_,this._teamInfo.clientSide,param2,this.isFitInfo(_loc3_));
            param1.addCache("pet",_loc5_.resourceUrl,_loc5_.onResoureceLoaded);
            this._changeFighterVec.push(_loc5_);
            this._initFighterVec.push(_loc5_);
         }
      }
      ...
      public function getFighter(param1:uint, param2:uint) : Fighter
      {
         ...
         {
            if(_loc3_.fighterInfo.userId == param1 && _loc3_.id == param2)
            {
               _loc4_ = _loc3_;
               break;
            }
         ...
         if(_loc4_ == null)
         {
            _loc4_ = this.getChangeFighter(param1,param2);
         }
         return _loc4_;
      }
      ...
      public function getInitFighter(param1:uint, param2:uint) : Fighter
      {
         var _loc4_:* = null;
         var _loc3_:Fighter = null;
         for each(_loc3_ in this._initFighterVec)
         {
            if(_loc3_.fighterInfo.userId == param1 && _loc3_.id == param2)
            {
               _loc4_ = _loc3_;
      ...
      public function getChangeFighter(param1:uint, param2:uint) : Fighter
      {
         var _loc4_:* = null;
         var _loc3_:Fighter = null;
         for each(_loc3_ in this._changeFighterVec)
         {
            if(_loc3_.fighterInfo.userId == param1 && _loc3_.id == param2)
            {
               _loc4_ = _loc3_;
      ...
      public function getRandomAliveFighterInfo() : FighterInfo
      {
         ...
         for each(_loc1_ in this._fighterInfoVec)
         {
            if(_loc1_.hp > 0 && this.isFitInfo(_loc1_) == false)
            {
               _loc2_ = _loc1_;
               break;
            }
      ...
      public function set mainFighter(param1:Fighter) : void
      {
         ...
         {
            if(this._fighterVec[_loc2_].fighterInfo.position == 1)
            {
