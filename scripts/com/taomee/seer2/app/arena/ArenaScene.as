package com.taomee.seer2.app.arena
{
   ...
   public class ArenaScene extends BaseScene
   {
      ...
      private var _arenaData:ArenaDataInfo = new ArenaDataInfo();
      ...
      {
         super.updateScene();
         var _loc1_:int = mapModel.ground.width;
      ...
      {
         ActorManager.addEventListener("actorSyncSvr",this.onActorSyncSvr);
      ...
      public function exitFight() : void
      {
         ...
         if(this.fightMode == 102 || this.fightMode == 103)
         {
      ...
      private function onActorSyncSvr(param1:ActorEvent) : void
      {
         ...
         onGetInfoComplete = function():void
         {
            ...
            FightManager.dispatchEvent(new FightStartEvent("FIGHT_LOADING_START"));
         ...
         onArenaLoadingBarClose = function():void
         {
            ...
            FightManager.dispatchEvent(new FightStartEvent("FIGHT_LOADING_END"));
         };
         ActorManager.removeEventListener("actorSyncSvr",this.onActorSyncSvr);
         completeMap();
         scene = this;
         interruptFlag = false;
         DialogPanel.hide();
      ...
      public function sortAllFighters() : void
      {
         ...
         sortByY = function(param1:Fighter, param2:Fighter):int
         {
            var _loc4_:uint = 0;
            var _loc3_:uint = 0;
            var _loc6_:uint = param1.fighterInfo.position;
            var _loc5_:uint = param2.fighterInfo.position;
            if(_loc6_ < _loc5_)
            {
               return 1;
            }
            if(_loc6_ == _loc5_)
            {
               _loc4_ = uint(param1.fighterSide);
               _loc3_ = uint(param2.fighterSide);
               if(_loc4_ < _loc3_)
               {
                  return 1;
               }
