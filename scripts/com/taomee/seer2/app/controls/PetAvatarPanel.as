package com.taomee.seer2.app.controls
{
   ...
   public class PetAvatarPanel extends Sprite
   {
      ...
      private static var _instance:PetAvatarPanel;
      ...
      private function initColoreeg() : void
      {
         ...
         this.colorEggBtn.addEventListener("click",this.showPanel);
      ...
      private function createChildren() : void
      {
         ...
         this._petListSp.addEventListener("rollOver",this.onPetListShow);
         this._petListSp.addEventListener("rollOut",this.onPetListHide);
      ...
      private function onPetListShow(param1:MouseEvent) : void
      {
         ...
         for each(_loc6_ in PetInfoManager.getAllBagPetInfo())
         {
            if(_loc6_.catchTime != this._startingPetInfo.catchTime)
            {
               _loc4_.push(_loc6_);
            }
         }
         ...
         {
            _loc5_ = new PetItem();
            this._petListSp.addChild(_loc5_);
            this._petList.push(_loc5_);
            _loc5_.x = -182;
            _loc5_.y = 2 + (_loc5_.height + 2) * _loc3_ + 37;
            _loc5_.info = _loc4_[_loc3_];
            _loc3_++;
         }
         if(_loc4_.length == 0)
         {
         ...
         {
            this._petListBg.height = 5 + 57 * _loc4_.length;
         }
      ...
      private function initEventListener() : void
      {
         ...
         {
            PetInfoManager.addEventListener("petAdd",this.onAddFirstPet);
         }
         this._mainPetItem.getPetIconMC().addEventListener("rollOver",this.onMainIconOver);
         this._mainPetItem.getPetIconMC().addEventListener("rollOut",this.onMainIconOut);
         this._mainPetItem.getPetIconMC().addEventListener("click",this.onShowPetBag);
         this._mainPetItem.lvLink.addEventListener("rollOver",this.onMainPetItemOver);
         this._mainPetItem.lvLink.addEventListener("rollOut",this.onMainPetItemOut);
      }
      ...
      {
         StatisticsManager.sendNovice("0x10033614");
      ...
      private function onAddAllPetBlood(param1:MessageEvent) : void
      {
         ...
         ActorManager.actorInfo.coins = uint(_loc3_);
         ...
         {
            ServerMessager.addMessage("背包中所有精灵已经恢复成功！花费" + (this.oldCoinNum - _loc3_) + "赛尔豆");
         }
      }
      ...
      {
         StatisticsManager.sendNovice("0x10033613");
