package com.taomee.seer2.app.controls.widget
{
   ...
   public class NonoInputFrameWidget extends Sprite implements IWidgetable
   {
      ...
      private function initEventListener() : void
      {
         ...
         this._nonoBtn.addEventListener("rollOver",this.onNono);
         this._nonoBtn2.buttonMode = true;
         this._nonoBtn2.addEventListener("click",this.onNono2);
         this._nonoBtn2.addEventListener("rollOver",this.onNono);
         for each(_loc1_ in this._btnList)
         {
            _loc1_.addEventListener("click",this.onBtn);
         }
         this.tempBtn.addEventListener("click",this.onTemp);
      ...
      private function updateGiftStatus(param1:int, param2:int) : void
      {
         ...
         var _loc4_:int = int(VipManager.vipInfo.level);
         if(VipManager.vipInfo.vipFlag == 0)
         {
            _loc4_ = 0;
         }
         this._giftInfo = VipConfig2.getGiftInfo(_loc4_);
         switch(param1)
         {
      ...
      {
         this.onHide(null);
         ToolbarEventDispatcher.dispatchEvent(new ToolbarEvent("pageChange",false,"NonoInput"));
      }
      ...
      private function onNono(param1:MouseEvent) : void
      {
         ...
         ToolbarEventDispatcher.dispatchEvent(new ToolbarEvent("pageChange",this._isShow,"NonoInput"));
         this.updateNonoPage();
         StatisticsManager.sendNovice("0x1003360F");
      }
      ...
      {
         var _loc2_:int = int(this._btnList.indexOf(param1.currentTarget as SimpleButton));
         var _loc3_:String = "";
         if(_loc2_ == 6)
         {
            VipManager.entryMi();
         ...
         {
            ToolbarEventDispatcher.dispatchEvent(new ToolbarEvent("pageChange",this._isShow,"NewVipPanel"));
         }
