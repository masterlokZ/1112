package com.taomee.seer2.app.questTiny.items
{
   ...
   public class QuestTinyItem extends TreeItem
   {
      ...
      private var _mainUI:MovieClip = UIManager.getMovieClip("QuestTinyItemUI");
      
      private var _nameTxt:TextField = this._mainUI["txt"];
      
      private var _flagMc:MovieClip = this._mainUI["flagMc"];
      ...
      public function QuestTinyItem()
      {
         ...
         this._mainUI.addEventListener("click",this.onClick);
      }
      ...
      {
         this._mainUI.removeEventListener("click",this.onClick);
         super.dispose();
      }
      ...
      private function updateName() : void
      {
         ...
         else if(this._quest.type == 1 || this._quest.type == 0)
         {
            _loc1_ = "【主线】";
         }
         else if(this._quest.type == 3 || this._quest.type == 2 || this.specialList.indexOf(this._quest.id) != -1)
         {
            _loc1_ = "【支线】";
         }
         else if(this._quest.type == 5)
         {
            _loc1_ = "【活动】";
         }
      ...
      {
         if(this._quest.isNewOnline && this._quest.status == 0)
         {
            this._flagMc.gotoAndStop(1);
         }
         else if(!this._quest.isNewOnline && this._quest.status == 0)
         {
            this._flagMc.gotoAndStop(2);
         }
         else if(this._quest.status == 1)
         {
            this._flagMc.gotoAndStop(3);
         }
         else if(this._quest.status == 3)
         {
      ...
      private function updateStep() : void
      {
         ...
         var _loc3_:* = null;
         if(this._quest.status == 1)
         {
            if(this._quest.type == 2)
            {
               _loc5_ = this._quest.des;
            }
            else
            {
               _loc5_ = this._quest.getCurrentOrNextStep().des;
            }
            _loc4_ = /(?<!')#.+?#/g;
            _loc7_ = [];
            if(_loc5_ != null && _loc5_.match(_loc4_) != null)
            {
               _loc7_ = _loc5_.match(_loc4_);
               if(_loc7_.length == 0 && this.specialList.indexOf(this._quest.id) != -1)
               {
                  changeToLen(1,QuestTinyStepItem);
                  _loc6_ = "<a href=\'event:BagginsLogPanel\'><font color=\'#33EE00\'><u>" + this._quest.getCurrentOrNextStep().des + "</u></font></a>";
                  getChildItemAt(0).update({
                     "data":this.data,
                     "txt":_loc6_
                  });
               ...
               {
                  changeToLen(_loc7_.length,QuestTinyStepItem);
                  _loc2_ = 0;
                  while(_loc2_ < _loc7_.length)
                  {
                     _loc1_ = (_loc7_[_loc2_] as String).split("#").join("");
                     _loc3_ = "<a href=\'event:" + MapConfig.formMapNameGetMapId(_loc1_) + "\'><font color=\'#33EE00\'><u>" + "前往" + _loc1_.replace("(副本)","").replace("[副本]","") + "</u></font></a>";
                     getChildItemAt(_loc2_).update({
                        "data":this.data,
                        "txt":_loc3_
                     });
                     _loc2_++;
