package com.taomee.seer2.app.plantSystem
{
   ...
   public class PlantLand
   {
      ...
      public function startSeed() : void
      {
         ...
         {
            PlantLoader.load(URLUtil.getRes("plant/seedItem/" + this._info.plantSeedId + ".swf"),"domain",this.onResLoaded);
         }
         else if(this._info.plantIsOpen == 0 && SceneManager.active.mapID == ActorManager.actorInfo.id)
         {
            if(this._mc.hasEventListener("click"))
            {
               return;
            }
            this._mc.addEventListener("click",this.onGet);
         }
         else if(this._info.plantIsOpen == 1 && SceneManager.active.mapID == ActorManager.actorInfo.id)
         {
            this._mc.addEventListener("click",this.onMouseLand);
         }
      }
      ...
      private function getTimeStr() : String
      {
         ...
         _loc1_ += _loc2_ + (uint(this._info.stageEndTime / 3600)).toString() + ":";
         ...
         _loc1_ += _loc2_ + (uint(this._info.stageEndTime % 3600 / 60)).toString() + ":";
         ...
         return _loc1_ + (_loc2_ + (this._info.stageEndTime % 3600 % 60).toString());
      }
      ...
      public function statusChange(param1:Boolean = false) : void
      {
         ...
         {
            QueueLoader.load(URLUtil.getRes("plant/seedItem/" + this._info.plantSeedId + ".swf"),"swf",function(param1:ContentInfo):void
            {
               var _loc2_:int = 0;
               ...
               {
                  _mc.removeEventListener("click",onGet);
                  _mc.addEventListener("click",onGet);
                  _seedMC.removeEventListener("click",onGet);
                  _seedMC.addEventListener("click",onGet);
               }
            });
         }
         else
         {
            ...
            {
               QueueLoader.load(URLUtil.getRes("plant/seedItem/" + this._info.plantSeedId + ".swf"),"swf",this.onResLoaded);
               return;
            }
            ...
            if(this._info.seedStage == 4)
            {
               this._mc.removeEventListener("click",this.onGet);
               this._mc.addEventListener("click",this.onGet);
               this._seedMC.removeEventListener("click",this.onGet);
               this._seedMC.addEventListener("click",this.onGet);
            }
         }
      }
      ...
      private function onPlantSetInfo(param1:MessageEvent) : void
      {
         ...
         this._plantSetInfo = new PlantSetInfo(_loc3_);
         var _loc5_:uint = uint(_loc3_.readUnsignedInt());
         var _loc4_:uint = uint(_loc3_.readUnsignedInt());
         var _loc2_:uint = uint(_loc3_.readUnsignedInt());
         this._info.plantId = _loc2_;
         this._info.userId = _loc4_;
         this._info.parseInfo(_loc3_);
         if(this._action == 1)
         {
            this.setBubble(4);
            this.openPlant();
      ...
      {
         this._mc.removeEventListener("click",this.onMouseLand);
         if(this._seedMC)
         {
            this._seedMC.removeEventListener("click",this.onGet);
         }
      ...
      private function showComplete() : void
      {
         ...
         ServerBufferManager.getServerBuffer(100014,function(param1:ServerBuffer):void
         {
            ...
            var _loc2_:QuestCompleteAnimation = null;
            var _loc3_:int = 6;
            var _loc5_:int = 0;
            while(_loc5_ < 6)
            {
               _loc4_ = {};
               _loc4_["task_id"] = param1.readDataAtPostion(_loc3_);
               _loc4_["step_id"] = param1.readDataAtPostion(++_loc3_);
               _loc4_["task_status"] = param1.readDataAtPostion(++_loc3_);
               _loc3_++;
               if(_loc4_["task_id"] == 10)
               {
                  if(_loc4_["task_status"] == 0)
                  {
                     _loc2_ = new QuestCompleteAnimation(1);
                     _loc2_.play();
      ...
      {
         this._mc.addEventListener("rollOver",this.onOver);
         this._mc.addEventListener("rollOut",this.onOut);
      }
