package com.taomee.seer2.app.plantSystem
{
   ...
   public class PlantManager
   {
      ...
      {
         this.updateLand();
         PlantEventControl.addEventListener("seedStatusChange",this.onSeedStatusChange);
      ...
      private function updateLand() : void
      {
         ...
         while(_loc2_ < 16)
         {
            this._landList.push(new PlantLand(SceneManager.active.mapModel.content["land" + _loc2_],this._scene.plantInfo.infoList[_loc2_],this._load));
            _loc2_++;
         }
         var _loc1_:int = 0;
         while(_loc1_ < 4)
         {
            SceneManager.active.mapModel.front["vip" + _loc1_].mouseChildren = false;
            SceneManager.active.mapModel.front["vip" + _loc1_].mouseEnabled = false;
            _loc1_++;
         }
      ...
      private function updateSteal() : void
      {
         ...
         this._steal.addEventListener("click",this.onSteal);
      ...
      private function onSelectItem(param1:PlantEvent) : void
      {
         ...
         LayerManager.stage.addEventListener("click",this.onClick);
      ...
      private function onPlantSetInfo(param1:MessageEvent) : void
      {
         ...
         this._landList[_loc3_].info.userId = _loc4_;
         this._landList[_loc3_].info.plantId = _loc3_;
         this._landList[_loc3_].info.parseInfo(_loc2_);
         this._landList[_loc3_].info.seedInfo = SeedConfig.getSeedInfo(this._landList[_loc3_].info.plantSeedId);
         switch(this._type)
         {
            case 0:
               this.updateSeed();
      ...
      private function updateSeed() : void
      {
         ...
         this.isClosePlantMouse(false);
         QueueLoader.load(URLUtil.getRes("plant/seedAnimation/" + this._landList[this._index].info.plantSeedId + ".swf"),"swf",this.onUpdateSeedComplete);
      }
      ...
      private function initRemove() : void
      {
         ...
         this._removeBtn.addEventListener("click",this.onRemove,false,1);
      }
      ...
      private function onRemove(param1:MouseEvent) : void
      {
         ...
         LayerManager.stage.addEventListener("click",this.onClickRemove);
         PlantMouse.startSwapMouse(this._mouse);
         this.isClosePlantMouse(true);
      }
      ...
      {
         LayerManager.stage.removeEventListener("click",this.onClick);
         PlantMouse.endSwapMouse();
      }
      
      private function removeRemoveMouse() : void
      {
         LayerManager.stage.removeEventListener("click",this.onClickRemove);
         PlantMouse.endSwapMouse();
      }
      ...
      public function dispose() : void
      {
         ...
         if(this._removeBtn)
         {
            this._removeBtn.removeEventListener("click",this.onRemove);
         }
         Connection.removeCommandListener(CommandSet.GET_RANKING_LIST_1232,this.onRanking);
         Connection.removeCommandListener(CommandSet.PLANT_SET_INFO_1519,this.onPlantSetInfo);
