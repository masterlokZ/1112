package com.taomee.seer2.app.team
{
   ...
   public class TeamManager extends BaseBean
   {
      ...
      {
         if(SceneManager.active && SceneManager.active.type == 1 && SceneManager.active.mapModel.id == 60)
         {
      ...
      {
         if(SceneManager.active.type == 1 && SceneManager.active.mapModel.id == 60)
         {
      ...
      private static function onTeamNotify(param1:MessageEvent) : void
      {
         ...
         var _loc2_:String = String(_loc6_.readUTFBytes(16));
         var _loc5_:TeamMainInfo = new TeamMainInfo();
         TeamMainInfo.readBaseInfo(_loc5_,_loc6_);
         var _loc4_:int = int(_loc6_.readUnsignedInt());
         TeamMainInfo.readMsgInfo(_loc5_,_loc6_);
         switch(_loc7_)
         {
            case 1084:
               processInvite(_loc7_,_loc3_,_loc2_,_loc5_);
               break;
            case 1092:
               processLeaveTeam(_loc8_,"你已经被踢出了战队");
               dispatchEvent("QuitTeam");
               break;
            case 1086:
               processReplyInvite(_loc3_,_loc2_,_loc4_);
               break;
            case 1118:
               processLeaveTeam(_loc8_,"你所在战队已被队长解散");
               dispatchEvent("teamDisolve");
               break;
            case 1119:
               processTransferCaptain(_loc8_,_loc2_,_loc4_);
               break;
            case 1166:
               processApplyJoinTeam(_loc7_,_loc3_,_loc2_,_loc5_,_loc4_);
               break;
            case 1571:
               rejectJonin(_loc7_,_loc3_,_loc2_,_loc5_);
         }
      }
      ...
      private static function processLeaveTeam(param1:int, param2:String) : void
      {
         ...
         {
            _teamInfo.clear();
            if(SceneManager.currentSceneType == 4)
            {
      ...
      private static function onQuestTeamList(param1:MessageEvent) : void
      {
         ...
         dispatchEvent("queryMemberListComplete",_loc2_);
      }
      ...
      private static function onCheckOnline(param1:MessageEvent) : void
      {
         ...
         _loc5_ = param1.message.getRawData();
         var _loc3_:int = int(_loc5_.readUnsignedInt());
         var _loc2_:int = 0;
         while(_loc2_ < _loc3_)
         {
            _loc4_.push(_loc5_.readUnsignedInt());
            _loc6_.push(_loc5_.readUnsignedShort());
            _loc2_++;
         }
         _memberList.update(_loc4_);
         _memberList.updateServerInfo(_loc4_,_loc6_);
         dispatchEvent("checkMemberListComplete");
      }
