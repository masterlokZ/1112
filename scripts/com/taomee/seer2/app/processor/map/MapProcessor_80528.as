package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_80528 extends MapProcessor
   {
      ...
      override public function init() : void
      {
         ...
         FightManager.addEventListener("FIGHT_OVER",this.overHandler);
         this.story();
         this.checkMobile();
      }
      ...
      private function checkMobile() : void
      {
         ...
         while(i < 3)
         {
            ...
            this._mobiles[i].addEventListener("click",this.toFight);
            this._mobiles[i].setPostion(Positions[i]);
            this._mobiles[i].height = 100;
            MobileManager.addMobile(this._mobiles[i],"npc");
            i++;
         }
         DayLimitListManager.getDaylimitList([1935],(function():*
         {
               ...
               case 2:
                  DayLimitListManager.getDaylimitList([1935],(function():*
                  {
                     ...
                     return callback = function(param1:DayLimitListInfo):void
                     {
                        ...
                        if(BitUtil.getBit(num,8) && BitUtil.getBit(num,9))
                        {
                           index = 3;
                        }
                        if(index == 2)
                        {
                           ...
                           {
                              FightManager.startFightWithWild(FightIds[index]);
                              FightManager.addEventListener("FIGHT_OVER",overHandler);
                           }]);
                        }
                        else
                        {
      ...
      private function overHandler(param1:FightStartEvent) : void
      {
         if(FightManager.getPositionIndex() == FightIds[2] || FightManager.getPositionIndex() == FightIds[3])
         {
            ...
            FightManager.removeEventListener("FIGHT_OVER",this.overHandler);
      ...
      private function isCanFight() : Boolean
      {
         ...
         {
            if([19,20,21].indexOf(_loc1_.type) != -1)
            {
               _loc2_ = false;
               break;
            }
