package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_80177 extends MapProcessor
   {
      ...
      override public function init() : void
      {
         ...
         DayLimitListManager.getDaylimitList([1080,1081,1083],this.getDayData);
      }
      
      private function getDayData(param1:DayLimitListInfo) : void
      {
         this.preTime = param1.getCount(1080);
         this.bossState = param1.getCount(1081);
         this.leftFightNum = param1.getCount(1083) >= 3 ? 0 : uint(3 - param1.getCount(1083));
         ServerBufferManager.getServerBuffer(211,this.getFullState);
      }
      ...
      private function getFullState(param1:ServerBuffer) : void
      {
         ...
         if(!buffer.readDataAtPostion(2))
         {
            ...
            ServerBufferManager.updateServerBuffer(211,2,1);
      ...
      private function setBoss() : void
      {
         ...
         while(i < 3)
         {
            ...
            this.bossList[i].addEventListener("click",this.toFight);
            i++;
         }
         this.buyTimeBtn.addEventListener("click",this.buyTime);
         this.buyCountBtn.addEventListener("click",this.buyCount);
         this.buyPassBtn.addEventListener("click",this.buyPass);
         this.barMc.gotoAndStop(4 - this.leftFightNum);
         overTime = uint(TimeManager.getServerTime() - this.preTime);
         if(overTime < 180)
         {
            this.leftTime = 180 - overTime;
            this.startTime();
         }
         else
         {
      ...
      private function toFight(param1:MouseEvent) : void
      {
         ...
         index = uint(this.bossList.indexOf(event.target as MovieClip));
         switch(int(index))
         {
            case 0:
