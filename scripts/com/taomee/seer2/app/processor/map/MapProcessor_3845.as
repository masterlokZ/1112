package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_3845 extends MapProcessor
   {
      ...
      private static function onBroadCast(param1:MessageEvent) : void
      {
         ...
         ModelLocator.getInstance().dispatchEvent(new LogicEvent("godFirePkBroadCast"));
      }
      ...
      private function pkValHandle() : void
      {
         ...
         Connection.send(CommandSet.GET_TOTAL_VOTE_INFO_1219,126,int(dateStr) * 10 + 1,int(dateStr) * 10 + 4);
         Connection.addCommandListener(CommandSet.GET_TOTAL_VOTE_INFO_1219,(function():*
         {
            ...
            return getCallState = function(param1:MessageEvent):void
            {
               ...
               var _loc4_:Vector.<uint> = Vector.<uint>([int(dateStr) * 10 + 1,int(dateStr) * 10 + 2,int(dateStr) * 10 + 3,int(dateStr) * 10 + 4]);
               var _loc3_:Array = [0,0,0,0];
               var _loc7_:int = 0;
               while(_loc7_ < _loc9_)
               {
                  _loc2_ = uint(_loc8_.readUnsignedInt());
                  _loc5_ = uint(_loc8_.readUnsignedInt());
                  if(_loc4_.indexOf(_loc2_) != -1)
                  {
                     _loc3_[_loc4_.indexOf(_loc2_)] = _loc5_;
                  }
                  _loc7_++;
               }
               var _loc6_:Array = [];
               ...
               {
                  _barList[0].scaleX = _loc6_[0] / (_loc6_[0] + _loc6_[1]);
               }
      ...
      private function addListener() : void
      {
         ...
         ModelLocator.getInstance().addEventListener("godFirePkBroadCast",this.onPkModify);
      }
      ...
      private function isInAct() : Boolean
      {
         ...
         {
            _loc1_ = new Date(TimeManager.getPrecisionServerTime() * 1000);
            if([6,0].indexOf(_loc1_.day) != -1)
            {
