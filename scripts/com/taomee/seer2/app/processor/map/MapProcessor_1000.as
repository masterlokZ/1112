package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_1000 extends MapProcessor
   {
      ...
      private var eyeFightId:Array = [208,206,207,209,210];
      
      private var dayLimitId:Array = [539,537,538,540,541];
      
      private var dayaLimitVec:Vector.<Parser_1065>;
      
      private var fightNum:Array = [1,1,1,1,3];
      ...
      override public function dispose() : void
      {
         ...
         while(_loc1_ < 5)
         {
            this.petVec[_loc1_].removeEventListener("click",this.toFight);
            _loc1_++;
         }
      }
      ...
      private function createNpc() : void
      {
         ...
         _loc1_ = new Mobile();
         _loc4_ = new Mobile();
         var _loc3_:Mobile = new Mobile();
         _loc2_.resourceUrl = URLUtil.getActivityMobile("Pet57");
         _loc2_.x = 438;
         _loc2_.y = 338;
         _loc1_.resourceUrl = URLUtil.getActivityMobile("Pet252");
         _loc1_.x = 353;
         _loc1_.y = 360;
         _loc4_.resourceUrl = URLUtil.getActivityMobile("Pet205");
         _loc4_.x = 790;
         _loc4_.y = 392;
         _loc3_.resourceUrl = URLUtil.getActivityMobile("Pet24");
         _loc3_.x = 725;
         _loc3_.y = 345;
         MobileManager.addMobile(_loc2_,"npc");
         MobileManager.addMobile(_loc1_,"npc");
         MobileManager.addMobile(_loc4_,"npc");
         MobileManager.addMobile(_loc3_,"npc");
      ...
      private function requestDayLimit() : void
      {
         ...
         var _loc2_:int = int(this.dayLimitId.length);
         var _loc1_:LittleEndianByteArray = new LittleEndianByteArray();
         _loc1_.writeUnsignedInt(_loc2_);
         var _loc3_:int = 0;
         while(_loc3_ < _loc2_)
         {
            _loc1_.writeUnsignedInt(this.dayLimitId[_loc3_]);
            _loc3_++;
         }
         Connection.send(CommandSet.GET_RANGE_DAY_LIMIT_1209,_loc1_);
      }
      ...
      private function getLimit(param1:MessageEvent) : void
      {
         ...
         while(_loc4_ < _loc5_)
         {
            _loc2_ = new Parser_1065(_loc3_);
            this.dayaLimitVec.push(_loc2_);
            _loc4_++;
         }
         this.checkFightPet();
         this.initEvent();
      ...
      {
         var _loc6_:Parser_1065 = null;
         var _loc5_:int = 0;
         var _loc2_:Parser_1065 = null;
         var _loc1_:int = 0;
         var _loc4_:int = 0;
         while(_loc4_ < this.dayaLimitVec.length - 1)
         {
            _loc6_ = this.dayaLimitVec[_loc4_];
            _loc5_ = int(this.dayLimitId.indexOf(_loc6_.id));
            if(_loc6_.count >= this.fightNum[_loc5_])
            {
               this.petVec[_loc4_].visible = false;
            }
            _loc4_++;
         }
         var _loc3_:int = 0;
         while(_loc3_ < this.dayaLimitVec.length - 1)
         {
            _loc2_ = this.dayaLimitVec[_loc3_];
            _loc1_ = int(this.dayLimitId.indexOf(_loc2_.id));
            this.isFightPet = true;
            if(_loc2_.count < this.fightNum[_loc1_])
            {
      ...
      private function initEvent() : void
      {
         ...
         while(_loc1_ < 5)
         {
            this.petVec[_loc1_].addEventListener("click",this.toFight);
            _loc1_++;
         }
      }
      ...
      private function toFight(param1:MouseEvent) : void
      {
         ...
         var _loc3_:int = int(this.petVec.indexOf(_loc2_));
         if(_loc3_ == 4)
         {
            if(this.isFightPet)
            {
