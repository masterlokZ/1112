package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_80348 extends MapProcessor
   {
      ...
      private function update() : void
      {
         this.updateData(function():void
         {
            ServerBufferManager.getServerBuffer(268,function(param1:ServerBuffer):void
            {
               ...
               getBarStateHandle();
               if(SceneManager.prevSceneType == 2 && FightManager.currentFightRecord.initData.positionIndex == FIGHT_INDEX_LIST[0])
               {
                  if(_par.infoVec[2] >= AWARD_NUM[2])
                  {
                     SceneManager.changeScene(9,80349);
                  }
                  else
                  {
      ...
      {
         var evt:MouseEvent = param1;
         var canFightNum:int = ActsHelperUtil.getCanNum(this._info.getCount(DAY_LIST[0]),this._par.infoVec[0],5);
         if(canFightNum > 0)
         {
      ...
      private function getBarStateHandle() : void
      {
         ...
         this._fightNum.text = ActsHelperUtil.getCanNum(this._info.getCount(DAY_LIST[0]),this._par.infoVec[0],5).toString();
      }
