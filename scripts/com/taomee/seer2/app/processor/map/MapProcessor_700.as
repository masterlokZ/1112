package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_700 extends MapProcessor
   {
      ...
      {
         DialogPanel.addEventListener("customUnitClick",this.onActivityUnitClick);
         StatisticsManager.sendNovice("0x1003356F");
      ...
      private function onGetEnergyLotteryCnt(param1:MessageEvent) : void
      {
         ...
         if(_loc2_.id == 345)
         {
            ...
            if(_loc3_ >= 1)
            {
               this._animationBtn.removeEventListener("click",this.onAnimation);
            }
            else
            {
               this._animationBtn.addEventListener("click",this.onAnimation);
            }
         }
      ...
      private function onAnimation(param1:MouseEvent) : void
      {
         ...
         var ran:uint = uint(uint(Math.random() * 5) + 1);
         MovieClipUtil.playFullScreen("res/sceneInteractive/" + ran + ".swf",function():void
         {
            _animationBtn.removeEventListener("click",onAnimation);
      ...
      {
         DialogPanel.removeEventListener("customUnitClick",this.onActivityUnitClick);
         Connection.removeCommandListener(CommandSet.DAILY_LIMIT_1065,this.onGetEnergyLotteryCnt);
         this._animationBtn.removeEventListener("click",this.onAnimation);
