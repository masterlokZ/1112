package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_60 extends TitleMapProcessor
   {
      ...
      override public function init() : void
      {
         ...
         StatisticsManager.sendNoviceAccountHttpd("0x1003345D");
         ...
         this.initPetMelee();
         ModelLocator.getInstance().addEventListener("trainerPkUpdate",this.initTwoYearNpcHandle);
      }
      ...
      private function initPetMelee() : void
      {
         ...
         this._petMelee.addEventListener("click",this.onPetMelee);
      }
         ...
         if(DateUtil.inInDateScope(5,13,31,13,41))
         {
            ServerBufferManager.getServerBuffer(29,function(param1:ServerBuffer):void
            {
               ...
               if(_loc2_ == 0)
               {
                  ...
                  ServerBufferManager.updateServerBuffer(29,9,1);
               }
            });
         }
      }
      ...
      private function initScreen() : void
      {
         ...
         this._screen.addEventListener("mouseOver",this.onScreenOver);
         this._screen.addEventListener("mouseOut",this.offScreenOver);
      }
      ...
      private function initPetTraining() : void
      {
         ...
         this._waterPet.addEventListener("click",this.onWaterPetTraining);
         this._firePet = _map.content["firePet"];
         this._firePet.buttonMode = true;
         this._firePet.addEventListener("click",this.onFirePetTraining);
         this._grassPet = _map.content["grassPet"];
         this._grassPet.buttonMode = true;
         this._grassPet.addEventListener("click",this.onGrassPetTraining);
         this._grassPet = _map.content["grassPet"];
         this._grassPet.buttonMode = true;
         this._grassPet.addEventListener("click",this.onGrassPetTraining);
         _map.content["ballBtn"].addEventListener("click",this.onBall);
      }
      ...
      {
         var _loc4_:uint = 0;
         var _loc6_:uint = (PetInfoManager.getFirstPetInfo().resourceId - 1) / 3;
         ...
         var _loc3_:uint = uint(_loc8_.readUnsignedInt());
         var _loc2_:uint = 0;
         while(_loc2_ < _loc3_)
         {
            _loc4_ = uint(_loc8_.readUnsignedInt());
            if(_loc8_.readUnsignedInt() == 0)
            {
               InitPetTrainingManager.currentStep = _loc4_;
               NpcDialog.show(INIT_PET_DIALOG_INFO[_loc6_].id,INIT_PET_DIALOG_INFO[_loc6_].name,[[0,DIALOG_CONTENT[_loc6_]]],["是的，现在就开始训练吧！","我待会再来找你..."],[this.showTrainingPanel,null]);
               return;
            }
            _loc2_++;
         }
         var _loc5_:* = "恭喜你，今天" + PetInfoManager.getFirstPetInfo().name + "完成了所有训练，请明天再来~";
         NpcDialog.show(INIT_PET_DIALOG_INFO[_loc6_].id,INIT_PET_DIALOG_INFO[_loc6_].name,[[0,_loc5_]],["好的，我知道了！"]);
      }
      ...
      {
         Connection.removeCommandListener(CommandSet.INITPET_TRAINING_STEP_STATUS_1088,this.onGetTrainingStatus);
         this._waterPet.removeEventListener("click",this.onWaterPetTraining);
         this._firePet.removeEventListener("click",this.onFirePetTraining);
         this._grassPet.removeEventListener("click",this.onGrassPetTraining);
         this._waterPet = null;
         this._firePet = null;
         this._grassPet = null;
      }
      ...
      private function onItemGiven(param1:ItemEvent) : void
      {
         ...
         if(_loc2_.cmdId == 1162)
         {
            ...
            this._petPyramidAnn.addEventListener("enterFrame",this.onAnnEnterFrame);
         }
      }
      ...
      private function onAnnEnterFrame(param1:Event) : void
      {
         ...
         {
            this._petPyramidAnn.removeEventListener("enterFrame",this.onAnnEnterFrame);
      ...
      private function onPetPyramidBtnClick(param1:MouseEvent) : void
      {
         ...
         StatisticsManager.sendNovice("0x10033481");
      }
      ...
      {
         ItemManager.removeEventListener1("serverItemGiven",this.onItemGiven);
      ...
      private function initTwoYearNpcHandle(param1:LogicEvent = null) : void
      {
         ...
         this._oldNpc = MobileManager.getMobile(6,"npc");
         if(this._oldNpc)
         {
         ...
         DayLimitListManager.getDaylimitList(this.DAY_LIST,function(param1:DayLimitListInfo):void
         {
            ...
            {
               if(!MobileManager.getMobile(6,"npc"))
               {
      ...
      private function initNpc1() : void
      {
         ...
         MobileManager.addMobile(this._twoYearNpc,"npc");
         this._twoYearNpc.addEventListener("click",this.onTwoYearNpcClick);
      }
      ...
      private function afterFinish() : void
      {
         if(this._twoYearNpc)
         {
            this._twoYearNpc.removeEventListener("click",this.onTwoYearNpcClick);
      ...
      private function disposeTwoYearNpcHandle() : void
      {
         ...
         this._oldNpc = null;
         clearInterval(this.timeId);
         ModelLocator.getInstance().removeEventListener("trainerPkUpdate",this.initTwoYearNpcHandle);
      }
   }
}
