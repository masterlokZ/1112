package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_80124 extends MapProcessor
   {
      
      private const SWAP_ID:uint = 2312;
      ...
      private var bloodNums:Array = [1,1,1];
      
      public function MapProcessor_80124(param1:MapModel)
      {
         super(param1);
      }
      ...
      {
         super.dispose();
         ShootController.removeEventListener("playEnd",this.shootComplete);
      }
      ...
      override public function init() : void
      {
         ...
         isA = currentDate.fullYear == 2014 && currentDate.month == 0 && currentDate.date == 18 && currentDate.hours == 14 && currentDate.minutes < 21;
         isB = currentDate.fullYear == 2014 && currentDate.month == 0 && currentDate.date == 19 && currentDate.hours == 14 && currentDate.minutes < 21;
         if(isA || isB)
         {
            ShootController.addEventListener("playEnd",this.shootComplete);
            ...
            while(i < 3)
            {
               this.bloodList.push(_map.front["blood" + i]);
               this.bloodList[i].gotoAndStop(1);
               this.shootTargetList.push(_map.front["target" + i]);
               this.eftList.push(_map.front["eft" + i]);
      ...
      private function shootComplete(param1:ShootEvent) : void
      {
         ...
         var isA:Boolean = currentDate.fullYear == 2014 && currentDate.month == 0 && currentDate.date == 18 && currentDate.hours == 14 && currentDate.minutes < 21;
         var isB:Boolean = currentDate.fullYear == 2014 && currentDate.month == 0 && currentDate.date == 19 && currentDate.hours == 14 && currentDate.minutes < 21;
         if(!isA && !isB)
         {
            AlertManager.showAlert("活动时间已经结束了,去传送室看看其他活动吧!",function():void
            {
         ...
         while(i < 3)
         {
            if(info.userID == ActorManager.actorInfo.id && this.shootTargetList[i].hitTestPoint(info.endPos.x,info.endPos.y))
            {
               ...
               if(this.bloodNums[i] == 35)
               {
                  ...
                  SwapManager.swapItem(2312,1,function(param1:IDataInput):void
                  {
                     new SwapInfo(param1);
                  });
               }
