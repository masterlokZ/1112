package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_80249 extends MapProcessor
   {
      ...
      override public function init() : void
      {
         ...
         this.updata();
         ServerBufferManager.getServerBuffer(257,function(param1:ServerBuffer):void
         {
            var _loc2_:Boolean = Boolean(param1.readDataAtPostion(1));
            ServerBufferManager.updateServerBuffer(257,1,1);
            if(_loc2_ == false)
            {
         ...
         this._hurtBar["getPet0"].addEventListener("click",this.onGetRwd);
         this._hurtBar["getPet1"].addEventListener("click",this.onGetRwd);
         this._hurtBar["getPet2"].addEventListener("click",this.onGetRwd);
      }
      ...
      private function setup() : void
      {
         ...
         this._mask.y = 0;
         this._fightBtn.addEventListener("click",this.onClick);
         this._readyBtn.addEventListener("click",this.onClick);
         this._exitBtn.addEventListener("click",this.onExitCopy);
         var _loc1_:SimpleButton = _map.front["enter"]["enterBtn"] as SimpleButton;
         _loc1_.addEventListener("click",this.onEnter);
      ...
      private function updata() : void
      {
         ActiveCountManager.requestActiveCountList([FOREVER_LIMIT[2],FOREVER_LIMIT[3],FOREVER_LIMIT[4],FOREVER_LIMIT[5],FOREVER_LIMIT[6]],function(param1:Parser_1142):void
         {
            ...
            if(_copyOverBoo)
            {
               ServerMessager.addMessage("获得海啸值 +" + _loc4_);
            }
            if(_barCount >= 50)
            {
      ...
      private function onComplete(param1:SceneEvent) : void
      {
         ...
         {
            _loc2_ = int(FIGHT_ID.indexOf(FightManager.currentFightRecord.initData.positionIndex));
            if(_loc2_ != -1)
            {
            }
            _copyOverBoo = true;
      ...
      override public function dispose() : void
      {
         ...
         {
            this._fightBtn.removeEventListener("click",this.onClick);
            this._readyBtn.removeEventListener("click",this.onClick);
            this._exitBtn.removeEventListener("click",this.onExitCopy);
            _loc1_ = _map.front["enter"]["exitBtn"] as SimpleButton;
            _loc1_.removeEventListener("click",this.onEnter);
         }
         super.dispose();
      }
   }
