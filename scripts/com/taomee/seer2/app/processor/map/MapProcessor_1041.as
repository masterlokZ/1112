   ...
   public class MapProcessor_1041 extends MapProcessor
   {
      ...
      private function initUI() : void
      {
         ...
         while(_loc1_ < 5)
         {
            ...
            this.boxVec[_loc1_].addEventListener("click",this.clickBox);
            _loc1_++;
         }
      ...
      private function clickBox(param1:MouseEvent) : void
      {
         ...
         _loc2_ = int(this.boxVec.indexOf(MovieClip(param1.target)));
         _loc3_ = 604 + _loc2_;
      ...
      {
         SceneManager.changeScene(1,1040);
      }
      ...
      private function initShoot() : void
      {
         ...
         ShootController.addEventListener("playStart",this.disEnableMap);
         ShootController.addEventListener("playEnd",this.checkTarget);
      }
      ...
      private function checkTarget(param1:ShootEvent) : void
      {
         ...
         if(!this.isSuccess && _loc2_.userID == ActorManager.actorInfo.id && this.werWolf.hitTestPoint(_loc2_.endPos.x,_loc2_.endPos.y))
         {
            ...
            _loc4_ = this.werWolfBloodNum / 120;
            this.setBarProgress(this.werWolfBloodMc,_loc4_);
            if(this.werWolfBloodNum <= 0)
            {
               ...
               while(_loc3_ < 5)
               {
                  this.removeBoxAndBar(_loc3_,false);
                  if(Boolean(this.boxVec[_loc3_].parent) && !this.boxVec[_loc3_].visible)
                  {
                     this.boxVec[_loc3_].visible = true;
                  }
                  _loc3_++;
               }
               this.isSuccess = true;
      ...
      private function createTips() : void
      {
         ...
         while(_loc2_ < 5)
         {
            if(this.boxVec[_loc2_].parent)
            {
               _loc1_ = new MouseClickHintSprite();
               _loc1_.x = this.boxVec[_loc2_].x;
               _loc1_.y = this.boxVec[_loc2_].y;
               this.hitTipsVec[_loc2_] = _loc1_;
               this.boxVec[_loc2_].parent.addChild(this.hitTipsVec[_loc2_]);
            }
            _loc2_++;
         }
      }
