package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_80320 extends MapProcessor
   {
      ...
      private var dayLimit:Array = [1353,1354,1365,1366];
      ...
      private var wTimer:Timer = new Timer(100);
      
      private var giftTimer:Timer = new Timer(100);
      ...
      private function initMC() : void
      {
         ...
         this.boxBtn.addEventListener("click",this.onClickBox);
         this.iceBtn.addEventListener("click",this.onIce);
         this.hintMC.addEventListener("click",this.onHammerMc);
         this.checkStates();
         this.progressSp.addEventListener("enterFrame",this.onEnter);
         this.wTimer.addEventListener("timer",this.onWaitBar);
         this.giftTimer.addEventListener("timer",this.onGiftBar);
      }
      ...
      {
         --this.leftTime;
         var _loc1_:String = DateUtil.getMS(this.leftTime);
      ...
      private function updateBloodBar() : void
      {
         ...
         Connection.addCommandListener(CommandSet.GET_TOTAL_VOTE_INFO_1219,(function():*
         {
            ...
            return getCallState = function(param1:MessageEvent):void
            {
               ...
               var _loc4_:Vector.<uint> = Vector.<uint>([int(dateStr)]);
               var _loc3_:Array = [0];
               var _loc6_:int = 0;
               while(_loc6_ < _loc8_)
               {
                  _loc5_ = uint(_loc7_.readUnsignedInt());
                  _loc2_ = uint(_loc7_.readUnsignedInt());
                  if(_loc4_.indexOf(_loc5_) != -1)
                  {
                     _loc3_[_loc4_.indexOf(_loc5_)] = _loc2_;
                  }
                  _loc6_++;
               }
               curBlood = 100000 - _loc3_[0];
               curBlood = curBlood < 0 ? 0 : curBlood;
               boxBar.scaleX = curBlood / 100000;
      ...
      {
         this.progressSp.removeEventListener("enterFrame",this.onEnter);
         this.wTimer.stop();
      }
   }
}
