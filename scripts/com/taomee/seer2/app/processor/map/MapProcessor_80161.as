package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_80161 extends TitleMapProcessor
   {
      ...
      {
         this._use.addEventListener("click",this.onUse);
         this._help.addEventListener("click",this.onHelp);
         this._tip.addEventListener("click",this.onTip);
         this._addTurn.addEventListener("click",this.onAddTurn);
         this._leave.addEventListener("click",this.onLeaveClick);
      }
         ...
         else if(this._par.infoVec[2] == 2)
         {
            ItemManager.addEventListener1("requestSpecialItemSuccess",(function():*
            {
               ...
               return getItem = function(param1:ItemEvent):void
               {
                  ...
                  num = int(ItemManager.getSpecialItem(MI_ID_LIST[1]) != null ? int(ItemManager.getSpecialItem(MI_ID_LIST[1]).quantity) : 0);
                  if(num > 0)
                  {
      ...
      private function onLeave() : void
      {
         ...
         SwapManager.swapItem(SWAP_LIST[0],1,function(param1:IDataInput):void
         {
            ...
            ActiveCountManager.requestActiveCountList(FOR_LIST,function(param1:Parser_1142):void
            {
               ...
               _par = par;
               var index:int = getChangeIndex(oldPar.infoVec[0],_par.infoVec[0]);
               if(index != -1)
               {
      ...
      private function getChangeIndex(param1:int, param2:int) : int
      {
         ...
         while(_loc3_ < 4)
         {
            if(BitUtil.getBit(param1,_loc3_) != BitUtil.getBit(param2,_loc3_))
            {
               _loc4_ = _loc3_;
               break;
            }
      ...
      private function update() : void
      {
         ...
         ActiveCountManager.requestActiveCountList(FOR_LIST,function(param1:Parser_1142):void
         {
            ...
            DayLimitListManager.getDaylimitList(DAY_LIST,function(param1:DayLimitListInfo):void
            {
               ...
               _turnNum.text = getCanNum(_info.getCount(DAY_LIST[1]),_info.getCount(DAY_LIST[2]),3).toString();
      ...
      private function onNpc(param1:MouseEvent) : void
      {
         ...
         else
         {
            ...
            {
               FightManager.startFightWithWild(855);
            }]);
         }
      }
      ...
      private function yingMoAnShaActDispose() : void
      {
         this.removeNpc();
         this.removeMouseHint();
         this._use.removeEventListener("click",this.onUse);
         this._help.removeEventListener("click",this.onHelp);
         this._tip.removeEventListener("click",this.onTip);
         this._addTurn.removeEventListener("click",this.onAddTurn);
         this._leave.removeEventListener("click",this.onLeaveClick);
         this._use = null;
         this._help = null;
         this._tip = null;
         this._allHurt = null;
