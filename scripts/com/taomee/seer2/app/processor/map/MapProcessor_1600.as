package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_1600 extends MapProcessor
   {
      ...
      {
         super.dispose();
         DialogPanel.removeEventListener("customReplyClick",this.getReplay);
         DialogPanel.removeEventListener("dialogShow",this.onDiaLogShow);
         NpcManager.removeEventListener("actorArrive",this.onArrive);
      }
      
      override public function init() : void
      {
         var _loc1_:int = 0;
         StatisticsManager.sendNovice("0x10034064");
         DialogPanel.addEventListener("customReplyClick",this.getReplay);
         NpcManager.addEventListener("actorArrive",this.onArrive);
         var _loc2_:Vector.<Mobile> = MobileManager.getMobileVec("npc");
         if(_loc2_)
         {
            _loc1_ = 0;
            while(_loc1_ < _loc2_.length)
            {
               if(_loc2_[_loc1_])
               {
                  (_loc2_[_loc1_] as Mobile).buttonMode = false;
               }
               _loc1_++;
            }
         }
      ...
      protected function getReplay(param1:DialogPanelEvent) : void
      {
         ...
         {
            ServerBufferManager.getServerBuffer(121,function(param1:ServerBuffer):void
            {
               ...
               if(mark == 0)
               {
                  NpcDialog.show(337,"天龙骑士-唐",[[0,"想加入赛尔骑士团？！那么至少你要拥有一只驯化过的坐骑精灵！"]],["使用引导","获得骑乘晶片","只是逛逛而已"],[function():void
                  {
                     ...
                     {
                        ModuleManager.addEventListener("PetRideGuidePanel","dispose",onGuidePanelDispose);
                        ModuleManager.toggleModule(URLUtil.getAppModule("PetRideGuidePanel"));
                        ServerBufferManager.updateServerBuffer(121,1,1);
                     }]);
                  },function():void
                  {
      ...
      protected function onDiaLogShow(param1:DialogPanelEvent) : void
      {
         ...
         {
            _loc2_ = new CustomUnit("active","天龙冲锋号！骑宠大赛","天龙冲锋号！骑宠大赛");
      ...
      private function onCustomUnitClick(param1:DialogPanelEvent) : void
      {
         ...
         if(str == "天龙冲锋号！骑宠大赛")
         {
            ...
            {
               var _loc2_:* = new Date(TimeManager.getPrecisionServerTime() * 1000);
               var _loc1_:* = new Date(2013,8,29,6);
               if(_loc2_ < _loc1_)
               {
      ...
      {
         ModuleManager.removeEventListener("PetRideGuidePanel","dispose",this.onGuidePanelDispose);
         ModuleManager.toggleModule(URLUtil.getAppModule("PetRideShopPanel"));
      }
   }
}
