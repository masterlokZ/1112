package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_80219 extends MapProcessor
   {
      ...
      {
         QueueLoader.load(URLUtil.getActivityAnimation("superGaiYaAct/SuperGaiYaAct"),"swf",function(param1:ContentInfo):void
         {
      ...
      {
         DayLimitManager.getDoCount(1207,function(param1:int):void
         {
            var val:int = param1;
            if(val != 63)
            {
            ...
            else
            {
               ...
               DayLimitManager.getDoCount(1206,function(param1:int):void
               {
                  if(param1 == 0)
                  {
                     playBossMc();
      ...
      private function createOpenMenu() : void
      {
         ...
         while(_loc1_ < 3)
         {
            ...
            this._openList[_loc1_].addEventListener("click",this.onOpen);
            _loc1_++;
         }
      }
      ...
      {
         var _loc2_:int = int(this._openList.indexOf(param1.currentTarget as SimpleButton));
         switch(_loc2_)
         {
            case 0:
      ...
      {
         ActiveCountManager.requestActiveCount(204366,function(param1:uint, param2:uint):void
         {
            var type:uint = param1;
            var val:uint = param2;
            if(val == 0)
            {
               SwapManager.swapItem(2936,1,function(param1:IDataInput):void
               {
                  new SwapInfo(param1);
            ...
            {
               ItemManager.addEventListener1("requestSpecialItemSuccess",(function():*
               {
                  var getItem:Function;
                  return getItem = function(param1:ItemEvent):void
                  {
                     var num:int;
                     var evt:ItemEvent = param1;
                     ItemManager.removeEventListener1("requestSpecialItemSuccess",getItem);
                     num = int(ItemManager.getSpecialItem(MI_BUY_LIST[1]) != null ? int(ItemManager.getSpecialItem(MI_BUY_LIST[1]).quantity) : 0);
                     if(num > 0)
                     {
                        AlertManager.showAlert("你已经是战斗加成效果了，使用完了再买吧!");
                     }
      ...
      private function clearOpenMenu() : void
      {
         ...
         if(this._openList)
         {
            ...
            {
               this._openList[_loc1_].removeEventListener("click",this.onOpen);
               _loc1_++;
            }
            this._openList = null;
         }
      ...
      private function addPetByIndex(param1:int) : void
      {
         ...
         this._activityMonsterVec[param1].addEventListener("click",this.onSmallBossClick);
      }
      
      private function onSmallBossClick(param1:MouseEvent) : void
      {
         var _loc2_:int = int(this._activityMonsterVec.indexOf(param1.currentTarget as Mobile));
      ...
      private function clearPetList() : void
      {
         ...
         if(this._activityMonsterVec)
         {
            ...
            while(_loc1_ < this._activityMonsterVec.length)
            {
               ...
               {
                  this._activityMonsterVec[_loc1_].removeEventListener("click",this.onSmallBossClick);
                  MobileManager.removeMobile(this._activityMonsterVec[_loc1_],"npc");
               }
               _loc1_++;
            }
         }
      ...
      private function createNpc(param1:Array) : Mobile
      {
         ...
         var _loc4_:Array = param1[0] as Array;
         var _loc6_:int = param1[1] as int;
         var _loc5_:Array = param1[2] as Array;
         var _loc3_:String = param1[3] as String;
         _loc2_ = new Mobile();
         _loc2_.width = _loc4_[0];
         _loc2_.height = _loc4_[1];
         _loc2_.setPostion(new Point(_loc5_[0],_loc5_[1]));
         _loc2_.resourceUrl = URLUtil.getNpcSwf(_loc6_);
         _loc2_.labelPosition = 1;
         _loc2_.label = _loc3_;
      ...
      private function startFight() : void
      {
         ...
         {
            var val:int = param1;
            ActiveCountManager.requestActiveCount(204352,function(param1:uint, param2:uint):void
            {
               var type:uint = param1;
