package com.taomee.seer2.app.processor.map.diceThing
{
   ...
   public class DiceMap10372Manager
   {
      ...
      private static var _ins:DiceMap10372Manager;
      
      private var dayLimit:Array = [1313,1314,1315];
      
      private var forever:Array = [204670,204671,204672];
      
      private var swap_id:Array = [3126,3128,3127,3135,3136];
      
      private var fight_id:Array = [1201,1202,1203];
      ...
      public static function get ins() : DiceMap10372Manager
      {
         if(_ins == null)
         {
            _ins = new DiceMap10372Manager();
         }
         return _ins;
      }
      ...
      public function setup() : void
      {
         ...
         if(this.initBoo == false)
         {
            ...
            this._tm.addEventListener("timer",this.run);
         }
         ...
         this._diceMc.addEventListener("click",this.onDice);
         this._swapBtn.addEventListener("click",this.onDice);
         this._getBtn.addEventListener("click",this.onDice);
      ...
      private function onXmlLoaded(param1:ContentInfo) : void
      {
         ...
         var _loc3_:XML = XML(param1.content);
         var _loc5_:XMLList = _loc3_.tile;
         this._tileArr = [];
         for each(_loc4_ in _loc5_)
         {
            _loc2_ = {};
            _loc2_.seat = int(_loc4_.@seat);
            _loc2_.thingType = String(_loc4_.@tileType);
            _loc2_.x = int(_loc4_.@x);
            _loc2_.y = String(_loc4_.@y);
            this._tileArr.push(_loc2_);
         }
         this._tileArr.sortOn("seat",16);
      }
            ...
            DayLimitManager.getDoCount(dayLimit[1],function(param1:int):void
            {
               ...
               switch(val)
               {
                  ...
                  case 102:
                     ...
                     currentThing.addEventListener("thing_over",thingOver);
      ...
      private function thingOver(param1:DiceThingEvent) : void
      {
         ...
         SwapManager.swapItem(this.swap_id[2],1,function(param1:IDataInput):void
         {
            ...
            update(function():void
            {
               ...
               {
                  _roleMobile.addEventListener("finished",moveFinished);
                  moveFinished();
               }
               else
               {
      ...
      private function update(param1:Function = null) : void
      {
         ...
         DayLimitManager.getDoCount(this.dayLimit[0],function(param1:int):void
         {
            ...
            ActiveCountManager.requestActiveCountList(forever,function(param1:Parser_1142):void
            {
               ...
               var _loc4_:int = int(param1.infoVec[0]);
               var _loc6_:int = int(param1.infoVec[1]);
               var _loc5_:int = int(param1.infoVec[2]);
               _scoreFd.text = _loc6_ + "/100";
               score = _loc6_;
               _loc3_ = ActorManager.actorInfo.vipInfo.isVip();
               var _loc2_:int = _loc3_ ? 15 : 10;
               _remainFd.text = ActsHelperUtil.getCanNum(val,_loc4_,_loc2_) + "";
               _mark.width = _loc6_ / 100 * 450;
               if(_loc6_ < 100)
               {
