package com.taomee.seer2.app.processor.map.diceThing
{
   ...
   public class DiceMapLuffyManager
   {
      ...
      private function toSwap(param1:MouseEvent) : void
      {
         ...
         ModuleManager.addEventListener("LuffySwapPanel","dispose",this.onDispose);
      }
      
      private function onDispose(param1:ModuleEvent) : void
      {
         ModuleManager.removeEventListener("LuffySwapPanel","dispose",this.onDispose);
         this.getCoinNum();
      }
      ...
      private function getLeftCount() : void
      {
         DayLimitManager.getDoCount(this.useCountId,function(param1:uint):void
         {
            ...
            ActiveCountManager.requestActiveCount(buyedCountId,function(param1:uint, param2:uint):void
            {
               ...
               {
                  _leftCount = _loc4_ + freeCount - useCount;
               }
               _loc3_ = _map.content["countTxt"]["leftTxt"];
               _loc3_.text = "还可以抛" + _leftCount + "次";
            });
         });
      }
      ...
      private function clickDice(param1:MouseEvent) : void
      {
         ...
         if(this._leftCount <= 0)
         {
            ...
            AlertManager.showConfirm("你没有可以用的骰子了\n花费" + ShopPanelConfig.getItemPrice(604103) + "星钻买一个?",function():void
            {
               PayManager.buyItem(604103,function():void
               {
                  getLeftCount();
      ...
      private function checkThing() : void
      {
         ...
         switch(tileInfo.thingType)
         {
            ...
            case "chance":
               ...
               {
                  var _loc3_:DiceThingInfo = null;
                  var _loc5_:String = "com.taomee.seer2.app.processor.map.diceThing.";
                  var _loc7_:String = "chance.";
                  var _loc6_:String = tileInfo.thingType + int(Math.random() * 2);
                  _loc3_ = DiceMapThingConfig.thingMap.getValue(_loc6_) as DiceThingInfo;
                  _loc3_.currentSeat = currentTileSeat;
                  var _loc2_:String = _loc3_.className;
                  var _loc4_:Class = DomainUtil.getCurrentDomainClass(_loc5_ + _loc7_ + _loc2_);
                  currentThing = new _loc4_(_loc3_);
