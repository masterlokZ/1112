package com.taomee.seer2.app.processor.map.diceThing
{
   ...
   public class DiceMapSanjiManager
   {
      ...
      private function initUI() : void
      {
         ...
         this._getBtn.addEventListener("click",this.onGetBtn);
         if(!this.roleMobile)
         {
            this.initMobile();
         }
      ...
      private function toSwap(param1:MouseEvent) : void
      {
         ...
         ModuleManager.addEventListener("SanjiSwapPanel","dispose",this.onDispose);
      }
      
      private function onDispose(param1:ModuleEvent) : void
      {
         ModuleManager.removeEventListener("SanjiSwapPanel","dispose",this.onDispose);
         this.getCoinNum();
      }
      ...
      private function getCoinNum(param1:Boolean = false, param2:String = "") : void
      {
         ...
         {
            var _loc3_:int = 0;
            var _loc5_:TextField = _map.front["scoreFd"];
            _loc5_.text = param2 + "/100";
            ...
            if(needTip)
            {
               _loc3_ = param2 - _curCoin;
               if(_loc3_ > 0)
               {
                  ServerMessager.addMessage(tipTxt + "+" + _loc3_ + "亲密度");
               }
               else if(_loc3_ < 0)
               {
                  ServerMessager.addMessage(tipTxt + _loc3_ + "亲密度");
      ...
      private function clickDice(param1:MouseEvent) : void
      {
         ...
         if(this._leftCount <= 0)
         {
            ...
            AlertManager.showConfirm("你没有可以用的骰子了\n是否花费" + ShopPanelConfig.getItemPrice(604179) + "星钻买一个?",function():void
            {
               PayManager.buyItem(604179,function():void
               {
                  getLeftCount();
         ...
         SwapManager.swapItem(3128,1,function(param1:IDataInput):void
         {
            ...
            DayLimitManager.getDoCount(1314,function(param1:uint):void
            {
               ...
               if(param1 >= 11000)
               {
                  ...
                  ModuleManager.addEventListener("DiceBBQPanel","dispose",onGameOver);
               }
      ...
      {
         ModuleManager.removeEventListener("DiceBBQPanel","dispose",this.onGameOver);
