package com.taomee.seer2.app.processor.map.diceThing
{
   ...
   public class DiceMapPterosaurManager
   {
      ...
      private static var _instance:DiceMapPterosaurManager;
      ...
      private var _numValueList:Vector.<uint> = new Vector.<uint>();
      ...
      public static function getInstance() : DiceMapPterosaurManager
      {
         if(!_instance)
         {
            _instance = new DiceMapPterosaurManager(new instancs());
         }
         return _instance;
      }
      ...
      public function dispose(param1:SceneEvent) : void
      {
         ...
         if(this._swapBtn)
         {
            this._swapBtn.removeEventListener("click",this.toSwap);
         }
      ...
      private function parseXml() : void
      {
         ...
         QueueLoader.load(URLUtil.getDiceMapTiles("pterosaur.swf"),"swf",this.onTilesLoaded);
      }
      ...
      private function onTilesLoaded(param1:ContentInfo) : void
      {
         ...
         var _loc4_:ApplicationDomain = param1.domain;
         var _loc5_:uint = uint(this._map.ground.numChildren - 1);
         this._seatX = new Vector.<int>();
         this._seatY = new Vector.<int>();
         var _loc3_:int = 0;
         while(_loc3_ < this._maxSeat)
         {
            _loc6_ = DomainUtil.getClass(this._tileVec[_loc3_].thingType,_loc4_);
            _loc2_ = new _loc6_();
            if(_loc3_ == 0)
            {
               _loc2_.x = this._startX - this.OFFSET_PLAYER_X;
               _loc2_.y = this._startY - this.OFFSET_PLAYER_Y;
            }
            else
            {
               _loc2_.x = this._seatX[_loc3_ - 1] + this.OFFSET_TILES_X * (-(int((this._tileVec[_loc3_].before - 1) / 2)) * 2 + 1);
               _loc2_.y = this._seatY[_loc3_ - 1] + this.OFFSET_TILES_Y * (2 - Math.abs(this._tileVec[_loc3_].before - 2.5) * 2);
            }
            this._seatX.push(_loc2_.x);
            this._seatY.push(_loc2_.y);
            if(this._tileVec[_loc3_].before == 2 || this._tileVec[_loc3_].before == 3)
            {
               _loc5_ = uint(this._map.ground.numChildren - 1);
            }
            this._map.ground.addChildAt(_loc2_,_loc5_);
            _loc3_++;
         }
         this.initUI();
      }
      ...
      private function initUI() : void
      {
         ...
         this._swapBtn.addEventListener("click",this.toSwap);
         if(!this._roleMobile)
         {
            this.initMobile();
         }
         this.getPartsNum();
         this.getLeftCount();
         SceneManager.addEventListener("switchComplete",this.onFightComplete);
      }
      
      private function toSwap(param1:MouseEvent) : void
      {
         ModuleManager.showAppModule("PterosaurPanel");
         ModuleManager.addEventListener("PterosaurPanel","hide",this.onHide);
      }
      
      private function onHide(param1:ModuleEvent) : void
      {
         ModuleManager.removeEventListener("PterosaurPanel","hide",this.onHide);
         this.getPartsNum();
      }
      ...
      private function getPartsNum(param1:Boolean = false, param2:String = "") : void
      {
         ...
         ActiveCountManager.requestActiveCountList(this.FOR_ITEM,function(param1:Parser_1142):void
         {
            ...
            if(needTip)
            {
               ...
               while(_loc3_ < 5)
               {
                  _loc2_ = param1.infoVec[_loc3_] - _numValueList[_loc3_];
                  if(_loc2_ > 0)
                  {
                     _loc4_ += " " + ITEM_NAME[_loc3_] + "零件+" + _loc2_ + ",";
                     _loc6_ += _loc3_ + 1;
                     _loc5_ = _loc2_;
                  }
                  else if(_loc2_ < 0)
      ...
      {
         DayLimitManager.getDoCount(1432,function(param1:uint):void
         {
            var c:uint = param1;
            _useCount = c;
            ActiveCountManager.requestActiveCount(205015,function(param1:uint, param2:uint):void
            {
      ...
      private function clickDice(param1:MouseEvent) : void
      {
         ...
         {
            AlertManager.showConfirm("你没有可以用的骰子了\n是否花费" + ShopPanelConfig.getItemPrice(605152) + "星钻买一个?",function():void
            {
               PayManager.buyItem(605152,function():void
               {
                  getLeftCount();
                  SwapManager.swapItem(3355,1,dicePlay);
               },function():void
               {
         ...
         {
            SwapManager.swapItem(3355,1,this.dicePlay);
         }
      }
      ...
      private function dicePlay(param1:IDataInput = null) : void
      {
         ...
         {
            ActiveCountManager.requestActiveCount(205022,function(param1:uint, param2:uint):void
            {
      ...
      private function moveSeat() : void
      {
         ...
         this._roleMobile.addEventListener("finished",this.moveFinished);
      ...
      {
         this._roleMobile.removeEventListener("finished",this.moveFinished);
      ...
      private function checkThing() : void
      {
         ...
         {
            case "fight":
               FightManager.startFightWithBoss(1324);
               break;
            case "game":
               this.startGame();
               break;
      ...
      {
         ModuleManager.addEventListener("PterosaurGamePanel","dispose",this.onGameOver);
         ModuleManager.showAppModule("PterosaurGamePanel");
      }
      
      private function onGameOver(param1:ModuleEvent) : void
      {
         ModuleManager.removeEventListener("PterosaurGamePanel","dispose",this.onGameOver);
      ...
      private function initMobile() : void
      {
         if(this._roleMobile)
         {
            MobileManager.removeMobile(this._roleMobile,"pet");
            this._roleMobile = null;
         }
         if(this.roleMobileID != 0)
         {
            ...
            MobileManager.addMobile(this._roleMobile,"pet");
         }
         else
         {
