package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_80062 extends MapProcessor
   {
      ...
      private function addBoss() : void
      {
         ...
         {
            _loc2_ = new Mobile();
            _loc2_.width = BOSS_SIZE[_loc1_][0];
            _loc2_.height = BOSS_SIZE[_loc1_][1];
            _loc2_.x = BOSS_POS[_loc1_][0];
            _loc2_.y = BOSS_POS[_loc1_][1];
            _loc2_.buttonMode = true;
            _loc2_.resourceUrl = URLUtil.getNpcSwf(NPC_ID[_loc1_]);
      ...
      private function addBossClickEvent() : void
      {
         ...
         while(_loc1_ < this._bossList.length)
         {
            this._bossList[_loc1_].addEventListener("click",this.onBossClick);
            _loc1_++;
         }
      }
      ...
      private function removeEvent() : void
      {
         ...
         if(this._bossList)
         {
            ...
            while(_loc1_ < this._bossList.length)
            {
               this._bossList[_loc1_].removeEventListener("click",this.onBossClick);
               _loc1_++;
            }
         }
         if(this._menu)
         {
            this._menu.removeEventListener("click",this.onMenu);
         }
      }
      
      private function onBossClick(param1:MouseEvent) : void
      {
         var _loc2_:int = int(this._bossList.indexOf(param1.currentTarget as Mobile));
      ...
      {
         ActiveCountManager.requestActiveCount(203339,function(param1:uint, param2:uint):void
         {
            var type:uint = param1;
            var val:uint = param2;
            DayLimitManager.getDoCount(841,function(param1:int):void
            {
      ...
      {
         QueueLoader.load(URLUtil.getActivityAnimation("BloodKingAct"),"swf",function(param1:ContentInfo):void
         {
      ...
      private function addMenu() : void
      {
         ...
         this._menu.addEventListener("click",this.onMenu);
      }
