package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_80392 extends MapProcessor
   {
      ...
      {
         super.init();
         ActiveCountManager.requestActiveCountList([205498,205499],function(param1:Parser_1142):void
         {
            ...
            _isLast = t == 1;
            DayLimitManager.getDoCount(1590,function(param1:uint):void
            {
      ...
      private function createNpc() : void
      {
         ...
         {
            _loc1_ = new Mobile();
            _loc1_.setPostion(this.NPC_POS[_loc2_]);
            _loc1_.resourceUrl = URLUtil.getNpcSwf(this.NPC_ID[_loc2_]);
            _loc1_.labelPosition = 1;
            _loc1_.label = this.NPC_LABEL[_loc2_];
            _loc1_.labelImage.y = -_loc1_.height - 10;
            _loc1_.buttonMode = true;
            MobileManager.addMobile(_loc1_,"npc");
            _loc1_.addEventListener("click",this.onNpcClick);
            this._npc.push(_loc1_);
            _loc2_++;
         }
      }
      ...
      protected function onNpcClick(param1:MouseEvent) : void
      {
         ...
         index = int(this._npc.indexOf(event.currentTarget as Mobile));
         ...
         {
            ShopManager.buyItemForId(605428,function():void
            {
      ...
      private function onComplete(param1:SceneEvent) : void
      {
         ...
         {
            index = int(this.FIGHT_LIST.indexOf(FightManager.currentFightRecord.initData.positionIndex));
            if(index != -1)
            {
