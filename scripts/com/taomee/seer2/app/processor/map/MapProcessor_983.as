package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_983 extends MapProcessor
   {
      ...
      private function initOver() : void
      {
         ...
         this._actBtn.addEventListener("click",this.showActPanel);
         ActiveCountManager.requestActiveCountList([202997,203101],this.getArmy);
      }
      ...
      private function onActorArriveShow(param1:NpcEvent) : void
      {
         ...
         else
         {
            NpcDialog.show(npcid,npcName,[[0,"经过数周的激战，我们的第二届精灵争霸赛落下帷幕了，祝贺你 " + ActorManager.actorInfo.nick + "！ " + this.ARMY_NAME[this.army_value - 1] + "为你而自豪。 请接受属于你的" + this.RANK_NAME[this.army_value - 1] + "奖励！"]],["我要领奖！","想一下获奖感言再领..."],[function():void
            {
               ...
               {
                  SwapManager.swapItem(1759,1,getReward);
               }]);
            }]);
         }
      }
      ...
      private function petKingInit() : void
      {
         ...
         this._scoreGate.addEventListener("click",this.onGate);
         this._ruleGate.addEventListener("click",this.onGate);
      }
      ...
      private function petKingDispose() : void
      {
         if(this._scoreGate)
         {
            this._scoreGate.removeEventListener("click",this.onGate);
            this._scoreGate = null;
         }
         if(this._ruleGate)
         {
            this._ruleGate.removeEventListener("click",this.onGate);
            this._ruleGate = null;
         }
         if(this._awardOne)
         {
            this._awardOne.removeEventListener("click",this.onAward);
            this._awardOne = null;
         }
         if(this._awardTwo)
         {
            this._awardTwo.removeEventListener("click",this.onAward);
            this._awardTwo = null;
         }
      }
      ...
      private function createMiya() : void
      {
         ...
         {
            _loc2_ = new Mobile();
            _loc2_.resourceUrl = URLUtil.getNpcSwf(this.npcIdList[_loc1_]);
            _loc2_.buttonMode = true;
            _loc2_.addEventListener("click",this.onNpcClick);
            _loc2_.setPostion(this.miyaPosition[_loc1_]);
            MobileManager.addMobile(_loc2_,"npc");
            this.miya.push(_loc2_);
            _loc1_++;
         }
      }
      ...
      private function onNpcClick(param1:MouseEvent) : void
      {
         ...
         index = int(this.miya.indexOf(e.currentTarget as Mobile));
         left = 0;
      ...
      private function disposeMiya() : void
      {
         ...
         {
            (this.miya[_loc1_] as Mobile).removeEventListener("click",this.onNpcClick);
            MobileManager.removeMobile(this.miya[_loc1_],"npc");
            _loc1_++;
         }
         this.miya = null;
      }
      ...
      override public function dispose() : void
      {
         ...
         if(this._actBtn)
         {
            this._actBtn.removeEventListener("click",this.showActPanel);
         }
      }
   }
}
