package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_1190 extends MapProcessor
   {
      ...
      private var dayLimit:Array = [0,0,0,0];
      ...
      override public function dispose() : void
      {
         ...
         while(_loc1_ < 4)
         {
            this.guard_list[_loc1_].removeEventListener("click",this.toFightBadu);
            _loc1_++;
         }
         if(this.zhua_mobile)
         {
            this.zhua_mobile.removeEventListener("click",this.toFightZhua);
         }
      }
      ...
      override public function init() : void
      {
         ...
         var _loc1_:int = 0;
         while(_loc1_ < this.DAYLIMIT_IDS.length)
         {
            _loc2_.writeUnsignedInt(this.DAYLIMIT_IDS[_loc1_]);
            _loc1_++;
         }
         DayLimitListManager.getDoCount(_loc2_,this.getGuardState);
      }
      ...
      private function initMobile() : void
      {
         ...
         {
            _loc1_ = new Mobile();
            this.guard_list.push(_loc1_);
            this.guard_list[_loc2_].x = this.GUARD_SEAT[_loc2_].x;
            this.guard_list[_loc2_].y = this.GUARD_SEAT[_loc2_].y;
            _loc2_++;
         }
      }
      ...
      private function getGuardState(param1:DayLimitListInfo) : void
      {
         ...
         while(_loc3_ < 4)
         {
            ...
            {
               this.guard_list[_loc3_].resourceUrl = URLUtil.getNpcSwf(598);
            }
            else
            {
               this.guard_list[_loc3_].resourceUrl = URLUtil.getNpcSwf(597);
               this.guard_list[_loc3_].buttonMode = true;
               this.guard_list[_loc3_].mouseChildren = false;
               this.guard_list[_loc3_].addEventListener("click",this.toFightBadu);
               this.isCall = false;
            }
            MobileManager.addMobile(this.guard_list[_loc3_],"npc");
            _loc3_++;
         }
         DayLimitManager.getDoCount(733,this.getDayCount);
      ...
      private function showCandle() : void
      {
         ...
         QueueLoader.load(URLUtil.getQuestAnimation("sceneAnimation/10197"),"domain",this.onLoadComplete);
      }
      ...
      private function showZhua() : void
      {
         ...
         this.zhua_mobile.resourceUrl = URLUtil.getNpcSwf(595);
         this.zhua_mobile.x = 350;
         this.zhua_mobile.y = 285;
         MobileManager.addMobile(this.zhua_mobile,"npc");
         this.zhua_mobile.addEventListener("click",this.toFightZhua);
      }
      ...
      private function toFightZhua(param1:MouseEvent) : void
      {
         ...
         NpcDialog.show(113,"超级NONO",[[0,"每天有3次挑战机会，您已经挑战" + over + "次！是否立即开始挑战？"]],["开始挑战","准备一下"],[function():void
         {
            ...
            FightManager.startFightWithBoss(514);
         }]);
      }
      
      private function onFightOver(param1:FightStartEvent) : void
      {
         FightManager.removeEventListener("FIGHT_OVER",this.onFightOver);
