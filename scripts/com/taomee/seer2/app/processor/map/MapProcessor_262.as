package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_262 extends TitleMapProcessor
   {
      ...
      {
         this.initRarePet();
         StatisticsManager.sendNovice("0x10033443");
      }
      ...
      private function initRarePet() : void
      {
         ...
         this.hideRarePet();
         this._info = RarePetConfig.getRarePetInfo(75);
         RarePetManager.addEventListener("startTimeUp",this.onRarePetShow);
         if(RarePetManager.isInTimeRange(this._info) != 0)
         {
            _loc1_ = uint(TimeManager.getServerTime() - RarePetManager.getCookTime(75));
         ...
         this.stoneGame.addEventListener("click",this.toClickStone);
      }
      ...
      private function showRarePet() : void
      {
         if(this._isShow == false)
         {
            ...
            this._rarePetMC.addEventListener("click",this.onRarePetClick);
         }
      }
      ...
      private function hideRarePet() : void
      {
         ...
         this._rarePetMC.removeEventListener("click",this.onRarePetClick);
      }
      ...
      private function onRarePetClick(param1:MouseEvent) : void
      {
         ...
         {
            RarePetManager.startFight(75);
            RarePetManager.addCookTime(75,TimeManager.getServerTime());
            this.hideRarePet();
         }
      }
      ...
      {
         RarePetManager.removeEventListener("startTimeUp",this.onRarePetShow);
         this.removeTimer();
         this.hideRarePet();
         this._rarePetMC.removeEventListener("end",this.buildEnd);
         ShootController.removeEventListener("playEnd",this.onShootEnd);
