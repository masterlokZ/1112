package com.taomee.seer2.app.processor.activity.openSchool
{
   ...
   public class OpenSchool
   {
      ...
      {
         this._npc = MobileManager.getMobile(65,"npc");
      ...
      private function addPlayNpc(param1:String, param2:uint) : void
      {
         ...
         if(this._index != 3)
         {
            ...
            this._cheer.addEventListener("click",this.onCheer,false,1);
            TooltipManager.addCommonTip(this._cheer,"加速1小时要消耗1000赛尔豆");
            this._playNpc.addEventListener("click",this.onPanel);
            this._playNpc.addEventListener("mouseOver",this.onOver);
            this._playNpc.addEventListener("mouseOut",this.onOut);
            this.addTimeMC();
         }
         else
         {
            this._playNpc.buttonMode = true;
            this._playNpc.addEventListener("click",this.onNpc);
         }
      ...
      private function updateTime(param1:uint) : void
      {
         ...
         var _loc2_:uint = uint(this.timeList[this._index] - (TimeManager.getServerTime() - (this._completeCount - this.timeList[this._index])));
         this._timeMC["mc"].scaleX = _loc2_ / this.timeList[this._index];
         var _loc3_:uint = (this._completeCount - TimeManager.getServerTime()) / 60;
      ...
      {
         ServerBufferManager.getServerBuffer(41,this.onOpenSchool);
      }
      ...
      private function onOpenSchool(param1:ServerBuffer) : void
      {
         ...
         if(server.readDataAtPostion(0) != 1)
         {
            ...
            {
               ServerBufferManager.updateServerBuffer(41,0,1);
               nextNpc();
            }]);
         }
         else
      ...
      private function nextNpc() : void
      {
         ...
         {
            QuestManager.addEventListener("stepComplete",this.onQuestStepComplete);
      ...
      {
         this._npc.visible = false;
         QuestManager.removeEventListener("stepComplete",this.onQuestStepComplete);
      ...
      public function dispose() : void
      {
         ...
         if(this._playNpc)
         {
            this._playNpc.removeEventListener("mouseOver",this.onOver);
            this._playNpc.removeEventListener("mouseOut",this.onOut);
         }
         Tick.instance.removeRender(this.updateTime);
         this._index = 0;
         this._timeMC = null;
