package com.taomee.seer2.app.processor.activity.expDoorAct
{
   ...
   public class ExpDoorAct
   {
      ...
      public function ExpDoorAct(param1:MapModel)
      {
         ...
         {
            _curIndex = -1;
            if(SceneManager.prevSceneType == 1 && _par.infoVec[0] == 0)
            {
            ...
            else if(SceneManager.prevSceneType == 2)
            {
               ...
               else if(FightManager.fightWinnerSide == 1)
               {
                  if(FightManager.currentFightRecord.initData.positionIndex == FIGHT_INDEX_LIST[_curIndex])
                  {
                     if(_curIndex == 2)
                     {
                        ModuleManager.addEventListener("ExpResultPanel","hide",onResultHide);
                     ...
                     {
                        ModuleManager.addEventListener("ExpResultPanel","hide",onResultHide1);
      ...
      private function onResultHide(param1:ModuleEvent) : void
      {
         ...
         ModuleManager.removeEventListener("ExpResultPanel","hide",this.onResultHide);
      ...
      {
         var evt:ModuleEvent = param1;
         ModuleManager.removeEventListener("ExpResultPanel","hide",this.onResultHide1);
         this._startMcList[this._curIndex + 1].visible = true;
         MovieClipUtil.playMc(this._startMcList[this._curIndex + 1],2,this._startMcList[this._curIndex + 1].totalFrames,function():void
         {
            _startMcList[_curIndex + 1].gotoAndStop(1);
      ...
      private function initEvent() : void
      {
         ...
         this._openPetBag.addEventListener("click",this.onOpenPetBag);
         this._leave.addEventListener("click",this.onLeave);
         for each(_loc1_ in this._miBuyList)
         {
            _loc1_.addEventListener("click",this.onMiBuy);
         }
      }
      
      private function onMiBuy(param1:MouseEvent) : void
      {
         var _loc2_:int = int(this._miBuyList.indexOf(param1.currentTarget as SimpleButton));
      ...
      {
         var _loc6_:Mobile = null;
         var _loc5_:int = 0;
         while(_loc5_ < param3)
         {
            _loc6_ = new SpawnedPet(param2,param1,param4);
            MobileManager.addMobile(_loc6_,"spawnedPet");
            _loc5_++;
         }
         this.updateAllExp();
      }
      ...
      public function dispose() : void
      {
         ...
         if(this._openPetBag)
         {
            this._openPetBag.removeEventListener("click",this.onOpenPetBag);
         }
         if(this._leave)
         {
         ...
         if(this._miBuyList)
         {
            for each(_loc1_ in this._miBuyList)
            {
               _loc1_.removeEventListener("click",this.onMiBuy);
            }
         }
         if(this._itemList)
         {
