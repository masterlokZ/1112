package com.taomee.seer2.app.processor.activity.fightCounter
{
   ...
   public class FightBearProcess
   {
      ...
      private function init() : void
      {
         ...
         this._house.addEventListener("click",this.onClickHouseHandler);
         if(SceneManager.prevSceneType == 2 && FightManager.currentFightRecord.initData.positionIndex == 243)
         {
         ...
         if(SceneManager.prevSceneType == 2 && FightManager.currentFightRecord.initData.positionIndex == 244)
         {
            if(FightManager.fightWinnerSide == 1)
            {
               this.winSecond();
         ...
         if(SceneManager.prevSceneType == 2 && FightManager.currentFightRecord.initData.positionIndex == 245)
         {
            if(FightManager.fightWinnerSide == 1)
            {
               return;
      ...
      private function onRandomStatus(param1:MessageEvent) : void
      {
         ...
         if(id == 0)
         {
            ...
            this._house.removeEventListener("click",this.onClickHouseHandler);
            MovieClipUtil.playMc(this._counterMC,1,this._counterMC.totalFrames,function():void
            {
               _counterMC.buttonMode = true;
               _counterMC.addEventListener("click",onFightHandler);
            });
         }
         else if(id == 1)
         {
         ...
         else if(id == 2)
         {
            if(SceneManager.active.mapID == 10109)
            {
               MovieClipUtil.playFullScreen(URLUtil.getActivityAnimation("AbaoTravel/full1"),function():void
               {
                  ...
                  _house.removeEventListener("click",onClickHouseHandler);
                  _counterMC = getMovie("couonter");
                  SceneManager.active.mapModel.content.addChild(_counterMC);
                  _counterMC.buttonMode = true;
                  _counterMC.addEventListener("click",onFightCounterHandler);
               },true,false,2);
            }
            else if(SceneManager.active.mapID == 101090)
            {
      ...
      {
         var url:String = URLUtil.getActivityAnimation("AbaoTravel/fightCounter");
