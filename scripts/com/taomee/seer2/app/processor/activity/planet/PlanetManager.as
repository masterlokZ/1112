   ...
   public class PlanetManager
   {
      ...
      private static function onSeer(param1:ItemEvent) : void
      {
         ...
         while(_loc3_ < _info.idList.length)
         {
            ...
            _loc2_.addEventListener("rollOver",onOver);
            _loc2_.addEventListener("rollOut",onOut);
            _loc2_.addEventListener("click",onIcon);
      ...
      private static function updateInfo() : void
      {
         ...
         var _loc1_:Array = [];
         var _loc4_:int = 0;
         while(_loc4_ < _info.idList.length)
         {
            _loc3_ = ItemManager.getSpecialItem(_info.idList[_loc4_]);
            if(!_loc3_)
            {
               _loc2_.push(_info.idList[_loc4_]);
               _loc1_.push(_info.swapList[_loc4_]);
            }
            _loc4_++;
         }
         _info.idList = _loc2_;
         _info.swapList = _loc1_;
      }
      ...
      private static function onIcon(param1:MouseEvent) : void
      {
         ...
         index = int(_iconVec.indexOf(icon));
         SwapManager.swapItem(_info.swapList[index],1,function(param1:IDataInput):void
         {
            ...
            {
               QueueLoader.load(URLUtil.getActivityAnimation("planet/title"),"swf",onResLoaded);
            },true,false,2);
         });
         if(_info.idList[0] == 501005)
         {
            StatisticsManager.sendNovice("0x1003339D");
         }
         if(_info.idList[0] == 501006)
         {
            StatisticsManager.sendNovice("0x10033598");
         }
         if(_info.idList[0] == 501007)
         {
            StatisticsManager.sendNovice("0x10033897");
         }
      }
