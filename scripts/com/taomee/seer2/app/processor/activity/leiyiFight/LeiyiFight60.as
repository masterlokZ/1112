package com.taomee.seer2.app.processor.activity.leiyiFight
{
   ...
   public class LeiyiFight60
   {
      ...
      {
         super();
         ModuleManager.addEventListener("AppearResultPanel","dispose",this.onDisposePanel);
         ...
         this._npc = MobileManager.getMobile(154,"npc");
         DialogPanel.addEventListener("dialogShow",this.toShowDilog);
      }
      
      private function toShowDilog(param1:DialogPanelEvent) : void
      {
         var _loc2_:CustomUnit = new CustomUnit("active","开学高效资质训练","SchoolOpenAptitudeTrainPanel");
         var _loc4_:CustomUnit = new CustomUnit("active","开学极速成长训练","ShoolOpenGrowUpPanel");
         var _loc3_:CustomUnit = new CustomUnit("active","开学有奖智慧考验","SeerIIAnswerPanel");
      ...
      {
         if(SceneManager.prevSceneType == 2 && FightManager.currentFightRecord.initData.positionIndex == 113)
         {
      ...
      private function onData(param1:MessageEvent) : void
      {
         ...
         while(_loc4_ < _loc5_)
         {
            _loc2_ = uint(_loc3_.readUnsignedInt());
            this._num = _loc3_.readUnsignedInt();
            _loc4_++;
         }
         this.GameOver();
      }
      ...
      private function onDisposePanel(param1:ModuleEvent) : void
      {
         if(SceneManager.prevSceneType == 2 && FightManager.currentFightRecord.initData.positionIndex == 113)
         {
            ModuleManager.removeEventListener("AppearResultPanel","dispose",this.onDisposePanel);
            if(this._num < 10)
            {
      ...
      public function dispose() : void
      {
         ...
         DialogPanel.removeEventListener("dialogShow",this.toShowDilog);
