package com.taomee.seer2.app.processor.activity.zhouSiAct
{
   ...
   public class ZhouSiAct
   {
      
      private static var _self:ZhouSiAct;
      
      private static const SEARCH_NUM_FOR:uint = 204078;
      
      private static const FIND_MAP_NUM_FOR:uint = 204079;
      
      private static const SEARCH_NUM_DAY:int = 1095;
      
      private static const GIFT_GET_MINE:int = 316;
      
      private static const USE_SWAP:int = 2722;
      
      private static const MI_NUM_BUY_ID:uint = 400481;
      
      private static const RANDOM_ID:int = 89;
      ...
      {
         QueueLoader.load(URLUtil.getActivityAnimation("zhouSiAct/ZhouSiAct"),"swf",function(param1:ContentInfo):void
         {
      ...
      {
         this._miNumBuy.addEventListener("click",this.onMiNumBuy);
         this._forward.addEventListener("click",this.onForward);
         this._leave.addEventListener("click",this.onLeave);
      }
      ...
      private function onForward(param1:MouseEvent) : void
      {
         ...
         if(this.forwardFilter())
         {
            ...
            MovieClipUtil.playMc(this._forwardMC,2,this._forwardMC.totalFrames,function():void
            {
               ...
               Connection.send(CommandSet.RANDOM_EVENT_1140,89,0);
            });
         }
      }
      ...
      private function onEventStart(param1:MessageEvent) : void
      {
         ...
         if(parser.index == 89)
         {
            ...
            if(parser.id == 1)
            {
               ModuleManager.showModule(URLUtil.getAppModule("ZhouSiEventOneActPanel"),"正在打开面板...",{"type":this.getRandom(1,4)});
            }
            else if(parser.id == 2)
            {
               ...
               MovieClipUtil.playMc(sceneMC,2,sceneMC.totalFrames,function():void
               {
                  ...
                  sceneMC = null;
                  ModuleManager.showModule(URLUtil.getAppModule("ZhouSiEventTwoActPanel"),"正在打开面板...",{"func":function(param1:Boolean):void
                  {
                     ...
                     if(isFinish)
                     {
                        ...
                        MovieClipUtil.playMc(sceneMC,2,sceneMC.totalFrames,function():void
                        {
                           DisplayUtil.removeForParent(sceneMC);
                           sceneMC = null;
                           SwapManager.swapItem(2722,1,function(param1:IDataInput):void
                           {
                              new SwapInfo(param1);
                              _menu.visible = true;
                           },null);
            ...
            {
               ModuleManager.showModule(URLUtil.getAppModule("ZhouSiEventThreeActPanel"),"正在打开面板...");
            }
            else if(parser.id == 4)
            {
               this._menu.visible = false;
               ...
               MovieClipUtil.playMc(sceneMC,2,sceneMC.totalFrames,function():void
               {
                  DisplayUtil.removeForParent(sceneMC);
                  sceneMC = null;
                  ModuleManager.addEventListener("MiniGameConsolePanel","dispose",function(param1:ModuleEvent):void
                  {
                     _menu.visible = true;
                  });
                  ModuleManager.showModule(URLUtil.getAppModule("MiniGameConsolePanel"),"正在打开小游戏...",{"gameId":26});
               },true);
            }
            else if(parser.id == 5)
            {
            ...
            else if(parser.id == 6)
            {
               ...
               MovieClipUtil.playMc(sceneMC,2,sceneMC.totalFrames,function():void
               {
                  DisplayUtil.removeForParent(sceneMC);
                  sceneMC = null;
                  ModuleManager.addEventListener("ZhouSiEventFourActPanel","dispose",function(param1:ModuleEvent):void
                  {
                     addRandomGift();
                  });
                  ModuleManager.showModule(URLUtil.getAppModule("ZhouSiEventFourActPanel"),"正在打开遇到悬浮物提示面板...");
               },true);
            }
            else if(parser.id == 7)
            {
               sceneMcNameList = ["SceneMC_5","SceneMC_6","SceneMC_7","SceneMC_8"];
               ActiveCountManager.requestActiveCount(204079,function(param1:uint, param2:uint):void
               {
                  ...
                  if(204079 == type)
                  {
                     ...
                     MovieClipUtil.playMc(sceneMC,2,sceneMC.totalFrames,function():void
                     {
                        DisplayUtil.removeForParent(sceneMC);
                        sceneMC = null;
                        ModuleManager.showModule(URLUtil.getAppModule("ZhouSiMeetBossActPanel"),"正在打开遇到boss挑战事件提示面板...",{"type":val});
                     },true);
                  }
               });
            }
      ...
      private function addRandomGift() : void
      {
         ...
         while(_loc1_ < 5)
         {
            _loc2_ = this.getMovie("RandomGift");
            this._giftList.push(_loc2_);
            _loc2_.x = 215 + Math.random() * 700;
            _loc2_.y = 50 + Math.random() * 400;
            this._map.content.addChild(_loc2_);
            _loc2_.buttonMode = true;
            _loc2_.addEventListener("click",this.onGiftClick);
            _loc1_++;
         }
      }
      ...
      private function clearGiftList() : void
      {
         ...
         if(this._giftList)
         {
            ...
            {
               this._giftList[_loc1_].removeEventListener("click",this.onGiftClick);
      ...
      private function onGiftClick(param1:MouseEvent) : void
      {
         ...
         index = int(this._giftList.indexOf(evt.currentTarget as MovieClip));
         MineManger.getMineItem(316,function(param1:Parser_1060):void
         {
            param1.showResult();
      ...
      private function onMiNumBuy(param1:MouseEvent) : void
      {
         ...
         ModuleManager.addEventListener("BuyPropPanel","dispose",this.onBuyPanelHide);
      ...
      private function onBuyPanelHide(param1:ModuleEvent) : void
      {
         ...
         ModuleManager.removeEventListener("BuyPropPanel","dispose",this.onBuyPanelHide);
         this.update();
      }
      ...
      private function update(param1:Function = null) : void
      {
         ...
         this._forward.mouseEnabled = false;
         DayLimitManager.getDoCount(1095,function(param1:int):void
         {
            var val:int = param1;
            ActiveCountManager.requestActiveCount(204078,function(param1:uint, param2:uint):void
            {
               var _loc3_:int = 0;
               if(param1 == 204078)
               {
      ...
      public function dispose() : void
      {
         ...
         if(this._miNumBuy)
         {
            this._miNumBuy.removeEventListener("click",this.onMiNumBuy);
            DisplayUtil.removeForParent(this._miNumBuy);
            this._miNumBuy = null;
         }
         if(this._forward)
         {
            this._forward.removeEventListener("click",this.onForward);
            DisplayUtil.removeForParent(this._forward);
            this._forward = null;
         }
         if(this._leave)
         {
            this._leave.removeEventListener("click",this.onLeave);
