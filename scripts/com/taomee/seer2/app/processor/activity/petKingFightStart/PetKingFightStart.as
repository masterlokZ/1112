package com.taomee.seer2.app.processor.activity.petKingFightStart
{
   ...
   public class PetKingFightStart
   {
      ...
      {
         var _loc2_:int = int(mapIdList.indexOf(SceneManager.active.mapID));
         if(_loc2_ == -1)
         {
            return;
         }
         ...
         {
            if(FightManager.currentFightRecord && FightManager.currentFightRecord.initData && Boolean(FightManager.currentFightRecord.initData.type) && FightManager.currentFightRecord.initData.type == "fightPVPWohoo")
            {
               ServerMessager.addMessage("您获得了2块争霸赛奖牌!");
            }
            else if(FightManager.currentFightRecord && FightManager.currentFightRecord.initData && Boolean(FightManager.currentFightRecord.initData.type) && FightManager.currentFightRecord.initData.type == "fightPVPTrainer")
            {
               ServerMessager.addMessage("您获得了2块争霸赛奖牌!");
            }
