package com.taomee.seer2.app.processor.activity.sniper
{
   ...
   public class Sniper
   {
      ...
      private static var _rankingInfoList:Vector.<SniperInfo>;
      ...
      {
         SceneManager.addEventListener("switchStart",onSwitchStart);
      ...
      private static function onSwitchComplete(param1:SceneEvent) : void
      {
         ...
         if(_index != -1)
         {
            _boss = MobileManager.getMobile(514,"npc");
            _boss.visible = false;
            startBlood();
         }
      }
      ...
      private static function startRanking() : void
      {
         ...
         _rankingInfoList = Vector.<SniperInfo>([]);
      ...
      private static function onRanking(param1:MessageEvent) : void
      {
         ...
         while(_loc2_ < _loc5_)
         {
            _loc3_ = new SniperInfo();
            _loc3_.userId = _loc4_.readUnsignedInt();
            _loc3_.nick = _loc4_.readUTFBytes(16);
            _loc3_.hurt = _loc4_.readUnsignedInt();
            _rankingInfoList.push(_loc3_);
            _loc2_++;
         }
         addRanking();
      }
      ...
      private static function addRanking() : void
      {
         ...
         while(i < _rankingInfoList.length)
         {
            ...
            mc["proportion"].text = "(" + (_rankingInfoList[i].hurt / 200000000 * 100).toFixed(1) + "%" + ")";
            mc.x = 20;
            mc.y = 110 + i * 25;
         ...
         DayLimitManager.getDoCount(477,function(param1:uint):void
         {
            ...
            _totalMC["proportion"].text = "(" + (param1 / 200000000 * 100).toFixed(1) + "%" + ")";
            _totalMC.x = 20;
      ...
      private static function sortVec(param1:SniperInfo, param2:SniperInfo) : int
      {
      ...
      private static function onGetBlood(param1:MessageEvent) : void
      {
         ...
         var _loc5_:uint;
         _currCount = _loc5_ = uint(_loc3_.readUnsignedInt());
         ...
         if(_currCount <= 0)
         {
            ...
            for each(_loc2_ in _rankingList)
            {
      ...
      private static function startEvent() : void
      {
         if(_rec)
         {
            ...
            _rec.addEventListener("click",onBoss);
         }
      }
