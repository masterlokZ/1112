package com.taomee.seer2.app.processor.activity.pkKingCallAct
{
   ...
   public class PkKingCallAct
   {
      ...
      private static function onComplete(param1:SceneEvent = null) : void
      {
         ...
         if(SceneManager.prevSceneType == 2 && FightManager.fightWinnerSide == 1 && FightManager.currentFightRecord.initData.positionIndex == 1617)
         {
            ...
            DayLimitListManager.getDaylimitList(DAY_LIST,function(param1:DayLimitListInfo):void
            {
               ...
               while(i < 3)
               {
                  if(!BitUtil.getBit(par.infoVec[0],i))
                  {
                     ...
                     if(i > 0 && BitUtil.getBit(par.infoVec[0],i - 1))
                     {
                        _curGateIndex = i;
                     }
                  }
               ...
               ServerBufferManager.getServerBuffer(389,function(param1:ServerBuffer):void
               {
                  var _loc2_:int = 0;
                  while(_loc2_ < 3)
                  {
      ...
      {
         SceneManager.changeScene(9,MAP_LIST[1]);
      }
      ...
      private static function addNpc() : void
      {
         ...
         _npc.addEventListener("click",onNpc);
      }
            ...
            else
            {
               NpcDialog.show(400,"小赛尔",[[0,"缪斯！缪斯！最强王者争霸赛就要开始了，来参加嘛？"]],["雷伊他们都会来哟~"],[function():void
               {
                  NpcDialog.show(705,"缪斯",[[0,"哦？他们也要参赛？"]],["他们是三个队伍的领袖哦。"],[function():void
                  {
                     ...
                     {
                        ServerBufferManager.updateServerBuffer(389,_curGateIndex + 1,1);
         ...
         if(_curGateIndex == 1)
         {
            if(_talkState[_curGateIndex])
            {
               ...
               else
               {
                  ...
                  {
                     FightManager.startFightWithWild(1617);
                  }]);
               }
            }
            else
      ...
      private static function removeNpc() : void
      {
         ...
         {
            removeMouseHint();
            _npc.removeEventListener("click",onNpc);
