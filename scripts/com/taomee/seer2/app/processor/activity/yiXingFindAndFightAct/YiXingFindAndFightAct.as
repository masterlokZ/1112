package com.taomee.seer2.app.processor.activity.yiXingFindAndFightAct
{
   ...
   public class YiXingFindAndFightAct
   {
      ...
      private var _isTickShow:Vector.<Boolean> = Vector.<Boolean>([false,false,false,false,false,false,false,false,false,false,false]);
      ...
      {
         QueueLoader.load(URLUtil.getActivityAnimation("yiXingFindAndFightAct"),"swf",function(param1:ContentInfo):void
         {
      ...
      private function initRes() : void
      {
         ...
         this._hurtedMC.addEventListener("click",this.onHurtedClick);
      }
      ...
      private function onHurtedClick(param1:MouseEvent) : void
      {
         ...
         SwapManager.swapItem(2023,1,function(param1:IDataInput):void
         {
            ...
            MovieClipUtil.playMc(_hurtedMC,2,_hurtedMC.totalFrames,function():void
            {
               ...
               if(_curFindState < 5)
               {
                  ...
                  {
                     SceneManager.changeScene(1,FIND_MAP[_curFindState + 1]);
                  },true,false,2,true);
               }
               else
               {
      ...
      {
         if(DateUtil.isDay(2013,10,19) || DateUtil.isDay(2013,10,20))
         {
      ...
      public function getCurMapId(param1:Function = null) : void
      {
         ...
         DayLimitManager.getDoCount(837,function(param1:int):void
         {
         ...
         while(_loc2_ < this.TIME_REGION.length)
         {
            if(DateUtil.isInHourScope(this.TIME_REGION[_loc2_][0],this.TIME_REGION[_loc2_][2],this.TIME_REGION[_loc2_][1],this.TIME_REGION[_loc2_][3]))
            {
               if(!this._isTickShow[_loc2_])
               {
                  ...
                  if(!SceneManager.isSwitching && SceneManager.active && SceneManager.active.type == 1)
                  {
      ...
      private function getTimeIndex() : int
      {
         var _loc2_:* = -1;
         var _loc1_:int = 0;
         while(_loc1_ < this.TIME_REGION.length)
         {
            if(DateUtil.isInHourScope(this.TIME_REGION[_loc1_][0],this.TIME_REGION[_loc1_][2],this.TIME_REGION[_loc1_][1],this.TIME_REGION[_loc1_][3]))
            {
               _loc2_ = _loc1_;
               break;
            }
            if(Boolean(this.TIME_REGION[_loc1_ + 1]) && this.isBeforeHourScope(this.TIME_REGION[_loc1_ + 1][0],this.TIME_REGION[_loc1_ + 1][1]))
            {
               _loc2_ = _loc1_ + 1;
               break;
            }
      ...
      private function isBeforeHourScope(param1:uint, param2:uint = 0) : Boolean
      {
         ...
         var _loc3_:Date = new Date(_loc4_.fullYear,_loc4_.month,_loc4_.date,param1,param2);
         if(_loc4_ < _loc3_)
         {
            return true;
         }
      ...
      private function findHandleDispose() : void
      {
         if(this._hurtedMC)
         {
            this._hurtedMC.removeEventListener("click",this.onHurtedClick);
      ...
      private function fightHandleDispose() : void
      {
         Tick.instance.removeRender(this.onTimeCheck);
         this.removeBossNpc(1);
         SceneManager.removeEventListener("switchStart",this.onMapSwitchStart);
         SceneManager.removeEventListener("switchComplete",this.onMapSwitchComplete);
      }
      ...
      private function createBossNpc(param1:Array, param2:int = 0) : void
      {
         if(!this._bossNpc)
         {
            ...
            this._bossNpc.labelPosition = 1;
            this._bossNpc.label = "异形";
            this._bossNpc.labelImage.y = -this._bossNpc.height - 10;
            this._bossNpc.buttonMode = true;
            MobileManager.addMobile(this._bossNpc,"npc");
            this.showMouseHintAtMonster(this._bossNpc);
            if(param2 == 0)
            {
               this._bossNpc.addEventListener("click",this.onBossNpcClick);
            }
            else
            {
               this._bossNpc.addEventListener("click",this.onBossNpcClick1);
            }
         }
      }
      ...
      private function removeBossNpc(param1:int = 0) : void
      {
         if(this._bossNpc)
         {
            ...
            if(param1 == 0)
            {
               this._bossNpc.removeEventListener("click",this.onBossNpcClick);
            }
            else
            {
               this._bossNpc.removeEventListener("click",this.onBossNpcClick1);
            }
            ...
            NpcDialog.show(635,"异形",[[0,"哼！那你就来领教一下始祖吸血家族的厉害！"]],["来啊！谁怕谁啊！"],[function():void
            {
               NpcDialog.show(635,"异形",[[0,"口气倒不小，这个星球已经被我们弟兄侵入！处处都有我们的身影，有种就来找我们吧。"]],["你们跑不掉的！"],[function():void
               {
                  ...
                  getCurMapId(function(param1:int):void
                  {
                     ...
                     if(_curMapId != 0)
                     {
                        SceneManager.changeScene(1,_curMapId);
                     }
                  });
               }]);
            }]);
      ...
      private function onBossNpcClick1(param1:MouseEvent) : void
      {
         ...
         {
            FightManager.startFightWithWild(632);
         }]);
      }
