package com.taomee.seer2.app.processor.activity.fishingMatch
{
   ...
   public class FishingMatchManager
   {
      ...
      private var sceneArr:Array = [152,261,220,790];
      ...
      {
         SceneManager.addEventListener("switchComplete",this.changeScene);
      }
      ...
      private function changeScene(param1:SceneEvent) : void
      {
         ...
         while(_loc3_ < 4)
         {
            ...
            {
               FishEventDispatcher.getInstance().addEventListener("fishing",this.onFishing);
               break;
            }
            _loc3_++;
         }
      ...
      {
         this.npc = MobileManager.getMobile(443,"npc");
         this.npc.addOverHeadMark(new AcceptableMark());
         NpcManager.addEventListener("actorArrive",this.onNpc);
      }
      ...
      private function onNpc(param1:NpcEvent) : void
      {
         ...
         NpcManager.removeEventListener("actorArrive",this.onNpc);
      ...
      {
         var _loc2_:int = 0;
         var _loc1_:int = 400813;
         while(_loc1_ < 400817)
         {
            if(ItemManager.getItemQuantityByReferenceId(_loc1_) > 0)
            {
               _loc2_++;
            }
            _loc1_++;
      ...
      {
         FishEventDispatcher.getInstance().removeEventListener("fishing",this.onFishing);
         Connection.removeCommandListener(CommandSet.ITEM_EXCHANGE_1055,this.onGetReward);
         Connection.removeCommandListener(CommandSet.DIGGER_MINE_1060,this.showResult151);
         Connection.removeCommandListener(CommandSet.DAILY_LIMIT_1065,this.onDailyLimitHandler);
         DialogPanel.removeEventListener("customUnitClick",this.onUnit);
         SceneManager.removeEventListener("switchComplete",this.changeScene);
         this._map = null;
         if(this.npc)
         {
