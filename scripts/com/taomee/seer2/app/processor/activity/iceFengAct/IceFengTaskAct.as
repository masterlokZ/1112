package com.taomee.seer2.app.processor.activity.iceFengAct
{
   ...
   public class IceFengTaskAct
   {
      ...
      {
         super();
         SceneManager.addEventListener("switchComplete",this.onStateHandle);
         SceneManager.addEventListener("switchComplete",this.onStateDispose);
      }
      ...
      public function addFollow() : void
      {
         ...
         this._eggFollow.resourceUrl = URLUtil.getPetSwf(3030);
         ...
         this._eggFollow.addEventListener("click",this.onEggFollowClick);
      }
      ...
      private function onStateHandle(param1:SceneEvent) : void
      {
         ...
         {
            index = int(this.MAP_ID.indexOf(SceneManager.active.mapID));
            if(index != -1)
            {
      ...
      private function isNeedSetState() : Boolean
      {
         ...
         {
            if(SceneManager.prevSceneType == 2 && (this._par.infoVec[0] == 2 || this._par.infoVec[0] == 4))
            {
            ...
            if(this._par.infoVec[1] >= this.TASK_FINISH[this._par.infoVec[0] - 1] && TimeManager.getPrecisionServerTime() >= this._par.infoVec[2] + 180)
            {
               _loc1_ = true;
            }
         }
      ...
      private function updateFollowState() : void
      {
         ...
         if(TimeManager.getPrecisionServerTime() >= this._par.infoVec[2] + 180)
         {
         ...
         else
         {
            ...
            this._leaveTime = this._par.infoVec[2] + 180 - TimeManager.getPrecisionServerTime();
            ...
            this.getURL(function():void
            {
               if(_par.infoVec[1] == 0)
               {
                  if(SceneManager.active.mapModel)
                  {
                     ...
                     ItemManager.addEventListener1("mineSuccess",onMine);
                  }
               }
            });
         }
      ...
      private function onMine(param1:ItemEvent) : void
      {
         ...
         if(evt.content == this.MINE_ID_LIST[this._mineCurIndex])
         {
            DisplayUtil.removeForParent(this._tipMc);
            this._tipMc = null;
            ItemManager.removeEventListener1("mineSuccess",this.onMine);
            ServerMessager.addMessage("任务完成，获得20成熟度!");
            ActiveCountManager.requestActiveCountList(this.FOR_LIST,function(param1:Parser_1142):void
            {
               var par:Parser_1142 = param1;
      ...
      private function getURL(param1:Function = null) : void
      {
         ...
         QueueLoader.load(URLUtil.getActivityAnimation("iceFengTaskAct/IceFengTaskAct"),"swf",function(param1:ContentInfo):void
         {
      ...
      private function fightHandleInit(param1:int) : void
      {
         ...
         if(this._par.infoVec[0] == this.MAP_ID.indexOf(SceneManager.active.mapID) + 1)
         {
            ...
            else if(SceneManager.prevSceneType == 2 && FightManager.fightWinnerSide == 1 && FightManager.currentFightRecord.initData.positionIndex <= 9)
            {
               ActiveCountManager.requestActiveCountList(this.FOR_LIST,function(param1:Parser_1142):void
               {
                  var par:Parser_1142 = param1;
