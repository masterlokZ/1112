package com.taomee.seer2.app.processor.activity.frozen
{
   ...
   public class FrozenManager
   {
      ...
      private var _actor:FrozenActor;
      
      private var _frpzen:Frozen;
      
      private var _randomFrpzen:int;
      
      private var _frozenPointList:Vector.<FrozenPoint>;
      ...
      public static function isActivityTime() : Boolean
      {
         ...
         if(DateUtil.isInHourScope(14,15,30,30) || DateUtil.isInHourScope(18,19,30,30))
         {
            _loc1_ = true;
         }
         return _loc1_;
      ...
      private function init() : void
      {
         this.initPoint();
         this.initActor();
         this._frpzen = new Frozen(this._map.content["frozen"]);
         FrozenBubble.setupBubble();
         this._actor.thisActor.addEventListener("move",this.onMove);
         this._map.content.addEventListener("enterFrame",this.onEnterFrame);
      ...
      {
         this._frozenPointList = Vector.<FrozenPoint>([]);
         var _loc1_:int = 0;
         while(_loc1_ < 5)
         {
            this._frozenPointList.push(new FrozenPoint(this._map.content["point" + _loc1_],this._map.content["thisPoint" + _loc1_]));
            _loc1_++;
         }
      }
      ...
      private function onFrozenInfo(param1:MessageEvent) : void
      {
         ...
         this._frpzen.setType(_loc4_);
         this._frpzen.setHp(_loc3_);
         this._type = _loc4_;
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         var _loc7_:Actor = ActorManager.getActor();
         _loc3_ = ArrayUtil.toArray(ActorManager.getAllRemoteActors());
         _loc3_.push(_loc7_);
         var _loc2_:int = 0;
         while(_loc2_ < 5)
         {
            _loc5_ = this._frozenPointList[_loc2_];
            _loc6_ = false;
            _loc4_ = 0;
            while(_loc4_ < _loc3_.length)
            {
               if(_loc5_.thisMc.hitTestObject(_loc3_[_loc4_]))
               {
                  _loc6_ = true;
               }
               _loc4_++;
            }
            this.isPlayIndex(_loc6_,_loc2_);
            _loc2_++;
         }
      }
      ...
      {
         var _loc3_:FrozenPoint = this._frozenPointList[param2];
         this._index = param2;
      ...
      {
         var _loc1_:FrozenPoint = null;
         var _loc2_:int = 0;
         for each(_loc1_ in this._frozenPointList)
         {
            if(_loc1_.status)
            {
               _loc2_++;
         ...
         FrozenBubble.enterBubble(_loc2_);
      }
      
      private function onMove(param1:MoveEvent) : void
      {
         var _loc3_:FrozenPoint = null;
         var _loc4_:Boolean = false;
         var _loc5_:Actor = ActorManager.getActor();
         if(this._actor.getBlood())
         {
            this._actor.getBlood().x = _loc5_.x - 65;
            this._actor.getBlood().y = _loc5_.y - 100;
         }
         var _loc2_:int = 0;
         while(_loc2_ < 5)
         {
            _loc3_ = this._frozenPointList[_loc2_];
            _loc4_ = false;
            if(_loc3_.thisMc.hitTestObject(_loc5_))
            {
               _loc4_ = true;
            }
            this.verification(_loc4_,_loc2_);
            _loc2_++;
         }
      }
      ...
      private function onFrozenActivityType(param1:MessageEvent) : void
      {
         ...
         _loc7_ = param1.message.getRawData();
         ...
         while(_loc4_ < _loc8_)
         {
            ...
            {
               case 0:
                  this._type = _loc7_.readUnsignedInt();
                  this.updateType();
                  break;
               case 1:
            ...
            if(_loc9_ == 4)
            {
               ...
               if(this._actor.thisActor.getInfo().id == _loc5_)
               {
               ...
               {
                  _loc2_ = UpdateActivityActor.getActor(_loc5_);
                  _loc2_.releaseUser();
               }
            }
            _loc4_++;
      ...
      {
         var userList:Vector.<FrozenActor>;
         var actor:FrozenActor = null;
         var hp:uint = 0;
      ...
      private function onShootOver(param1:ShootEvent) : void
      {
         ...
         var _loc4_:Vector.<FrozenActor> = UpdateActivityActor.getList();
         for each(_loc3_ in _loc4_)
         {
            if(_loc3_.actor.hitTestPoint(_loc2_.endPos.x,_loc2_.endPos.y))
            {
               if(_loc2_.id == 3 && _loc3_.status == false)
               {
                  Connection.send(CommandSet.FROZEN_FIGHT_1154,_loc3_.actor.getInfo().id);
                  _loc3_.releaseUser();
                  return;
               }
            }
      ...
      public function dispose() : void
      {
         ...
         if(this._actor)
         {
            ...
            this._actor.thisActor.removeEventListener("move",this.onMove);
         }
         if(this._frpzen)
         {
            this._frpzen.removeBlood();
         ...
         if(this._map)
         {
            this._map.content.removeEventListener("enterFrame",this.onEnterFrame);
         }
         Connection.removeErrorHandler(CommandSet.FROZEN_DEFEND_1155,this.onError);
