package com.taomee.seer2.app.processor.activity.ghostMorph
{
   ...
   public class GhostMorphManager
   {
      ...
      private static var _actorList:Vector.<GhostActor>;
      
      private static var _currActor:GhostActor;
      ...
      private static function onComplete(param1:SceneEvent) : void
      {
         ...
         var index:int = int(mapIdList.indexOf(SceneManager.active.mapID));
         if(GhostMorphLibrary.checkTime() && index != -1)
         {
            _actorList = Vector.<GhostActor>([]);
            _currPointArr = pointArr[index];
            _bufArr = bufList[index];
            _load = new DevilLoad();
            _load.playLoad("ghostMorph/ghostMorph",function():void
            {
               ItemManager.addEventListener1("requestSpecialItemSuccess",requestItemSuccess);
      ...
      private static function onGameComplete(param1:Event) : void
      {
         ...
         SwapManager.swapItem(673,1,function(param1:IDataInput):void
         {
            ...
            ServerBufferManager.updateServerBuffer(56,40,1);
      ...
      {
         ItemManager.removeEventListener1("requestSpecialItemSuccess",requestItemSuccess);
         ServerBufferManager.getServerBuffer(56,onGhostMorph);
      }
      
      private static function onGhostMorph(param1:ServerBuffer) : void
      {
         var _loc2_:MovieClip = null;
         var _loc4_:GhostActor = null;
         var _loc3_:int = 0;
         _ser = param1;
         ...
         _loc3_ = 0;
         while(_loc3_ < _bufArr.length)
         {
            if(param1.readDataAtPostion(_bufArr[_loc3_]) != 1)
            {
               _loc2_ = _load.getMC("ghost" + _loc3_);
               _loc2_.x = _currPointArr[_loc3_].x;
               _loc2_.y = _currPointArr[_loc3_].y;
               _loc4_ = new GhostActor();
               _loc4_.mc = _loc2_;
               _loc4_.index = _bufArr[_loc3_];
               _loc4_.addMC();
               SceneManager.active.mapModel.content.addChild(_loc4_);
               _loc4_.addEventListener("click",onClick);
               _loc4_.update();
               _actorList.push(_loc4_);
            }
            _loc3_++;
         }
      }
      ...
      {
         _currActor = param1.currentTarget as GhostActor;
         _ran = Math.random() * 30;
         if(_ran < 10)
         {
            _ran = 1;
      ...
      private static function clickYes() : void
      {
         ...
         {
            ServerBufferManager.updateServerBuffer(56,_currActor.index,1);
            str = "";
            switch(int(_ran))
            {
               case 0:
            ...
            SwapManager.swapItem(682,1,function(param1:IDataInput):void
            {
               ...
               ServerBufferManager.updateServerBuffer(56,1,_loc2_);
               if(_loc2_ >= 10)
               {
                  ServerBufferManager.updateServerBuffer(56,49,1);
