package com.taomee.seer2.app.processor.copy.handler
{
   ...
   public class CopyProcessor_80025 extends CopyProcessor
   {
      
      private const BOSS_INDEX:Vector.<int> = Vector.<int>([447,448,449,450]);
      ...
      public function CopyProcessor_80025(param1:CopyItemInfo)
      {
         super(param1);
      }
      ...
      {
         StatisticsManager.sendNovice("0x10033874");
      }
      ...
      private function onComplete(param1:SceneEvent) : void
      {
         ...
         if(SceneManager.prevSceneType == 2)
         {
            if(FightManager.fightWinnerSide == 1)
            {
               ...
               {
                  posIndex = int(this.BOSS_INDEX.indexOf(index));
                  if(posIndex == 0)
                  {
                     this.cartoonHandle("80025_2","sceneTalk_2",function():void
                     {
                        _stateOneNpc.removeEventListener("click",onNpcClick);
                        ...
                        _stateTwoNpc.addEventListener("click",onNpcClick);
                     });
                  }
                  else if(posIndex == 1)
                  {
                     this.cartoonHandle("80025_3","sceneTalk_3",function():void
                     {
                        var _loc2_:ActorPreview = null;
                        DisplayUtil.removeForParent(_stateTwoNpc);
                        _stateTwoNpc.removeEventListener("click",onNpcClick);
                        _stateTwoNpc = null;
                        _stateThreeNpc = new Vector.<ActorPreview>();
                        var _loc1_:int = 0;
                        while(_loc1_ < 3)
                        {
                           _loc2_ = new ActorPreview();
                           _loc2_.setData(ActorManager.actorInfo);
                           if(_loc1_ == 0)
                           {
                              _loc2_.scaleX = -ACTOR_SCALE[_loc1_];
                              _loc2_.scaleY = ACTOR_SCALE[_loc1_];
                           }
                           else
                           {
                              _loc2_.scaleX = ACTOR_SCALE[_loc1_];
                           ...
                           _loc2_.x = POS_LIST[2][_loc1_][0];
                           _loc2_.y = POS_LIST[2][_loc1_][1];
                           _loc2_.buttonMode = true;
                           _mapModel.content.addChildAt(_loc2_,0);
                           _loc2_.mask = _mapModel.content["mark" + _loc1_];
                           _stateThreeNpc.push(_loc2_);
                  ...
                  {
                     this.cartoonHandle("80025_4","sceneTalk_4",function():void
                     {
                        var _loc1_:ActorPreview = null;
                        for each(_loc1_ in _stateThreeNpc)
                        {
                           _loc1_.removeEventListener("click",onNpcClick);
                        ...
                        _stateFourNpc.addEventListener("click",onNpcClick);
                     });
                  }
                  else if(posIndex == 3)
                  {
      ...
      private function clearEvent() : void
      {
         ...
         if(this._stateOneNpc)
         {
            this._stateOneNpc.removeEventListener("click",this.onNpcClick);
         }
         if(this._stateTwoNpc)
         {
            this._stateTwoNpc.removeEventListener("click",this.onNpcClick);
         }
         if(this._stateFourNpc)
         {
            this._stateFourNpc.removeEventListener("click",this.onNpcClick);
         }
         if(this._stateThreeNpc)
         {
            for each(_loc1_ in this._stateThreeNpc)
            {
               _loc1_.removeEventListener("click",this.onNpcClick);
            }
         }
      }
