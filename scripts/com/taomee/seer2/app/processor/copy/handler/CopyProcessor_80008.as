package com.taomee.seer2.app.processor.copy.handler
{
   ...
   public class CopyProcessor_80008 extends CopyProcessor
   {
      ...
      {
         StatisticsManager.sendNovice("0x1003375E");
      }
         ...
         if(SceneManager.prevSceneType == 2)
         {
            if(FightManager.fightWinnerSide == 1)
            {
               ...
               if(index == 341)
               {
                  ...
                  if(this._killNum < 3)
                  {
                     ServerMessager.addMessage("你还需要击败" + (3 - this._killNum) + "只精灵就可以和伊莉塔对战！");
                  }
                  else
                  {
      ...
      private function updateMonster() : void
      {
         ...
         while(_loc1_ < _loc2_)
         {
            _loc3_ = new SpawnedPet(170,341,0);
      ...
      private function onBossClick(param1:MouseEvent) : void
      {
         ...
         switch(param1.currentTarget)
         {
            case this._stateOneBoss:
               if(this._killNum < 3)
               {
                  AlertManager.showAlert("你还需要击败" + (3 - this._killNum) + "只精灵就可以和伊莉塔对战！");
               }
               break;
