package com.taomee.seer2.app.processor.copy.handler
{
   ...
   public class CopyProcessor_80015 extends CopyProcessor
   {
      
      private const BOSS_INDEX:Vector.<int> = Vector.<int>([380,381,382]);
      
      private const MONSTER_INDEX:int = 379;
      ...
      public function CopyProcessor_80015(param1:CopyItemInfo)
      {
         super(param1);
      }
      ...
      {
         StatisticsManager.sendNovice("0x1003379D");
      }
      ...
      {
         this.cartoonHandle("80014_1","",function():void
         {
         ...
         if(SceneManager.prevSceneType == 2)
         {
            if(FightManager.fightWinnerSide == 1)
            {
               ...
               if(index == 379)
               {
                  ...
                  if(this._killNum < 1)
                  {
                     ServerMessager.addMessage("你还需要击败" + (1 - this._killNum) + "只精灵就可以和泰瑞纳斯对战！");
                  }
                  else
                  {
                     ...
                     this.cartoonHandle("80014_2","",function():void
                     {
               ...
               else if(this.BOSS_INDEX.indexOf(index) != -1)
               {
                  ...
                  if(posIndex == 0)
                  {
                     this.cartoonHandle("80014_3","",function():void
                     {
                  ...
                  {
                     this.cartoonHandle("80014_4","",function():void
                     {
      ...
      private function updateMonster() : void
      {
         ...
         while(_loc1_ < _loc2_)
         {
            _loc3_ = new SpawnedPet(170,379,0);
      ...
      private function onBossClick(param1:MouseEvent) : void
      {
         ...
         switch(param1.currentTarget)
         {
            case this._stateOneBoss:
               if(this._killNum < 1)
               {
                  AlertManager.showAlert("你需要击败" + (1 - this._killNum) + "只精灵就可以和泰瑞纳斯对战！");
               }
               break;
