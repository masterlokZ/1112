package com.taomee.seer2.app.processor.quest
{
   ...
   public class QuestProcessor
   {
      ...
      private function initQuest() : void
      {
         ...
         for each(_loc2_ in this.quest.getRelatedMapIds())
         {
            _loc1_ = this.createMapHandler(_loc2_);
            if(_loc1_ != null)
            {
               this._questHandlerMap.add(_loc2_,_loc1_);
            }
         }
      ...
      private function createMapHandler(param1:int) : QuestMapHandler
      {
         ...
         if(this._quest.type == 1)
         {
            _loc2_ += "main.";
         }
         else if(this._quest.type == 2 || this._quest.type == 3 || this._quest.type == 4 || this._quest.type == 6)
         {
            _loc2_ += "branch.";
         }
         else if(this._quest.type == 0)
         {
            _loc2_ += "story.";
         }
      ...
      public function processMapComplete() : void
      {
         ...
         {
            QueueLoader.load(URLUtil.getQuestAnimation("sceneAnimation/" + this._quest.id),"domain",this.onLoadComplete);
         }
         else
         {
            this.initHandlerMap();
      ...
      {
         var _loc5_:Rectangle = param1.getBounds(LayerManager.stage);
         var _loc4_:Number = _loc5_.left + _loc5_.width / 2;
         var _loc3_:Number = _loc5_.top;
         if(param2 != null)
         {
            _loc4_ += param2.x;
            _loc3_ += param2.y;
         }
         this.showMouseHintAt(_loc4_,_loc3_,null,false);
      }
