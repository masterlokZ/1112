package com.taomee.seer2.app.processor.quest.handler.main.quest97
{
   ...
   public class QuestMapHandler_97_3854 extends QuestMapHandler
   {
      ...
      {
         this._oldNpc = MobileManager.getMobile(param1,"npc");
         ...
         MobileManager.addMobile(this._newNpc,"npc");
         this._newNpc.addEventListener("click",this.onClickNpc);
      }
      ...
      private function afterFinish() : void
      {
         if(this._newNpc)
         {
            this._newNpc.removeEventListener("click",this.onClickNpc);
      ...
      {
         MovieClipUtil.getSwfContent(URLUtil.getActivityAnimation("findFireGameUI"),(function():*
         {
      ...
      private function onFireClick(param1:MouseEvent) : void
      {
         ...
         {
            this._fireMcList[i].removeEventListener("click",this.onFireClick);
            i++;
         }
         index = int(this._fireMcList.indexOf(evt.currentTarget as MovieClip));
         if(-1 != index)
         {
            DisplayObjectUtil.removeFromParent(this._cameraMc.parent);
            TweenNano.delayedCall(1,(function():*
            {
               var onComplete:Function;
               return onComplete = function():void
               {
                  changeNpc(3020);
      ...
      private function initGameEvent() : void
      {
         ...
         while(i < this._fireMcList.length)
         {
            this._fireMcList[i].addEventListener("click",this.onFireClick);
            i++;
         }
      }
      ...
      override public function processMapDispose() : void
      {
         ...
         if(this._newNpc)
         {
            if(this._newNpc.hasEventListener("click"))
            {
               this._newNpc.removeEventListener("click",this.onClickNpc);
            }
            this._newNpc = null;
