package com.taomee.seer2.app.processor.quest.handler.branch.quest10130
{
   ...
   public class Game10130
   {
      ...
      public function Game10130(param1:MovieClip, param2:int, param3:Function)
      {
         ...
         this._closeBtn.addEventListener("click",this.dispose);
         this._fun = param3;
      ...
      private function initMC0() : void
      {
         ...
         LayerManager.topLayer.addEventListener("mouseDown",this.onDown);
         LayerManager.topLayer.addEventListener("mouseUp",this.onUp);
      }
      
      private function onDown(param1:MouseEvent) : void
      {
         LayerManager.topLayer.addEventListener("enterFrame",this.onMove);
      ...
      {
         LayerManager.topLayer.removeEventListener("enterFrame",this.onMove);
         this._isDown = false;
      ...
      {
         LayerManager.topLayer.removeEventListener("mouseDown",this.onDown);
         LayerManager.topLayer.removeEventListener("mouseUp",this.onUp);
         LayerManager.topLayer.removeEventListener("enterFrame",this.onMove);
      ...
      private function initMC1() : void
      {
         ...
         while(_loc1_ < 5)
         {
            ...
            this._coinsList[_loc1_].addEventListener("click",this.onCoins);
            _loc1_++;
         }
      }
      ...
      private function onCoins(param1:MouseEvent) : void
      {
         ...
         var _loc2_:int = int(this._coinsList.indexOf(param1.currentTarget as MovieClip));
         this._coinsList[_loc2_].gotoAndStop(2);
         this._conisBo[_loc2_] = true;
         var _loc4_:uint = 0;
         for each(_loc3_ in this._conisBo)
         {
            if(_loc3_)
            {
               _loc4_++;
      ...
      private function initMC2() : void
      {
         ...
         this._title.addEventListener("click",this.onTitle);
         this._content = this._currMC["content"];
         this._content.gotoAndStop(1);
         this._content.addEventListener("click",this.onContent);
         var _loc1_:int = 0;
         while(_loc1_ < 3)
         {
            this._colorList.push(this._currMC["color" + _loc1_]);
            this._colorList[_loc1_].addEventListener("click",this.onColor);
            _loc1_++;
         }
      }
      ...
      {
         var _loc2_:int = int(this._colorList.indexOf(param1.currentTarget as SimpleButton));
