package com.taomee.seer2.app.processor.quest.handler.branch.quest10079
{
   ...
   public class QuestMapHandler_10079_810 extends QuestMapHandler
   {
      ...
      private var _mapPointList:Array = [[522,114],[860,321],[555,472],[661,248],[804,432]];
      ...
      override public function processMapComplete() : void
      {
         ...
         {
            DialogPanel.addEventListener("customReplyClick",this.onTestFightHandler);
         }
         if(QuestManager.isStepComplete(_quest.id,1) && QuestManager.isStepComplete(_quest.id,2) == false)
         {
            DialogPanel.addEventListener("customReplyClick",this.onFirstFightHandler);
      ...
      private function onAcceptHandler(param1:QuestEvent) : void
      {
         ...
         {
            DialogPanel.addEventListener("customReplyClick",onTestFightHandler);
         }]);
      }
      ...
      private function createPets() : void
      {
         ...
         while(_loc2_ < 5)
         {
            _loc1_ = new ActivePet(new Point(this._mapPointList[_loc2_][0],this._mapPointList[_loc2_][1]),1045);
            _loc1_.addEventListener("click",this.onPetHandler);
            _loc1_.targetPoint = new Point(380,400);
            MobileManager.addMobile(_loc1_,"modelPet");
            this._pets.push(_loc1_);
            _loc2_++;
         }
      ...
      private function onPetHandler(param1:MouseEvent) : void
      {
         ...
         var _loc4_:int = int(this._pets.indexOf(_loc2_));
         if(_loc4_ != -1)
         {
            this._pets.splice(_loc4_,1);
         }
         _loc3_ = int(this._pets.length);
         if(_loc3_ == 0)
         {
      ...
      private function hasHitOn(param1:int) : void
      {
         ...
         {
            _loc2_ = this._pets[_loc3_];
            if((Boolean(_loc2_)) && HitTest.complexHitTestObject(_loc2_,this._mc_0))
            {
               _loc6_ = HitTest.intersectionRectangle(_loc2_,this._mc_0);
               ++this._hitCount;
               if(this._hitCount > 10)
               {
                  this._petCount = 0;
                  ...
                  _loc4_ = 0;
                  while(_loc4_ < this._pets.length)
                  {
                     MobileManager.removeMobile(this._pets[_loc4_],"modelPet");
                     _loc4_++;
                  }
                  this._pets.length = 0;
               }
               else
               {
                  ...
                  this._petPoint = new Point(_loc6_.x,_loc6_.y + _loc6_.height);
                  this.loadPetEffect(0);
                  MobileManager.removeMobile(_loc2_,"modelPet");
                  _loc7_ = int(this._pets.indexOf(_loc2_));
                  if(_loc7_ != -1)
                  {
                     this._pets.splice(_loc7_,1);
                  }
                  _loc5_ = int(this._pets.length);
                  if(_loc5_ == 0)
                  {
                     if(this._petCount >= 5 && this._hitCount <= 10)
                     {
      ...
      private function onStepOne(param1:QuestEvent) : void
      {
         ...
         {
            DialogPanel.addEventListener("customReplyClick",onFirstFightHandler);
         }]);
      }
      ...
      private function initExchangePanel() : void
      {
         ...
         ItemManager.requestEquipList(function():void
         {
            ...
            {
               if(_loc8_[_loc7_].referenceId == 100358)
               {
                  thing2Coin = 1;
               }
               if(_loc8_[_loc7_].referenceId == 100359)
               {
                  thing3Coin = 1;
               }
               if(_loc8_[_loc7_].referenceId == 100360)
               {
                  thing4Coin = 1;
               }
               if(_loc8_[_loc7_].referenceId == 100361)
               {
                  thing5Coin = 1;
               }
               _loc7_++;
      ...
      override public function processMapDispose() : void
      {
         DialogPanel.removeEventListener("customReplyClick",this.onStartHandler);
         DialogPanel.addEventListener("customReplyClick",this.onTestFightHandler);
         DialogPanel.removeEventListener("customReplyClick",this.onFirstFightHandler);
      }
   }
}
