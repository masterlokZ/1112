package com.taomee.seer2.app.processor.quest.handler.story.quest49
{
   ...
   public class QuestMapHandler_49_840 extends QuestMapHandler
   {
      ...
      override public function processMapComplete() : void
      {
         ...
         {
            DialogPanel.addEventListener("customReplyClick",this.onNpcTalkHandler);
         }
      }
      ...
      private function npcTalk() : void
      {
         NpcDialog.show(489,"卡特琳娜",[[0,"吾……不想出手……汝等还是回去吧…"]],["我不是来和你打架的！"],[function():void
         {
            NpcDialog.show(489,"卡特琳娜",[[0,"请回吧……这里没有任何汝等需要的东西……"]],["（等等，我怎么越看她越眼熟……）"],[function():void
            {
               ...
               {
                  QuestManager.addEventListener("stepComplete",onStepOneHandler);
      ...
      {
         QuestManager.removeEventListener("stepComplete",this.onStepOneHandler);
         this.queenTalk();
      }
      ...
      private function onNpcTalkHandler(param1:DialogPanelEvent) : void
      {
         ...
         {
            DialogPanel.removeEventListener("customReplyClick",this.onNpcTalkHandler);
      ...
      private function intFireBall() : void
      {
         ...
         ShootController.addEventListener("playEnd",this.shootWaterHandler);
         this._preArr = [];
         this._afterArr = [];
         this.createPreBall();
      ...
      private function checkHitBall(param1:int) : void
      {
         ...
         _loc2_ = MobileManager.getMobile(489,"npc");
      ...
      private function gameOver() : void
      {
         ...
         while(_loc2_ < this._preArr.length)
         {
            if((this._preArr[_loc2_] as MovieClip).parent)
            {
               DisplayObjectUtil.removeFromParent(this._preArr[_loc2_]);
            }
            _loc2_++;
         ...
         var _loc1_:uint = 0;
         while(_loc1_ < this._afterArr.length)
         {
            if((this._afterArr[_loc1_] as MovieClip).parent)
            {
               DisplayObjectUtil.removeFromParent(this._afterArr[_loc1_]);
            }
            _loc1_++;
         }
