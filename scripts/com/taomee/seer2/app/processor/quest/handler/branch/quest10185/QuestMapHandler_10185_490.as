package com.taomee.seer2.app.processor.quest.handler.branch.quest10185
{
   ...
   public class QuestMapHandler_10185_490 extends QuestMapHandler
   {
      ...
      override public function processMapDispose() : void
      {
         ...
         {
            this.monkeyMobile.removeEventListener("click",this.clickMonkey);
         }
         QuestManager.removeEventListener("stepComplete",this.completeStep);
         if(this.bananaList)
         {
            _loc1_ = 0;
            while(_loc1_ < this.bananaList.length)
            {
               this.bananaList[_loc1_].removeEventListener("click",this.pickupBanana);
               _loc1_++;
            }
         }
      }
      ...
      private function createMonkey() : void
      {
         ...
         this.monkeyMobile.resourceUrl = URLUtil.getNpcSwf(577);
         ...
         MobileManager.addMobile(this.monkeyMobile,"npc");
         this.monkeyMobile.addEventListener("click",this.clickMonkey);
      }
      
      private function clickMonkey(param1:MouseEvent) : void
      {
         this.sceneMc = _processor.resLib.getMovieClip("ThrowBanana");
      ...
      private function throwBananaComplete() : void
      {
         ...
         this.sceneMc = _processor.resLib.getMovieClip("PickupBanana");
      ...
      private function lightBanana(param1:MovieClip) : void
      {
         ...
         param1.addEventListener("click",this.pickupBanana);
      }
      
      private function pickupBanana(param1:MouseEvent) : void
      {
         var _loc2_:int = int(this.bananaList.indexOf(param1.target as MovieClip));
         var _loc3_:MovieClip = this.bananaList[_loc2_];
         _loc3_.removeEventListener("click",this.pickupBanana);
         _loc3_.stop();
      ...
      {
         NpcDialog.show(577,"火爆猴",[[0,"叫你捡个香蕉皮都这么慢！你到底是不是机器人啊你！"]],["明明是你扔的……"],[function():void
         {
            ...
            {
               NpcDialog.show(577,"火爆猴",[[0,"我最开心的事情就是发火！你懂吗！发火火火！！别来教训我，你们最好都离我远点！"]],["不如，我带你去个好地方，你一定会开心的！"],[playFull]);
            }]);
         }]);
      }
      ...
      private function playFull() : void
      {
         ...
         {
            QuestManager.addEventListener("stepComplete",completeStep);
      ...
      private function completeStep(param1:QuestEvent) : void
      {
         ...
         {
            QuestManager.removeEventListener("stepComplete",this.completeStep);
            SceneManager.changeScene(1,340);
         }
      }
   }
}
