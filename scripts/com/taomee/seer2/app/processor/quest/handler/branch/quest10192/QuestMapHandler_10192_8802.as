package com.taomee.seer2.app.processor.quest.handler.branch.quest10192
{
   ...
   public class QuestMapHandler_10192_8802 extends QuestMapHandler
   {
      ...
      override public function processMapComplete() : void
      {
         ...
         if(QuestManager.isAccepted(_quest.id) && QuestManager.isStepComplete(_quest.id,1) == false)
         {
            ...
            if(_isDialogHeben)
            {
               ...
               ShootController.addEventListener("playStart",this.onOpen);
               ShootController.addEventListener("playEnd",this.onPlayEnd);
            }
         }
      ...
      private function startQuest1() : void
      {
         ...
         this._s.addEventListener("click",this.onS);
      }
      ...
      private function onS(param1:MouseEvent) : void
      {
         ...
         NpcDialog.show(192,"S",[[0,"怎么回事？什么也看不见啊？小赛尔你在哪里？"]],["我在这！"],[function():void
         {
            ...
            {
               _isDialogHeben = true;
               ShootController.removeEventListener("playEnd",onPlayEnd);
               ShootController.removeEventListener("playStart",onOpen);
               _processor.showMouseHintAt(370,493);
               _isShoot = false;
               ShootController.addEventListener("playStart",onOpen);
               ShootController.addEventListener("playEnd",onPlayEnd);
            }]);
         }]);
      }
      ...
      {
         ShootController.removeEventListener("playStart",this.onOpen);
         ...
         if(info.userID == ActorManager.actorInfo.id && info.id == 3)
         {
            ...
            if(inde != 100)
            {
               MovieClipUtil.playMc(this._xueList[inde],2,this._xueList[inde].totalFrames,function():void
               {
                  ...
                  var _loc2_:* = 1;
                  for each(_loc1_ in _xueIndexList)
                  {
                     if(_loc1_ == 0)
                     {
                        _loc2_ = _loc1_;
      ...
      override public function processMapDispose() : void
      {
         ...
         this.removeNpc();
         ShootController.removeEventListener("playStart",this.onOpen);
         ShootController.removeEventListener("playEnd",this.onPlayEnd);
         QuestManager.removeEventListener("complete",this.onComplete);
         QuestManager.removeEventListener("stepComplete",this.onStepComplete);
      }
   }
