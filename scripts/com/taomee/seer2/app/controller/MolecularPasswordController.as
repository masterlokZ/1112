package com.taomee.seer2.app.controller
{
   ...
   public class MolecularPasswordController
   {
      ...
      {
         if(SceneManager.active && SceneManager.active.type == 1 && SceneManager.active.mapModel.id == 30)
         {
            DialogPanel.removeEventListener("customUnitClick",onCustomUnitClick);
         }
      }
      ...
      private static function onCustomUnitClick(param1:DialogPanelEvent) : void
      {
         ...
         if(evt.content.params == "molecularPassword")
         {
            ...
            StatisticsManager.sendNovice("0x1003347B");
         }
      ...
      private static function onQuerySuccess(param1:MessageEvent) : void
      {
         ...
         var _loc7_:int = int(_loc5_.readUnsignedShort());
         var _loc6_:int = int(_loc5_.readUnsignedShort());
         var _loc3_:Vector.<ItemDescription> = new Vector.<ItemDescription>();
         var _loc2_:int = int(_loc5_.readUnsignedInt());
         var _loc4_:int = 0;
         while(_loc4_ < _loc2_)
         {
            _loc3_.push(new ItemDescription(_loc5_.readUnsignedInt(),_loc5_.readUnsignedShort()));
            _loc4_++;
         }
      ...
      public static function receive(param1:int, param2:String, param3:Array) : void
      {
         ...
         while(_loc4_ < _loc5_)
         {
            _loc6_.writeUnsignedInt(param3[_loc4_][0]);
            _loc6_.writeShort(param3[_loc4_][1]);
            _loc4_++;
         }
         ...
         Connection.send(CommandSet.MAGIC_NUM_USE_1068,_loc6_);
      }
      ...
      private static function onUseSuccess(param1:MessageEvent) : void
      {
         ...
         var _loc8_:LittleEndianByteArray = param1.message.getRawData();
         var _loc10_:int = int(_loc8_.readUnsignedInt());
         _loc9_ = int(_loc8_.readUnsignedByte());
         var _loc4_:* = _loc9_ == 0;
         var _loc3_:Vector.<ItemDescription> = new Vector.<ItemDescription>();
         var _loc7_:int = int(_loc8_.readUnsignedInt());
         var _loc6_:int = 0;
         while(_loc6_ < _loc7_)
         {
            _loc5_ = _loc8_.readUnsignedInt();
            _loc2_ = _loc8_.readUnsignedShort();
            _loc3_.push(new ItemDescription(_loc5_,_loc2_));
            if(_loc4_)
            {
               if(_loc5_ == 1)
               {
                  ActorManager.actorInfo.coins += _loc2_;
                  ActorManager.actorInfo.dispatchEvent(new UserInfoEvent("updateCoin"));
               }
            }
            _loc6_++;
         }
         if(_loc4_)
         {
            AlertManager.showItemListGaindeAlert(_loc3_);
            ItemManager.addItemList(_loc3_);
         }
         else
         {
      ...
      private static function onPasswordFail(param1:MessageEvent) : void
      {
         ...
         Connection.removeCommandListener(CommandSet.MAGIC_NUM_USE_1068,onUseSuccess);
         var _loc2_:uint = param1.message.statusCode;
         switch(int(_loc2_) - 100001)
         {
            case 0:
...
class PasswordView extends Sprite
{
   ...
   public function PasswordView()
   {
      ...
      this._passwordTxt.type = "input";
      this._passwordTxt.restrict = "0-9a-zA-Z";
      this._confirmBtn = this._mainUI["confirmBtn"];
      this._passwordTxt.addEventListener("change",this.onPasswordChange);
      this._confirmBtn.addEventListener("click",this.onComfirmBtnClick);
   }
   
   private function onPasswordChange(param1:Event) : void
   {
      if(StringUtil.trim(this._passwordTxt.text) == "")
      {
   ...
   {
      DialogPanel.hide();
      var _loc2_:String = StringUtil.trim(this._passwordTxt.text);
