package com.taomee.seer2.app.manager
{
   ...
   public class TheSpriteOfDragonManager
   {
      ...
      private function onResLoaded(param1:ContentInfo) : void
      {
         ...
         SimpleButton(this.callMc["callBtn"]).addEventListener("click",this.onCall);
         this.fightMc = this._mapFront["fightMc"];
         this.fightMc["modeBtn0"].addEventListener("click",this.onModeFight);
         this.fightMc["modeBtn1"].addEventListener("click",this.onModeFight);
         this.fightMc["swapBtn"].addEventListener("click",this.onSwapPet);
         this.winTxt = this.fightMc["winTxt"];
         this.callSuccessMc = this._mapFront["callSuccessMc"];
         SimpleButton(this.callSuccessMc["fightCallBtn"]).addEventListener("click",this.onFightCall);
         SimpleButton(this.callSuccessMc["fightCallNoBtn"]).addEventListener("click",this.onFightNoCall);
         this.fightFailMc = this._mapFront["fightFailMc"];
         this.fightFailMc["starBtn"].addEventListener("click",this.onBuyPass);
         this.fightFailMc["thanksBtn"].addEventListener("click",this.onBuyClose);
         this.longZhuBtn = this._mapFront["longZhuBtn"];
         this.longZhuBtn.addEventListener("click",this.onLongZhu);
         this.shopBtn = this._mapFront["shopBtn"];
         this.shopBtn.addEventListener("click",this.onShop);
      ...
      private function onBuyPass(param1:MouseEvent) : void
      {
         ...
         var idx:int = int(this.FIGHT_ID_ARR.indexOf(FightManager.currentFightRecord.initData.positionIndex));
      ...
      {
         var e:MouseEvent = param1;
         SwapManager.swapItem(4244,1,function(param1:IDataInput):void
         {
            new SwapInfo(param1);
            updateData();
         });
      ...
      {
         SceneManager.addEventListener("switchComplete",this.onCompleteSwitch);
         if(this.callNum == 1)
         {
      ...
      private function onCompleteSwitch(param1:SceneEvent) : void
      {
         ...
         {
            SceneManager.removeEventListener("switchComplete",this.onCompleteSwitch);
      ...
      private function updateShow() : void
      {
         ...
         var _loc2_:int = 0;
         var _loc1_:int = 0;
         while(_loc1_ < this.PET_ID_ARR.length)
         {
            if(BitUtil.getBit(this.dragonBit,_loc1_))
            {
               _loc2_++;
               this.dragonMcList[_loc1_].gotoAndStop(3);
            }
            else
            {
               this.dragonMcList[_loc1_].gotoAndStop(1);
               if(SimpleButton(this.dragonMcList[_loc1_]["reLifeBtn"]).hasEventListener("click") == false)
               {
                  SimpleButton(this.dragonMcList[_loc1_]["reLifeBtn"]).addEventListener("click",this.onReLife);
               }
               if(SimpleButton(this.dragonMcList[_loc1_]["buyLifeBtn"]).hasEventListener("click") == false)
               {
                  SimpleButton(this.dragonMcList[_loc1_]["buyLifeBtn"]).addEventListener("click",this.onBuyReLife);
               }
               TooltipManager.addCommonTip(this.dragonMcList[_loc1_]["zhuMc"],this.TIPS_ARR[_loc1_] + "之柱，\n拥有该龙就可点亮龙柱，也可用星钻一键点亮");
            }
            _loc1_++;
         }
      ...
      private function onCall(param1:MouseEvent) : void
      {
         ...
         else
         {
            ...
            MovieClipUtil.playMc(this.movieMc,2,this.movieMc.totalFrames,function():void
            {
               ...
               SwapManager.swapItem(4196,1,function(param1:IDataInput):void
               {
                  new SwapInfo(param1);
                  updateData();
               });
      ...
      {
         var e:MouseEvent = param1;
         var idx:int = int(this.dragonMcList.indexOf(e.currentTarget.parent));
      ...
      private function onReLife(param1:MouseEvent) : void
      {
         var idx:int = 0;
         var e:MouseEvent = param1;
         idx = int(this.dragonMcList.indexOf(e.currentTarget.parent));
         PetIsHaveManager.requestIsHavePet(this.PET_ID_ARR,function():void
         {
            if(PetIsHaveManager.petIsHave(PET_ID_ARR[idx]))
            {
               SwapManager.swapItem(4195,1,function(param1:IDataInput):void
               {
                  new SwapInfo(param1);
      ...
      {
         ServerBufferManager.getServerBuffer(418,function(param1:ServerBuffer):void
         {
            var _loc2_:Boolean = Boolean(param1.readDataAtPostion(1));
            if(!_loc2_)
            {
               ServerBufferManager.updateServerBuffer(418,1,1);
               ModuleManager.showAppModule("TheSpriteOfDragonRulePanel");
            }
         });
      }
