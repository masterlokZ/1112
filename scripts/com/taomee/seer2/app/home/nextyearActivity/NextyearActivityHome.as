package com.taomee.seer2.app.home.nextyearActivity
{
   ...
   public class NextyearActivityHome
   {
      ...
      private function processSelfHome() : void
      {
         ...
         this._nextyear.removeEventListener("click",this.onSelfClick);
         if(this._info.isYes == 1)
         {
            TooltipManager.addCommonTip(this._nextyear,"领取春节红包");
            this._nextyear.addEventListener("click",this.onSelfClick);
         }
         else
         {
      ...
      private function onGetGiftList(param1:MessageEvent) : void
      {
         ...
         var _loc8_:uint = uint(_loc6_.readUnsignedByte());
         _loc7_ = Vector.<uint>([]);
         var _loc3_:int = 0;
         while(_loc3_ < _loc8_)
         {
         ...
         var _loc2_:uint = uint(_loc6_.readUnsignedByte());
         var _loc5_:int = 0;
         while(_loc5_ < _loc2_)
         {
            _loc7_.push(2);
         ...
         this._nextCount = _loc8_ + _loc2_;
         var _loc4_:int = 0;
         while(_loc4_ < _loc7_.length)
         {
            this._getGiftVec[_loc4_].visible = true;
            this._getGiftVec[_loc4_].buttonMode = true;
            this._getGiftVec[_loc4_].gotoAndStop(_loc7_[_loc4_]);
            this._getGiftVec[_loc4_].addEventListener("click",this.onMouse);
            _loc4_++;
         }
      }
      ...
      private function initGetGift() : void
      {
         ...
         this._getGiftPanel["closeBtn"].addEventListener("click",this.onGetPanelClose);
