package mx.resources
{
   ...
   public class LocaleSorter
   {
      ...
      {
         var noLocales:int;
         var noPreferenceLocales:int;
         var preferenceLocalesID:Vector.<LocaleID>;
         var localesID:Vector.<LocaleID>;
         ...
         noLocales = int(locales.length);
         noPreferenceLocales = int(preferenceLocales.length);
         for(j = 0; j < noLocales; j++)
         {
            hasLocale[locales[j]] = j;
         }
         preferenceLocalesID = new Vector.<LocaleID>(noPreferenceLocales);
         for(i = 0; i < noPreferenceLocales; i++)
         {
            preferenceLocalesID[i] = LocaleID.fromString(preferenceLocales[i]);
         }
         localesID = new Vector.<LocaleID>(noLocales);
